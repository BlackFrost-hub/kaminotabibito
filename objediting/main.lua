...
6. dofile 只能用相对文件名（同目录），不能用路径（arg 不可用）。
  ...
  -- 这套 ObjEditing 的真实单位/技能 API 需要去 `.def/def/` 查定义：
  -- 例如单位图标字段 `uico` 对应 `.def/def/UnitOrBuildingOrHeroDefinition.lua`
  -- 真实 setter 是 `setIconGameInterface(...)`。
dofile('abilities.lua')
dofile('units.lua')
dofile('Unit/Unit.lua')
dofile('SystemAbility/SystemAbilityTemplates.lua')
dofile('HeroAbility/HeroAbility.lua')
dofile('Equipment/Equipment.lua')
dofile('Boss/Boss.lua')
dofile('Destructable/Destructable.lua')
dofile('Doodad/Doodad.lua')
