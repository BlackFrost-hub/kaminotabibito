...
createPlayerHeroChannelAbility('AKQ1', '塞莉亚·克莱尔-棱晶魔弹（Q）', {
  ...
  tooltip = '棱晶魔弹（Q）', tooltipExtended = '技能说明：向目标方向发射棱晶魔弹，命中后建立演算节点；场上的节点可使后续魔弹折射、穿透或追踪。|n伤害：主弹造成攻击力100%的魔法伤害；联动追加伤害最高为攻击力60%。|n施法距离：900|n伤害范围：命中半径100|n节点数量：最多2个|n冷却时间：5秒|n魔法消耗：最大魔法值的5.5%',
  icon = ICON, hotkey = 'Q', buttonX = 0, buttonY = 2, cooldown = 5, percentManaCost = 0.055,
})
...
createPlayerHeroChannelAbility('AKW1', '塞莉亚·克莱尔-解析结界（W）', {
  ...
  tooltip = '解析结界（W）', tooltipExtended = '技能说明：展开解析结界并获得护盾；成功防御时化解一次主要攻击并反冲攻击来源，结束时使范围内敌人减速。|n伤害：成功防御会反冲攻击来源。|n护盾：护盾值为攻击力的120%|n结界范围：半径320|n减速范围：半径350|n减速持续时间：2秒|n结界持续时间：4秒|n冷却时间：10秒|n魔法消耗：最大魔法值的7.5%',
...
createPlayerHeroChannelAbility('AKE1', '塞莉亚·克莱尔-锚定魔法阵（E）', {
  ...
  tooltip = '锚定魔法阵（E）', tooltipExtended = '技能说明：在目标点展开锚定魔法阵，敌人在阵中停留会逐渐被锁定。|n伤害：生效时造成攻击力110%的魔法伤害。|n施法距离：目标点|n伤害范围：半径300|n减速：35%，持续2秒|n定身条件：停留1.8秒|n定身时间：1.2秒|n魔法阵持续时间：4秒|n冷却时间：11秒|n魔法消耗：最大魔法值的7%',
  icon = ICON, hotkey = 'E', buttonX = 2, buttonY = 2, cooldown = 11, percentManaCost = 0.07,
})
...
createPlayerHeroChannelAbility('AKR1', '塞莉亚·克莱尔-高阶术式·闭锁（R）', {
  ...
  tooltip = '高阶术式·闭锁（R）', tooltipExtended = '技能说明：蓄力后锁定范围内的演算节点与连接，并根据连接类型释放不同的高阶术式。|n伤害：基础造成攻击力220%的魔法伤害；连接分支追加折射弹、穿透炮或封锁爆裂。|n锁定范围：半径450|n减速：40%，持续2.5秒|n控制：短暂硬直|n蓄力时间：1.5秒|n冷却时间：75秒|n魔法消耗：最大魔法值的15%',
...
createPlayerHeroActiveDChannelAbility('AKD1', '塞莉亚·克莱尔-术式转写（D）', {
  ...
  icon = ICON, hotkey = 'D', buttonX = 0, buttonY = 1, cooldown = 8, percentManaCost = 0.035,
})
