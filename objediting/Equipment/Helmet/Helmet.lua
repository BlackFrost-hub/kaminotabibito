-- Helmet equipment.

createEquipmentItem('I0EG', '巴尔扎罗斯的角冠', {
  baseId = 'ckng',
  icon = 'ReplaceableTextures\\CommandButtons\\Equipment\\Icon\\Helmet\\BTNbalzaroth_horned_crown.blp',
  model = 'war3mapImported\\equipment_helmet.mdl',
  abilities = ' ',
  classification = 'Permanent',
  level = 7,
  score = 9000,
  tooltipExtended = '|cffccffff[头盔]-力量/召唤|r|n|cffffcc99等级：B+|n评分：9000|r|n|cffffffcc[基础属性]|r|n力量+55|n护甲+30|n生命值+2600|n召唤物伤害+20%|n|cFF808080巴尔扎罗斯断裂的角冠沉重如铁，仍残留着命令护卫的威压。|r',
  description = '|cffccffff[头盔]-力量/召唤|r|n|cffffcc99等级：B+|n评分：9000|r|n|cffffffcc[基础属性]|r|n力量+55|n护甲+30|n生命值+2600|n召唤物伤害+20%|n|cFF808080巴尔扎罗斯断裂的角冠沉重如铁，仍残留着命令护卫的威压。|r',
})

createEquipmentItem('I0FP', '亡者凝视面甲', {
  baseId = 'rde1',
  icon = 'ReplaceableTextures\\CommandButtons\\Equipment\\Icon\\Helmet\\BTNaronkos_dead_gaze_faceplate.blp',
  model = 'war3mapImported\\ItemNerzhulMask.mdx',
  abilities = ' ',
  classification = 'Permanent',
  level = 6,
  score = 9800,
  tooltipExtended = '|cffccffff[头盔/重盔]-正面防御|r|n|cffffcc99等级：B++|n评分：9800|r|n|cffffffcc[基础属性]|r|n生命值+3500|n护甲+42|n魔法抗性+22%|n物理抗性+14%|n|cffffffcc[装备效果]|r|n直面亡者：来自正面100°内的直接伤害降低18%；减伤前伤害达到1000时，获得5秒亡者反击：下一次纯普攻或单体技能伤害提高25%，内置冷却1秒。|n|cFF808080狭窄目缝后的魂光从不转移视线，也不允许佩戴者逃避正面来敌。|r',
  description = '|cffccffff[头盔/重盔]-正面防御|r|n|cffffcc99等级：B++|n评分：9800|r|n|cffffffcc[基础属性]|r|n生命值+3500|n护甲+42|n魔法抗性+22%|n物理抗性+14%|n|cffffffcc[装备效果]|r|n直面亡者：来自正面100°内的直接伤害降低18%；减伤前伤害达到1000时，获得5秒亡者反击：下一次纯普攻或单体技能伤害提高25%，内置冷却1秒。|n|cFF808080狭窄目缝后的魂光从不转移视线，也不允许佩戴者逃避正面来敌。|r',
})

createEquipmentItem('I0FW', '无面记忆面纱', {
  baseId = 'rde1',
  icon = 'ReplaceableTextures\\CommandButtons\\Equipment\\Icon\\Helmet\\BTNancestral_twin_faceless_memory_veil.blp',
  model = 'war3mapImported\\ItemNerzhulMask.mdx',
  abilities = ' ',
  classification = 'Permanent',
  level = 6,
  score = 6850,
  tooltipExtended = '|cffccffff[头盔/面纱]-法师防具|r|n|cffffcc99等级：B|n评分：6850|r|n|cffffffcc[基础属性]|r|n智力+32|n魔法抗性+22%|n闪避+10%|n生命值+1800|n|cffffffcc[装备效果]|r|n记忆剥落：首次受到非装备技能或技能普攻造成的任一伤害类型时，记录6秒；期间再次受到同类型伤害时，本次伤害降低30%并清除记录，冷却10秒。|n|cFF808080面纱抹去了守卫的姓名，却仍记得如何适应重复降临的痛苦。|r',
  description = '|cffccffff[头盔/面纱]-法师防具|r|n|cffffcc99等级：B|n评分：6850|r|n|cffffffcc[基础属性]|r|n智力+32|n魔法抗性+22%|n闪避+10%|n生命值+1800|n|cffffffcc[装备效果]|r|n记忆剥落：首次受到非装备技能或技能普攻造成的任一伤害类型时，记录6秒；期间再次受到同类型伤害时，本次伤害降低30%并清除记录，冷却10秒。|n|cFF808080面纱抹去了守卫的姓名，却仍记得如何适应重复降临的痛苦。|r',
})

-- 默洛克渔夫重复收购奖励（B++ 档）。纯属性、无主动技能；评分依据见 TS 装备数据同条目注释。
createEquipmentItem('I0IR', '|cffFF8000鱼王骨冠|r', {
  baseId = 'ratf',
  icon = 'ReplaceableTextures\\CommandButtons\\Equipment\\Icon\\Helmet\\BTNfishking_bone_crown.blp',
  model = 'war3mapImported\\equipment_helmet.mdl',
  abilities = ' ',
  classification = 'Permanent',
  level = 6,
  score = 9000,
  tooltipExtended = '|cffccffff[头盔-全能·辅助]|r|n|cffffcc99等级：B++|n评分：9000|r|n|cffffffcc[基础属性]|r|n全属性+36|n生命值+2600|n护甲+30|n魔法抗性+18%|n|cFF808080牙还朝外。戴上它的人最好别低头看水。|r',
  description = '|cffccffff[头盔-全能·辅助]|r|n|cffffcc99等级：B++|n评分：9000|r|n|cffffffcc[基础属性]|r|n全属性+36|n生命值+2600|n护甲+30|n魔法抗性+18%|n|cFF808080牙还朝外。戴上它的人最好别低头看水。|r',
})

