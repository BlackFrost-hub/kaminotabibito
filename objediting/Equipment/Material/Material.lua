-- Fishing materials: raw catches, cooked at the campfire (h00C).
-- 区域钓鱼扩展：生鱼在此登记；烤制产物登记在 Equipment/Consumable/Consumable.lua。

createEquipmentItem('I0I0', '蛇纹翡翠鲤', {
  baseId = 'azhr',
  icon = 'ReplaceableTextures\\CommandButtons\\Equipment\\Icon\\Item\\BTNjade_scale_carp.blp',
  model = 'Doodads\\Ruins\\Water\\FishTropical\\FishTropical.mdl',
  abilities = ' ',
  classification = 'Charged',
  level = 1,
  priority = 233,
  goldCost = 70,
  tooltipExtended = '|cff808080[材料/食物]|r|n|cffc0c0c0蛇人族水池里啃食夜光翡翠矿脉碎屑的鲤鱼，鳞片泛着幽绿微光。|r|n|cffffcc99（篝火烤制 12 秒，超时 5 秒烤焦）|r',
  description = '|cff808080[材料/食物]|r|n|cffc0c0c0蛇人族水池里啃食夜光翡翠矿脉碎屑的鲤鱼，鳞片泛着幽绿微光。|r|n|cffffcc99（篝火烤制 12 秒，超时 5 秒烤焦）|r',
})

createEquipmentItem('I0I2', '月瞳鳗', {
  baseId = 'azhr',
  icon = 'ReplaceableTextures\\CommandButtons\\Equipment\\Icon\\Item\\BTNmoonpupil_eel.blp',
  model = 'Doodads\\Ruins\\Water\\FishTropical\\FishTropical.mdl',
  abilities = ' ',
  classification = 'Charged',
  level = 1,
  priority = 234,
  goldCost = 80,
  tooltipExtended = '|cff808080[材料/食物]|r|n|cffc0c0c0据传受美杜莎之瞳余韵浸染的鳗鱼，月下瞳纹会幽幽亮起。|r|n|cffffcc99（篝火烤制 15 秒，超时 5 秒烤焦）|r',
  description = '|cff808080[材料/食物]|r|n|cffc0c0c0据传受美杜莎之瞳余韵浸染的鳗鱼，月下瞳纹会幽幽亮起。|r|n|cffffcc99（篝火烤制 15 秒，超时 5 秒烤焦）|r',
})

createEquipmentItem('I0I4', '战痕月鳞鱼', {
  baseId = 'azhr',
  icon = 'ReplaceableTextures\\CommandButtons\\Equipment\\Icon\\Item\\BTNbattle_scar_moonscale.blp',
  model = 'Doodads\\Ruins\\Water\\FishTropical\\FishTropical.mdl',
  abilities = ' ',
  classification = 'Charged',
  level = 1,
  priority = 235,
  goldCost = 90,
  tooltipExtended = '|cff808080[材料/食物]|r|n|cffc0c0c0常年游弋在被魔力反复冲刷的湖岸，鳞片上布满细密战痕。|r|n|cffffcc99（篝火烤制 15 秒，超时 6 秒烤焦）|r',
  description = '|cff808080[材料/食物]|r|n|cffc0c0c0常年游弋在被魔力反复冲刷的湖岸，鳞片上布满细密战痕。|r|n|cffffcc99（篝火烤制 15 秒，超时 6 秒烤焦）|r',
})

createEquipmentItem('I0I6', '湖心星鲟', {
  baseId = 'azhr',
  icon = 'ReplaceableTextures\\CommandButtons\\Equipment\\Icon\\Item\\BTNlakeheart_sturgeon.blp',
  model = 'Doodads\\Ruins\\Water\\FishTropical\\FishTropical.mdl',
  abilities = ' ',
  classification = 'Charged',
  level = 1,
  priority = 236,
  goldCost = 100,
  tooltipExtended = '|cff808080[材料/食物]|r|n|cffc0c0c0沉在湖心深处的鲟鱼，体内沉积着星辉般的细砂。|r|n|cffffcc99（篝火烤制 18 秒，超时 6 秒烤焦）|r',
  description = '|cff808080[材料/食物]|r|n|cffc0c0c0沉在湖心深处的鲟鱼，体内沉积着星辉般的细砂。|r|n|cffffcc99（篝火烤制 18 秒，超时 6 秒烤焦）|r',
})

createEquipmentItem('I0I8', '污潮鳗', {
  baseId = 'azhr',
  icon = 'ReplaceableTextures\\CommandButtons\\Equipment\\Icon\\Item\\BTNfoul_tide_eel.blp',
  model = 'Doodads\\Ruins\\Water\\FishTropical\\FishTropical.mdl',
  abilities = ' ',
  classification = 'Charged',
  level = 1,
  priority = 237,
  goldCost = 70,
  tooltipExtended = '|cff808080[材料/食物]|r|n|cffc0c0c0在发黑的水道里长大的鳗鱼，鱼肉浸着刺鼻的腐臭魔力。|r|n|cffffcc99（篝火烤制 12 秒，超时 5 秒烤焦）|r',
  description = '|cff808080[材料/食物]|r|n|cffc0c0c0在发黑的水道里长大的鳗鱼，鱼肉浸着刺鼻的腐臭魔力。|r|n|cffffcc99（篝火烤制 12 秒，超时 5 秒烤焦）|r',
})

createEquipmentItem('I0IA', '黑水鳐', {
  baseId = 'azhr',
  icon = 'ReplaceableTextures\\CommandButtons\\Equipment\\Icon\\Item\\BTNblackwater_ray.blp',
  model = 'Doodads\\Ruins\\Water\\FishTropical\\FishTropical.mdl',
  abilities = ' ',
  classification = 'Charged',
  level = 1,
  priority = 238,
  goldCost = 80,
  tooltipExtended = '|cff808080[材料/食物]|r|n|cffc0c0c0潜伏在污染水层底部的鳐鱼，背鳍像一片化不开的黑。|r|n|cffffcc99（篝火烤制 15 秒，超时 5 秒烤焦）|r',
  description = '|cff808080[材料/食物]|r|n|cffc0c0c0潜伏在污染水层底部的鳐鱼，背鳍像一片化不开的黑。|r|n|cffffcc99（篝火烤制 15 秒，超时 5 秒烤焦）|r',
})

createEquipmentItem('I0IC', '净泉鳗', {
  baseId = 'azhr',
  icon = 'ReplaceableTextures\\CommandButtons\\Equipment\\Icon\\Item\\BTNpure_spring_eel.blp',
  model = 'Doodads\\Ruins\\Water\\FishTropical\\FishTropical.mdl',
  abilities = ' ',
  classification = 'Charged',
  level = 1,
  priority = 239,
  goldCost = 110,
  tooltipExtended = '|cff808080[材料/食物]|r|n|cffc0c0c0水脉复清后回到上游的鳗鱼，鳃边挂着清亮的泉珠。|r|n|cffffcc99（篝火烤制 18 秒，超时 6 秒烤焦）|r',
  description = '|cff808080[材料/食物]|r|n|cffc0c0c0水脉复清后回到上游的鳗鱼，鳃边挂着清亮的泉珠。|r|n|cffffcc99（篝火烤制 18 秒，超时 6 秒烤焦）|r',
})

createEquipmentItem('I0IE', '清辉水母鱼', {
  baseId = 'azhr',
  icon = 'ReplaceableTextures\\CommandButtons\\Equipment\\Icon\\Item\\BTNclear_glow_jellyfish.blp',
  model = 'Doodads\\Ruins\\Water\\FishTropical\\FishTropical.mdl',
  abilities = ' ',
  classification = 'Charged',
  level = 1,
  priority = 240,
  goldCost = 120,
  tooltipExtended = '|cff808080[材料/食物]|r|n|cffc0c0c0随净化后的水光浮起的水母鱼，通体流转着柔和的清辉。|r|n|cffffcc99（篝火烤制 20 秒，超时 6 秒烤焦）|r',
  description = '|cff808080[材料/食物]|r|n|cffc0c0c0随净化后的水光浮起的水母鱼，通体流转着柔和的清辉。|r|n|cffffcc99（篝火烤制 20 秒，超时 6 秒烤焦）|r',
})

-- 熔岩焰鱼：原条目在地图原始 item.ini，这里按项目既有做法（Consumable 的 I00Y 先例）在 obj 全字段重述，
-- 以覆盖地面模型与图标。obj 条目对已存在 ID 会整体生效，故必须把所有字段写全，不能只写模型。
-- 模型复用熔浆鱼王的邓氏鱼模型并缩到 0.35，让"鱼王的同类"在掉落物上一眼可辨。
createEquipmentItem('I0AW', '|CffFF8000熔岩焰鱼|r', {
  baseId = 'azhr',
  icon = 'ReplaceableTextures\\CommandButtons\\Equipment\\Icon\\Item\\BTNraw_molten_flamefish.blp',
  model = 'Unit\\Minion\\Dunkleosteus.mdx',
  scale = 0.35,
  abilities = ' ',
  classification = 'Charged',
  level = 1,
  priority = 50,
  goldCost = 50,
  charges = 1,
  tooltipExtended = '|Cff808080材料/食物|r|n|Cffc0c0c0专食熔岩中狂暴能量为生的鱼|n|r|Cffffcc99（篝火烤制 20 秒，超时 5 秒烤焦）|r',
  description = '|Cff808080材料/食物|r|n|Cffc0c0c0专食熔岩中狂暴能量为生的鱼|n|r|Cffffcc99（篝火烤制 20 秒，超时 5 秒烤焦）|r',
})
