...
function readTempString(idx: number): string {
...
function readTempReal(idx: number): number {
...
function readTempUnit(idx: number): any {
 ...
 */
function clearBuffBridgeGlobals(): void {
  const tu = jass.udg_TempUnit != null ? jass.udg_TempUnit : g.udg_TempUnit;
  if (tu != null) {
    tu[3] = null;
    tu[4] = null;
...
function registerOneStesEvent(trigger: any, eventName: string): void {
  const STES_Reg =
    jass.STES_Register ?? g.STES_Register ?? (globalThis as any).STES_Register;
 ...
 */
export function buffBridgeApplyFromGlobals(): void {
