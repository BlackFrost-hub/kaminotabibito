...
### 其他 EX 函数
- `EXExecuteScript` — 同步执行脚本字符串；**勿**在联机「中心计时器高频周期 + 多玩家」UI 里反复 `getObjectProperty`（见 `.cursor/rules/dzapi/ui-frame-types.mdc`「物编 EXExecuteScript 与英雄头像」；头像贴图仅创建 Frame 时设一次）。
- `EXDisplayChat`
- `EXDclareButtonIcon`
- `EXBlendButtonIcon`
