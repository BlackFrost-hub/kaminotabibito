...
| `Common\Effect\Form\Aura\ShalltearVampiricHeal.mdx` | 三束暗红血能从地面向单位中心快速收束，中央带白红闪光，吸血回流语义清楚。 | 保留源非循环 `Birth`（约 `1.833` 秒），迁移版补齐 1 帧安全 `Stand / Death`；运行时按 `1.9` 秒销毁。 | 夏提雅汲血强化穿刺、鲜血回收的自身恢复表现。 | 绑定夏提雅 `origin`；只用于真实吸血恢复，不用于血之复生。源 `吸血鬼目标\sem_xi_xue_gui_mu_biao.mdx`。 | 3903 | `ADD7316DBEB70579` |
| `Common\Effect\Form\Aura\sem_lan_huo.mdx` | 蓝色火焰、震荡环与光粒子组成的短促守护爆发。 | 源模型仅含非循环 `Birth`；运行时按 `0.35` 秒短播，不作为常驻光环。 | 菲利斯第二军团护卫靠近守护时，在护卫与Boss坐标各播放一次。 | 私有贴图迁入本分类 `Texture`；其余 5 张贴图保留 Warcraft 原生路径。 | 7334 | `B0ED7BE2A0BE04CC` |
