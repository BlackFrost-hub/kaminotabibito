...
const 世界地图区域配置By矩形键: Record<string, 世界地图区域运行配置 | undefined> = {};
...
function 取或创建区域配置(this: void, 矩形键: string): 世界地图区域运行配置 {
  let 配置 = 世界地图区域配置By矩形键[矩形键];
  if (配置 == null) {
    配置 = { 矩形键 };
    世界地图区域配置By矩形键[矩形键] = 配置;
    世界地图区域运行配置表.push(配置);
  }
  return 配置;
}
