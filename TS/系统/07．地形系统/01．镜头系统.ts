 ...
 */
export function panCameraToTimedForPlayer(
  whichPlayer: any,
  x: number,
  y: number,
  duration: number
): void {
  if (typeof (jass as any).GetLocalPlayer !== "function") return;
  const localPlayer = (jass as any).GetLocalPlayer();
  if (localPlayer !== whichPlayer) return;
  if (typeof (jass as any).PanCameraToTimed !== "function") return;
  (jass as any).PanCameraToTimed(x, y, duration);
}
