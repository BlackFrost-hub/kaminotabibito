 ...
 */
export const STAT_LIMITS: Record<string, { max: number; min: number }> = {
  "魔抗": { max: 0.50, min: -1 },
  "物理抗性": { max: 0.30, min: -1 },
  "技能抗性": { max: 0.80, min: -1 },
  "普攻抗性": { max: 0.80, min: -1 },
 ...
 * 默认值：max = 1（无上限），min = -1（无下限）
 */
export const ENEMY_STAT_LIMITS: Record<string, { max: number; min: number }> = {
  "魔抗": { max: 1, min: -1 },
  "物理抗性": { max: 1, min: -1 },
  "技能抗性": { max: 1, min: -1 },
  "普攻抗性": { max: 1, min: -1 },
 ...
 * value: 突破上限所需的特殊属性名
 */
export const BREAKABLE_LIMITS: Record<string, string> = {
  "伤害吸血": "伤害吸血上限",
  "伤害吸魔": "伤害吸魔突破",
};
 ...
 * 3. 移动速度 → 取最高值（防止无限加速）
 */
export const STAT_STACK_CONFIG: Record<string, string> = {
  //=========== 加法叠加（伤害加成类）===========
  ...
  "主动技能伤害": STACK_TYPE.ADDITIVE,
  "独立技能伤害": STACK_TYPE.ADDITIVE,
  "装备伤害": STACK_TYPE.ADDITIVE,
  "攻击特效伤害": STACK_TYPE.ADDITIVE,
  "普攻强化伤害": STACK_TYPE.ADDITIVE,
 ...
 * 用于修正JASS代码中的属性名错误
 */
export const JASS_ATTR_NAME_FIX: Record<string, string> = {
 ...
 */
export const DAMAGE_TYPE_ATTRS: Record<string, { damage: string; resist: string }> = {
  // 物理伤害
 ...
 */
export function applyStatLimit(attrName: string, value: number, isPlayer: boolean): number {
 ...
 * @returns 正确的属性名
 */
export function fixJassAttrName(attrName: string): string {
 ...
 */
export function getStackType(attrName: string): string {
