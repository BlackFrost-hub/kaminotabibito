...
function 读取YD用户数据(this: void, tableType: string, tableKey: any, attr: string, valueType: string): any {
  ...
  debugLogForce(模块名, "读取YD用户数据：tableType=", tableType, "tableKey=", tostring(tableKey), "attr=", attr, "valueType=", valueType, "value=", value);
  return value;
}
...
function 获取装备重伤值(unit: any): number {
  ...
  debugLogForce(模块名, "获取装备重伤值：unit=", tostring(unit), "playerId=", playerId);
  if (playerId >= 0 && playerId <= 3) {
    const v = 读取YD用户数据("player", owner, "重伤", "real");
    debugLogForce(模块名, "玩家重伤：", v);
    return typeof v === "number" ? 限制重伤值(v) : 0;
  }
  const v = 读取YD用户数据("unit", unit, "重伤", "real");
  debugLogForce(模块名, "单位重伤：", v);
  return typeof v === "number" ? 限制重伤值(v) : 0;
}
...
export function 施加重伤(unit: any, 重伤值: number, 持续时间: number = 重伤默认持续时间, source?: any): void {
  ...
  const 最终值 = 限制重伤值(重伤值);
  if (最终值 <= 0) return;
  debugLogForce(模块名, "施加重伤：target=", tostring(unit), "value=", 最终值, "duration=", 持续时间);
...
function on伤害事件(...args: any[]): void {
  ...
  debugLogForce(模块名, "伤害事件触发，args数量=", args.length);
  for (let i = 0; i < args.length; i++) {
    debugLogForce(模块名, "  args[", i, "]=", tostring(args[i]));
  }
  ...
  debugLogForce(模块名, "解析结果：target=", tostring(target), "source=", tostring(source));

  if (source == null || source === 0) {
    debugLogForce(模块名, "source无效，跳过");
    return;
  }
  if (target == null || target === 0) {
    debugLogForce(模块名, "target无效，跳过");
    return;
  }

  const 装备重伤 = 获取装备重伤值(source);
  debugLogForce(模块名, "装备重伤值=", 装备重伤);
  if (装备重伤 <= 0) {
    debugLogForce(模块名, "装备重伤<=0，跳过");
    return;
  }

  debugLogForce(模块名, "准备施加重伤给target");
  施加重伤(target, 装备重伤, 重伤默认持续时间, source);
  debugLogForce(模块名, "施加后目标当前重伤=", 获取单位重伤(target));
}

/** 注册重伤回调到治疗系统和伤害事件 */
export function initWoundSystem(): void {
  if (!重伤系统开关) return;
  debugLogForce(模块名, "initWoundSystem开始");
  registerHealCallback(计算重伤治疗量);
  debugLogForce(模块名, "治疗回调已注册");
  ...
  debugLogForce(模块名, "准备注册伤害回调：damageEventModule=", tostring(damageEventModule), "regCb=", tostring(regCb));
  regCb(damageEventModule, on伤害事件, 0);
  debugLogForce(模块名, "伤害回调已注册：on伤害事件=", tostring(on伤害事件));
  debugLogForce(模块名, "重伤系统初始化完成");
}
