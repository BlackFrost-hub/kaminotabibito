...
export type 英雄选择配置 = {
  可选玩家ID列表: ReadonlyArray<number>;
  双击确认窗口秒数: number;
  单击介绍显示秒数: number;
  选择系统关闭秒数: number;
  选择区域名称: string;
  英雄出生坐标: { X: number; Y: number };
  英雄已选择标记键: string;
  英雄点击次数键: string;
  玩家英雄单位组表名: string;
  玩家英雄单位组键: string;
  ...
  记录已选英雄键: string;
  记录玩家BB键: string;
  玩家英雄名写入字符串数组偏移: number;
  英雄确认公告前缀: string;
  选择系统关闭提示文本: string;
  选择确认后直接执行触发名?: string;
  英雄禁用技能名?: string;
  BB单位名?: string;
  英雄单击介绍表: Readonly<Record<string, string>>;
  必须保留的旧单位事件注册项: ReadonlyArray<英雄选择旧单位事件注册项>;
};

export type 英雄选择旧单位事件注册项 = {
  目标单位: "英雄" | "BB";
  旧触发名: string;
  事件名: string;
};

export const 英雄选择配置表: 英雄选择配置 = {
  可选玩家ID列表: [0, 1, 2, 3, 4, 5],
  双击确认窗口秒数: 1,
  单击介绍显示秒数: 20,
  选择系统关闭秒数: 0,
  选择区域名称: "英雄选择区域",
  英雄出生坐标: { X: -2501.3, Y: 28768.3 },
  英雄已选择标记键: "是否已选择英雄",
  英雄点击次数键: "点击次数",
  玩家英雄单位组表名: "玩家英雄",
  玩家英雄单位组键: "单位组",
