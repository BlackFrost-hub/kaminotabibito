...
const 本地存档 = require("系统.10．存档系统.01．本地存档.index") as {
  加载玩家本地存档: (this: void, player: any) => boolean;
  读取本地存档布尔: (this: void, player: any, field: number, fallback?: boolean) => boolean;
  设置本地存档布尔: (this: void, player: any, field: number, enabled: boolean, autoSave?: boolean) => boolean;
...
const 仇恨文字开关表: Record<number, boolean | undefined> = {};

let 已初始化 = false;
let 已加载本机显示配置 = false;

function 应用玩家显示配置(this: void, player: any): void {
...
function 加载本机显示配置(this: void): void {
  if (已加载本机显示配置) return;

  const localPlayer = 获取本地玩家();
  if (localPlayer == null || localPlayer === 0) return;
  已加载本机显示配置 = true;
  应用玩家显示配置(localPlayer);
}

function 强制加载命令玩家显示配置(this: void, whichPlayer: any): void {
...
function 保存显示开关配置(this: void, whichPlayer: any, field: number, enabled: boolean): void {
...
function 读取仇恨文字开关(this: void, playerId: number): boolean {
...
function 切换仇恨文字命令动作(this: void, whichPlayer: any): void {
  强制加载命令玩家显示配置(whichPlayer);
  const playerId = 获取玩家编号(whichPlayer);
...
export function 本地玩家是否开启仇恨文字(this: void): boolean {
  加载本机显示配置();
  const localPlayer = 获取本地玩家();
  if (localPlayer == null || localPlayer === 0) return true;
