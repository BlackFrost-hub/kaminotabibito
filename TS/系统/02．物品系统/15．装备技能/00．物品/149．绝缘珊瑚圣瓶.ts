...
import { 单位持有第二章后段Boss战利品, 是敌对单位, 取冷却键, 冷却就绪, 进入冷却, 开始通用护盾, 临时受到治疗率, 第二章后段Boss战利品装备名 } from "../../../03．技能系统/00．技能模板+函数/01．技能函数/20．物品辅助/07．装备辅助";
...
function on绝缘珊瑚圣瓶治疗(this: void, source: any, target: any, amount: number, isItemHeal: boolean): void {
  ...
  if (!冷却就绪(key)) return;
  进入冷却(key, 10);
  开始通用护盾(source, target, 520, 5, "绝缘珊瑚圣瓶");
