...
import { 单位持有伤害事件装备, 执行物品治疗, 是指定伤害类型, 伤害事件伤害类型, 造成伤害事件伤害, type 伤害事件上下文 } from "../04．伤害事件/00．公共/01．伤害事件工具";
const { 是否普通敌人 } = require("系统.03．技能系统.00．技能模板+函数.02．通用函数.index") as { 是否普通敌人: (this: void, unit: any) => boolean };

const jass = require("jass.common") as any;
const GetUnitLevel = jass.GetUnitLevel as (u: any) => number;
 ...
 */
export function 处理沙漠蜥蜴之魂造成伤害(this: void, ctx: 伤害事件上下文): void {
  if (!单位持有伤害事件装备(ctx.attacker, 伤害事件装备ID.沙漠蜥蜴之魂)) return;

  const 等级 = GetUnitLevel(ctx.target);
  if (等级 >= 25) return;
  if (!是否普通敌人(ctx.target)) return;
