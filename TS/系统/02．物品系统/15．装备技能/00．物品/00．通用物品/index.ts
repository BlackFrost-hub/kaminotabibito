...
const { 处理通用物品合成打造, 处理一次性打造壳合成 } = require("./03．合成打造") as {
  处理通用物品合成打造: (this: void, 单位: any, 物品: any) => void;
  处理一次性打造壳合成: (this: void, 单位: any, 物品类型ID: number, 物品?: any) => void;
};
const { 是一次性打造壳 } = require("./00．通用物品工具") as {
...
function on通用物品拾取(this: void, 单位: any, 物品: any): void {
  ...
  if (是一次性打造壳(物品)) {
    处理一次性打造壳合成(单位, GetItemTypeId(物品), 物品);
    return;
  }
  处理通用物品吃书清理(单位, 物品);
  处理通用物品获取特效(单位, 物品);
...
function 执行延迟打造壳合成(this: void, 上下文?: 延迟打造壳上下文): void {
  if (上下文 == null) return;
  处理一次性打造壳合成(上下文.单位, 上下文.物品类型ID, 上下文.物品);
}

function on一次性打造壳丢弃(this: void, 单位: any, 物品: any): void {
  if (!是玩家英雄或BB(单位) || !是一次性打造壳(物品)) return;
  const 上下文: 延迟打造壳上下文 = { 单位, 物品类型ID: GetItemTypeId(物品), 物品 };
