...
import { 攻击者类型满足, 距离满足限制 } from "../08．攻击效果/00．公共/01．攻击效果工具";
import { 读取玩家暴击伤害 } from "../../../03．技能系统/00．技能模板+函数/01．技能函数/20．物品辅助";

const 魔古战刃被动最大攻击距离 = 200;
...
export function 处理魔古战刃伤害(this: void, target: any, attacker: any, applied: number, snapshot: any): void {
  ...
  if (snapshot.isSkillDamage === true || snapshot.isSkillAttack === true || snapshot.isWrappedSkillDamage === true) return;
  if (!攻击者类型满足(attacker, "近战")) return;
  if (!距离满足限制(attacker, target, undefined, 魔古战刃被动最大攻击距离)) return;
