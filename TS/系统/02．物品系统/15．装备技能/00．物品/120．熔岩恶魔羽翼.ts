...
const 熔岩恶魔羽翼物品ID = stringToFourCCSafe(resolveItemIdByName("熔岩恶魔羽翼"));
const 距离阈值 = 300;
...
export function 处理熔岩恶魔羽翼伤害修正(this: void, context: any, 当前伤害: number): number {
  if (熔岩恶魔羽翼物品ID === 0) return 当前伤害;
  if (context == null || context.target == null || context.target === 0 || context.attacker == null || context.attacker === 0) return 当前伤害;
  if (!单位持有伤害事件装备(context.target, 熔岩恶魔羽翼物品ID)) return 当前伤害;
