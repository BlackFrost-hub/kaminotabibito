...
import { 添加单目标周期效果, type 单目标周期效果事件 } from "../../../03．技能系统/00．技能模板+函数/04．机制组件/09．装备通用机制/17．单目标周期效果";

function 银魔手套周期(this: void, event: 单目标周期效果事件): void {
  造成伤害事件伤害(event.来源, event.目标, event.数值, 伤害事件伤害类型.暗影突袭);
  执行物品治疗(event.来源, event.来源, event.数值, undefined);
}
