...
const 精致木盾物品ID = stringToFourCCSafe(resolveItemIdByName("精致木盾"));
const 距离阈值 = 400;
...
export function 处理精致木盾伤害修正(this: void, context: any, 当前伤害: number): number {
  if (精致木盾物品ID === 0) return 当前伤害;
  if (context == null || context.target == null || context.target === 0 || context.attacker == null || context.attacker === 0) return 当前伤害;
  if (!单位持有伤害事件装备(context.target, 精致木盾物品ID)) return 当前伤害;
