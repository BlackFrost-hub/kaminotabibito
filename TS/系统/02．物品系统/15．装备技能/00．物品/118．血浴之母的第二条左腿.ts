...
const 血浴之母的第二条左腿物品ID = stringToFourCCSafe(resolveItemIdByName("血浴之母的第二条左腿"));
const 减伤系数 = 0.01;

export function 处理血浴之母的第二条左腿伤害修正(this: void, context: any, 当前伤害: number): number {
  if (血浴之母的第二条左腿物品ID === 0) return 当前伤害;
  if (context == null || context.target == null || context.target === 0) return 当前伤害;
  if (context.isPhysicalDamage !== true) return 当前伤害;
  if (!单位持有伤害事件装备(context.target, 血浴之母的第二条左腿物品ID)) return 当前伤害;
