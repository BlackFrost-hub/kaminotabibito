...
const { registerManualBuff } = require('系统.05．Buff系统.00．Buff系统') as any;
创建同目标普攻计数触发器({
    名称: '滴管长枪投影-滴管汲血',
    窗口秒: 5,
    次数阈值: 3,
    ...
    冷却作用域: '攻击者目标',
    仅纯普攻: true,
    过滤: (e) => 单位持有装备(e.source, 四Boss战利品装备名.滴管长枪投影),
    on触发: (e) => {
        const damage = 取攻击力(e.source) * 0.8 + 220,
            healByDamage = damage * 0.25,
            healCap = 取最大生命(e.source) * 0.06;
        ...
        registerManualBuff(e.target, 'BSH1', 8, 0, { sourceName: '滴管长枪投影' });
