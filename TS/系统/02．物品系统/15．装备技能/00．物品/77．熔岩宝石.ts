...
const { 注册范围脉冲效果 } = require("系统.03．技能系统.00．技能模板+函数.01．技能函数.20．物品辅助.04．范围脉冲效果") as {
  注册范围脉冲效果: (this: void, params: {
    物品类型ID: number;
    间隔毫秒: number;
    半径: number;
...
const { 造成火焰伤害, 取最大生命, 播放点特效, 取单位X, 取单位Y } = require("../05．物品使用/00．公共/02．物品使用工具") as {
