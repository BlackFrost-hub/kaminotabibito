...
function 取伤害概率(this: void, 配置: 攻击效果配置项, ctx: 攻击效果上下文): number {
  if (配置.伤害概率计算 != null) return 配置.伤害概率计算(ctx);
  if (配置.伤害概率 != null) return 配置.伤害概率;
  return 1;
}

function 取治疗概率(this: void, 配置: 攻击效果配置项, ctx: 攻击效果上下文): number {
...
function 计算伤害(this: void, 配置: 攻击效果配置项, ctx: 攻击效果上下文): number {
  ...
  if (lifeFactor != null) amount += 取最大生命(ctx.target) * lifeFactor;
...
function 执行范围伤害(this: void, 配置: 攻击效果配置项, ctx: 攻击效果上下文): void {
  ...
  if (!(radius > 0)) return;
  const list = 获取敌方范围单位(ctx.source, ctx.target, radius, 配置.范围包含主目标 === true);
  const amount = 计算伤害(配置, ctx);
  let spreadCount = 0;
  ...
  if (spreadCount > 0 && (配置.扩散成功主目标伤害倍率 ?? 0) > 0) {
    攻击效果造成伤害(ctx.source, ctx.target, ctx.applied * (配置.扩散成功主目标伤害倍率 ?? 0), 配置.伤害类型);
  }
}
