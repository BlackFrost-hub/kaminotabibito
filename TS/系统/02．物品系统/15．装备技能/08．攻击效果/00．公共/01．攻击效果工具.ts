...
import type { 武器类型 } from "../../../../03．技能系统/00．技能模板+函数/02．通用函数/01．便捷短函数集合/07．武器类型";
...
const { 获取单位最终武器类型 } = require("系统.03．技能系统.00．技能模板+函数.02．通用函数.01．便捷短函数集合.07．武器类型") as {
  获取单位最终武器类型: (this: void, unit: any) => 武器类型;
};
...
export function 单位武器类型满足(this: void, unit: any, type?: 武器类型): boolean {
  if (type == null || type === "") return true;
  if (!单位是英雄(unit)) return false;
  return 获取单位最终武器类型(unit) === type;
}
