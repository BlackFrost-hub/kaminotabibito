...
/** 设置多面板项目值 */
function multiboardSetItemValue(mb: any, col: number, row: number, val: string): void {
  if (mb == null) return;
...
/** 设置多面板项目图标 */
function multiboardSetItemIcon(mb: any, col: number, row: number, icon: string): void {
  if (mb == null) return;
  const item = jass.MultiboardGetItem(mb, row - 1, col - 1);
  if (item != null) {
...
/** 设置多面板项目样式 */
function multiboardSetItemStyle(mb: any, col: number, row: number, showValue: boolean, showIcon: boolean): void {
  if (mb == null) return;
  const item = jass.MultiboardGetItem(mb, row - 1, col - 1);
  if (item != null) {
...
/** 检查系统是否启用 */
export function isMultiboardSystemEnabled(): boolean {
