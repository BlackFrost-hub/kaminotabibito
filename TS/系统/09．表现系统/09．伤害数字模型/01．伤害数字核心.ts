...
function 选取伤害颜色(this: void, damageType: 伤害类型快照): { r: number; g: number; b: number } {
  ...
  if (damageType.isTrueDamage) return { r: 255, g: 240, b: 120 };//真实/精神伤害
  ...
  if (damageType.isPhysicalDamage) return { r: 255, g: 90, b: 90 };//物理伤害
  if (damageType.isMagicDamage) return { r: 120, g: 140, b: 255 };//魔法伤害
  //兜底颜色代码，先普攻色/精神/强化色，避免被物理/魔法伤害抢色。
  if (damageType.rawDamageType === jass.DAMAGE_TYPE_MIND) return { r: 255, g: 240, b: 120 };//精神伤害兜底
  ...
  if (damageType.rawDamageType === jass.DAMAGE_TYPE_NORMAL) return { r: 255, g: 90, b: 90 };//物理伤害兜底
