...
function 执行充能测试(): void {
  ...
  debugLogForce(模块名, "开始充能，id=", 充能ID);
}
...
function 注册聊天测试(): void {
  ...
  const trig113 = CreateTrigger();
  TriggerRegisterPlayerChatEvent(trig113, Player(0), 充能测试命令, true);
  TriggerAddAction(trig113, on聊天113测试);
  debugLogForce(模块名, "已注册测试：113=开始3秒充能");
}
