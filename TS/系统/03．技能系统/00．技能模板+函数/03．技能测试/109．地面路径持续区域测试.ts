...
function 地面路径持续区域测试_单段创建(段序号: number, X: number, Y: number): void {
  debugLogForce(模块名, "已铺设火焰段：序号=", 段序号, " 坐标=(", X, ",", Y, ")");
}

function 地面路径持续区域测试_全部铺设完成(实例ID: number): void {
  debugLogForce(模块名, "火焰路径铺设完成：实例ID=", 实例ID);
}

function 地面路径持续区域测试_销毁(实例ID: number): void {
  debugLogForce(模块名, "火焰路径已销毁：实例ID=", 实例ID);
}

function on聊天1009测试(): void {
...
function 注册聊天测试(): void {
  ...
  TriggerAddAction(trig, on聊天1009测试);
