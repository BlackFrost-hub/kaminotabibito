...
const DzSetEffectVisible = japi.DzSetEffectVisible as ((effect: any, enable: boolean) => void) | undefined;
...
function 记录待销毁特效(this: void, effect: any): void {
  待销毁队列.push({
    effect,
...
function 清理到期特效(this: void): void {
  let writeIndex = 0;
...
function 创建一次模型序列特效(this: void): void {
  const unit = 获取测试单位();
  if (unit == null || unit === 0) {
...
function on每秒驱动(this: void): void {
  当前秒计数++;
  清理到期特效();
  if (!已开启) return;
  创建一次模型序列特效();
...
function on聊天1036开启测试(this: void): void {
  开启测试();
}

function on聊天1037关闭测试(this: void): void {
  关闭测试();
}
