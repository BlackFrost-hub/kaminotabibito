...
const AddSpecialEffectTarget = jass.AddSpecialEffectTarget as (model: string, widget: any, attachPointName: string) => any;
...
let 下一个无敌ID = 1;
const 活跃无敌实例: 无敌实例[] = [];
const 单位无敌层数: Record<number, number> = {};
const 单位无敌特效: Record<number, any> = {};
let 无敌Tick已启动 = false;

function 获取单位当前无敌层数(单位句柄ID: number): number {
  return 单位无敌层数[单位句柄ID] ?? 0;
}

function 设置单位当前无敌层数(单位句柄ID: number, 层数: number): void {
  if (层数 <= 0) {
...
function 单位仍可操作(单位: any): boolean {
  return 单位 != null && GetUnitTypeId(单位) !== 0;
}

function 创建单位无敌特效(单位: any, 单位句柄ID: number): void {
  if (!单位仍可操作(单位)) return;
  if (单位无敌特效[单位句柄ID] != null) return;
  单位无敌特效[单位句柄ID] = AddSpecialEffectTarget(DEFAULT_INVULNERABLE_EFFECT, 单位, "origin");
}

function 销毁单位无敌特效(单位句柄ID: number): void {
...
function 确保无敌Tick启动(): void {
  if (无敌Tick已启动) return;
  无敌Tick已启动 = true;
  onTick10ms(无敌Tick);
}

function 尝试关闭无敌Tick(): void {
  if (活跃无敌实例.length > 0 || !无敌Tick已启动) return;
  无敌Tick已启动 = false;
  offTick10ms(无敌Tick);
}

function 移除无敌实例(index: number): void {
...
export function 开始无敌帧(单位: any, 持续时间: number): number {
  if (单位 == null || 持续时间 <= 0) return 0;
...
export function 取消无敌帧(无敌ID: number): boolean {
  for (let i = 活跃无敌实例.length - 1; i >= 0; i--) {
    const 实例 = 活跃无敌实例[i];
    if (实例.id !== 无敌ID) continue;
    实例.激活 = false;
    移除无敌实例(i);
    尝试关闭无敌Tick();
    return true;
...
export function 取消单位所有无敌帧(单位: any): boolean {
  if (单位 == null) return false;
...
export function 单位是否处于无敌帧中(单位: any): boolean {
  if (单位 == null) return false;
  return 获取单位当前无敌层数(GetHandleId(单位)) > 0;
}

export function 获取单位无敌帧层数(单位: any): number {
  if (单位 == null) return 0;
