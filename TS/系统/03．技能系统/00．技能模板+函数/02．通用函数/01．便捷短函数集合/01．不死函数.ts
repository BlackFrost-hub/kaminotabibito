...
const 不死集合: Record<number, true | undefined> = {};

function on不死伤害修正(context: any): number {
  const target = context.target;
  if (target == null || target === 0) return context.currentDamage;

  const 单位ID = GetHandleId(target);
  if (单位ID === 0) return context.currentDamage;
  if (不死集合[单位ID] == null) return context.currentDamage;
 ...
 */
export function 令单位不死(单位: any): void {
 ...
 * 移除单位的不死状态。
 */
export function 移除单位不死(单位: any): void {
  if (单位 == null || 单位 === 0) return;
  const 单位ID = GetHandleId(单位);
  if (单位ID === 0) return;
  delete 不死集合[单位ID];
 ...
 * 查询单位是否处于不死状态。
 */
export function 单位是否不死(单位: any): boolean {
