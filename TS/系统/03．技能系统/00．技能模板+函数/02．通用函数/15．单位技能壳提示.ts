...
const DzSetUnitAbilityUberTip = japi.DzSetUnitAbilityUberTip as ((unit: any, abilityId: number, tip: string) => boolean) | undefined;
...
export function 设置单位技能壳普通提示(this: void, 单位: any, 配置列表: readonly 单位技能壳提示配置[]): void {
  ...
  if (typeof DzSetUnitAbilityTip !== "function" && typeof DzSetUnitAbilityUberTip !== "function") return;
  for (let i = 0; i < 配置列表.length; i++) {
    const 配置 = 配置列表[i];
    if (配置.技能ID == null || 配置.技能ID.length < 4 || 配置.提示 === "") continue;
    const 技能ID = stringToFourCC(配置.技能ID);
    if (typeof DzSetUnitAbilityTip === "function") {
      DzSetUnitAbilityTip(单位, 技能ID, 配置.提示);
    }
    if (typeof DzSetUnitAbilityUberTip === "function" && 配置.扩展提示 != null) {
      DzSetUnitAbilityUberTip(单位, 技能ID, 配置.扩展提示);
    }
