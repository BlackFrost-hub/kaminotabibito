...
function 标准化角度(角度: number): number {
  let 结果 = 角度;
  while (结果 < 0) {
    结果 += 360;
  }
...
function 取最小夹角(角度A: number, 角度B: number): number {
...
function 取坐标朝向角(源X: number, 源Y: number, 目标X: number, 目标Y: number): number {
  return (jass.Atan2(目标Y - 源Y, 目标X - 源X) as number) * bj_RADTODEG;
}

export function 单位是否在扇形区域(
  单位: any,
  X: number,
  Y: number,
  半径: number,
...
export function 获取扇形区域单位(参数: 扇形区域参数): any[] {
...
export function 创建扇形单位组(参数: 扇形区域参数): any {
