...
export type 原地击飞结束原因 = "完成" | "中断" | "死亡" | "主单位死亡";
export type 原地击飞开始回调 = (this: void, 单位: any, 击飞ID: number) => void;
export type 原地击飞结束回调 = (this: void, 单位: any, 原因: 原地击飞结束原因, 击飞ID: number) => void;

export interface 原地击飞参数 {
  持续时间: number;
  /** 每次抖动的最低附加高度，默认 200。 */
  最小高度?: number;
  /** 每次抖动的最高附加高度，默认 250。 */
  最大高度?: number;
  /** 起手脚下特效，默认战争践踏冲击波。传空字符串可关闭。 */
  冲击波模型?: string;
  /** 持续顶飞期间脚下循环创建的特效。传空或不传则不循环创建。 */
  持续特效模型?: string;
  /** 持续特效创建间隔，默认 0.08 秒。 */
  持续特效间隔?: number;
  暂停单位?: boolean;
...
const 活动原地击飞列表: 原地击飞实例[] = [];
const 原地击飞映射: Record<number, 原地击飞实例 | undefined> = {};
const 单位当前原地击飞: Record<number, number | undefined> = {};

let 下一个原地击飞ID = 0;
let 已注册到中心计时器 = false;
let tick计数 = 0;

function 分配原地击飞ID(this: void): number {
  下一个原地击飞ID += 1;
  return 下一个原地击飞ID;
}
...
function 注册到中心计时器(this: void): void {
  if (已注册到中心计时器) return;
  已注册到中心计时器 = true;
  onTick10ms(on原地击飞系统Tick);
}

function 从中心计时器注销(this: void): void {
  if (!已注册到中心计时器) return;
  已注册到中心计时器 = false;
  offTick10ms(on原地击飞系统Tick);
}

function 尝试收尾中心计时器(this: void): void {
  if (活动原地击飞列表.length !== 0) return;
  tick计数 = 0;
  从中心计时器注销();
}

function 解析高度区间(this: void, 参数: 原地击飞参数): { 最小高度: number; 最大高度: number } {
...
function 播放冲击波(this: void, 单位: any, 模型: string | undefined): void {
...
function 创建脚下特效(this: void, 单位: any, 模型: string): void {
  if (模型 === "") return;
...
function 内部移除原地击飞(this: void, 实例: 原地击飞实例): void {
  const 击飞ID = 实例.id;
  const 单位ID = 实例.单位ID;
...
function 结束原地击飞实例(this: void, 实例: 原地击飞实例, 原因: 原地击飞结束原因): void {
  if (原地击飞映射[实例.id] !== 实例) return;
...
function 更新原地击飞高度(this: void, 实例: 原地击飞实例): void {
  const 单位 = 实例.单位;
  const 当前高度 = GetUnitFlyHeight(单位);
...
function 更新持续特效(this: void, 实例: 原地击飞实例): void {
  if (实例.持续特效模型 === "") return;
  实例.持续特效计时 += TICK_INTERVAL;
  if (实例.持续特效计时 < 实例.持续特效间隔) return;
  实例.持续特效计时 = 0;
  创建脚下特效(实例.单位, 实例.持续特效模型);
}

function on原地击飞系统Tick(this: void): void {
  tick计数 += 1;
...
export function 开始原地击飞(this: void, 单位: any, 参数: 原地击飞参数): number {
  if (!单位存活(单位)) return 0;
...
export function 停止原地击飞(this: void, 击飞ID: number, 原因: 原地击飞结束原因 = "中断"): boolean {
...
export function 停止单位原地击飞(this: void, 单位: any, 原因: 原地击飞结束原因 = "中断"): boolean {
  const 击飞ID = 单位当前原地击飞[取单位ID(单位)] ?? 0;
  if (击飞ID <= 0) return false;
  return 停止原地击飞(击飞ID, 原因);
}

export function 单位是否正在原地击飞(this: void, 单位: any): boolean {
  const 击飞ID = 单位当前原地击飞[取单位ID(单位)] ?? 0;
  return 击飞ID > 0 && 原地击飞映射[击飞ID] != null;
}

export function 获取单位当前原地击飞ID(this: void, 单位: any): number {
