...
export interface 范围脉冲效果参数 {
  物品类型ID: number;
  间隔毫秒: number;
  半径: number;
  脉冲回调: (this: void, unit: any, target: any, currentCount: number) => void;
  目标过滤?: (this: void, unit: any, target: any, currentCount: number) => boolean;
  排除自身?: boolean;
}
...
type 范围脉冲实例 = 范围脉冲效果参数 & {
  下次触发时间: number;
  单位状态: Record<number, 范围脉冲状态 | undefined>;
};
...
function 处理丢弃(this: void, 配置: 范围脉冲实例, unit: any, _item: any, currentCount: number): void {
  const unitId = 获取单位ID(unit);
  if (unitId === 0) return;
  if (currentCount <= 0) {
    delete 配置.单位状态[unitId];
  ...
  for (let i = 0; i < 范围脉冲实例表.length; i++) {
    ...
    for (const unitKey in 配置.单位状态) {
      ...
      for (let j = 0; j < units.length; j++) {
        ...
        if (配置.排除自身 !== false && target === 状态.单位) continue;
        if (IsUnitType(target, UNIT_TYPE_DEAD) === true) continue;
        if (配置.目标过滤 != null && 配置.目标过滤(状态.单位, target, currentCount) !== true) continue;
