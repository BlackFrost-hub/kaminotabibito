...
function 解析改向速度(this: void, 实例: 原生弹幕内部实例, 新速度?: number): number {
  if (新速度 != null && 新速度 > 0) return 新速度;
  if (实例.当前速度 > 0) return 实例.当前速度;
  if (实例.参数.速度 > 0) return 实例.参数.速度;
  return 400;
}

function 应用原生弹幕改向(this: void, 实例: 原生弹幕内部实例, 参数: 原生弹幕改向参数): boolean {
  if (实例.已结束) return false;
...
export function 按单位设置原生弹幕指定角度飞行(
  this: void,
  弹幕单位: any,
  朝向角度: number,
  新速度?: number,
  ...
  if (弹幕ID <= 0) return false;
  return 设置原生弹幕指定角度飞行(弹幕ID, 朝向角度, 新速度);
}

export function 按反弹单位面向反弹原生弹幕(
  this: void,
  弹幕ID: number,
  反弹单位: any,
  新速度?: number,
  ...
  if (反弹单位 == null || 反弹单位 === 0) return false;
  return 设置原生弹幕指定角度飞行(弹幕ID, GetUnitFacing(反弹单位) + 附加角度, 新速度);
}

export function 按单位反弹原生弹幕(
  this: void,
  弹幕单位: any,
  反弹单位: any,
  新速度?: number,
