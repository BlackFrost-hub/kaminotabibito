...
export function 开始冲锋并附带残影表现(单位: any, 位移参数: any, 表现参数: 冲锋残影表现参数): number {
  const 带动作冲锋参数 = { ...位移参数 };
  if (表现参数.动画序号 != null) 带动作冲锋参数.动画序号 = 表现参数.动画序号;
  if (表现参数.动画名 != null && 表现参数.动画名 !== "") 带动作冲锋参数.动画名 = 表现参数.动画名;
  const 冲锋ID = 开始冲锋(单位, 带动作冲锋参数);
  if (冲锋ID <= 0) return 0;

  const 实际表现参数: 冲锋残影表现参数 = { ...表现参数 };
  if (实际表现参数.动画序号 == null && 带动作冲锋参数.动画序号 != null) 实际表现参数.动画序号 = 带动作冲锋参数.动画序号;
  if ((实际表现参数.动画名 == null || 实际表现参数.动画名 === "") && 带动作冲锋参数.动画名 != null) 实际表现参数.动画名 = 带动作冲锋参数.动画名;
  应用单位动画表现(单位, 实际表现参数);

  const 残影模型 = 解析残影模型(单位, 实际表现参数);
  if (残影模型 === "") {
    return 冲锋ID;
  }

  const 飞行高度变化 = 实际表现参数.飞行高度变化 ?? 0;
  if (飞行高度变化 !== 0) {
    确保单位可设置飞行高度(单位);
  ...
  const 实例: 冲锋残影表现实例 = {
    ...
    动画序号: 实际表现参数.动画序号,
    动画名: 实际表现参数.动画名,
    动画速度: 实际表现参数.动画速度 != null && 实际表现参数.动画速度 > 0 ? 实际表现参数.动画速度 : DEFAULT_ANIMATION_SPEED,
    残影生命周期: 实际表现参数.残影生命周期 != null && 实际表现参数.残影生命周期 > 0 ? 实际表现参数.残影生命周期 : DEFAULT_AFTERIMAGE_LIFETIME,
    残影透明度: 实际表现参数.残影透明度 != null ? 限制到字节(实际表现参数.残影透明度) : DEFAULT_AFTERIMAGE_ALPHA,
    染色R: 实际表现参数.染色R != null ? 限制到字节(实际表现参数.染色R) : 255,
    染色G: 实际表现参数.染色G != null ? 限制到字节(实际表现参数.染色G) : 255,
    染色B: 实际表现参数.染色B != null ? 限制到字节(实际表现参数.染色B) : 255,
    残影生成间隔: 实际表现参数.残影生成间隔 != null && 实际表现参数.残影生成间隔 > 0 ? 实际表现参数.残影生成间隔 : DEFAULT_AFTERIMAGE_INTERVAL,
    下次生成剩余时间: 0,
    飞行高度变化,
    已应用飞行高度变化: 飞行高度变化 !== 0,
    残影缩放: 实际表现参数.残影缩放 != null && 实际表现参数.残影缩放 > 0 ? 实际表现参数.残影缩放 : DEFAULT_AFTERIMAGE_SCALE,
    残影朝向: 实际表现参数.残影朝向,
  };
