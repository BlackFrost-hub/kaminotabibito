...
const { matchUnitFilter, isValidUnit } = require("lib.扩展函数.自定义扩展函数.02．条件判断函数") as {
  matchUnitFilter: (this: void, targetUnit: any, sourceUnit: any, options: 攻击力降低范围筛选) => boolean;
...
export interface 范围攻击力降低Buff参数 extends 攻击力降低Buff参数 {
  x?: number;
  y?: number;
  中心单位?: any;
  范围: number;
...
const 攻击力降低状态表: Record<string, 攻击力降低运行状态 | undefined> = {};
...
function 取有效BuffID(this: void, BuffID: string | undefined): string {
  return BuffID != null && BuffID !== "" ? BuffID : 默认攻击力降低BuffID;
}
...
function on攻击力降低移除(this: void, 单位: any, BuffID: string, _row: { effect: number }): void {
  const key = 取单位键(单位, BuffID);
  if (key === "") return;

  const 状态 = 攻击力降低状态表[key];
  delete 攻击力降低状态表[key];
  if (状态 == null) return;
...
export function 施加单体攻击力降低Buff(this: void, 来源单位: any, 目标单位: any, 参数: 攻击力降低Buff参数): boolean {
...
export function 施加范围攻击力降低Buff(this: void, 来源单位: any, 参数: 范围攻击力降低Buff参数): number {
