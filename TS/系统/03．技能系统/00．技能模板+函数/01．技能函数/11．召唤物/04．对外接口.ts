...
const 模块名 = "召唤物入口";
const 默认召唤物单位类型 = stringToFourCC("e08P");
...
function 规范化召唤物参数输入(参数: 召唤物输入参数): 规范化召唤物参数 {
  return {
    ...
    单位类型: 归一化单位类型(参数.单位类型 ?? 参数.unitType ?? 参数.uid) ?? 默认召唤物单位类型,
    召唤物单位: 参数.召唤物单位 ?? 参数.Summon,
    单位名称: 参数.单位名称 ?? 参数.名称 ?? 参数.名字 ?? 参数.name ?? 参数.unitName,
    X: 解析位置X(参数),
    Y: 解析位置Y(参数),
    位置: 参数.位置 ?? 参数.loc,
    朝向: 解析朝向(参数),
    ...
    模型文件: 解析模型文件(参数),
    生命值: 参数.生命值 ?? 参数.HP,
    生命值受小怪倍率: 参数.生命值受小怪倍率 ?? 参数.hpScaleWithCreep,
    生命值受难度倍率: 参数.生命值受难度倍率 ?? 参数.hpScaleWithDifficulty,
    ...
    普攻弹道自导: 参数.普攻弹道自导 ?? 参数.弹道自导 ?? 参数.missileHoming,
    索敌范围: 参数.索敌范围 ?? 参数.主动攻击范围 ?? 参数.acquireRange ?? 参数.acquire,
    护甲: 参数.护甲 ?? 参数.def,
    缩放: 参数.缩放 ?? 参数.size,
    透明度: 参数.透明度 ?? 参数.alpha,
    红: 参数.红 ?? 参数.red,
