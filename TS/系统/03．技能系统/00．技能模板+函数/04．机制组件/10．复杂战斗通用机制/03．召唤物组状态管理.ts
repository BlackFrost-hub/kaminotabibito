...
export interface 召唤物组状态参数 {
  ...
  on单位死亡?: (this: void, 单位: any, 击杀者: any, 组: 召唤物组状态, 变量?: any) => void;
  on全部死亡?: (this: void, 组: 召唤物组状态, 变量?: any) => void;
}
...
class 召唤物组状态实现 implements 召唤物组状态 {
  ...
  开始批次(预期登记数量: number = 0): void {
    ...
    this.批次开放 = true;
    this.当前批次预期登记数量 = 预期登记数量 > 0 ? 预期登记数量 : 0;
  }
  ...
  取死亡数量(): number {
    let count = 0;
    for (let i = 0; i < this.单位列表.length; i++) {
      if (this.死亡表[GetHandleId(this.单位列表[i])] === true) count++;
    }
    return count;
  }
  ...
  处理死亡(单位: any, 击杀者: any): void {
    ...
    this.死亡表[id] = true;
    if (this.参数.on单位死亡 != null) this.参数.on单位死亡(单位, 击杀者, this, this.参数.变量);
    if (this.已销毁) return;
    this.广播数量变化();
    this.尝试调度全灭();
  }
  ...
  private 尝试调度全灭(): void {
    if (this.批次开放 || this.单位列表.length <= 0 || this.取存活数量() > 0) return;
    ...
    this.全灭延迟ID = addDelayedCallback(delay, function 召唤物组全灭(this: void): void {
      ...
      if (self.参数.on全部死亡 != null) self.参数.on全部死亡(self, self.参数.变量);
