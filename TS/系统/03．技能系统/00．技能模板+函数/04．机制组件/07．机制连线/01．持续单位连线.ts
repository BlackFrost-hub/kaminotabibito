...
class 持续单位连线实现 implements 持续单位连线实例 {
  ...
  constructor(闪电: any, 参数: 持续单位连线参数) {
    ...
    this.Tick回调ID = addPeriodicCallback(参数.Tick间隔毫秒 ?? 30, on持续单位连线Tick, this);
  }
...
function on持续单位连线Tick(this: void, variable?: any): void {
  const 实例 = variable as 持续单位连线实现 | undefined;
