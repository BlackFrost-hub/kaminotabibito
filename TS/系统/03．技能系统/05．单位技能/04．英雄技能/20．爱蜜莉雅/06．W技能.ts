...
import { 爱蜜莉雅技能配置, 爱蜜莉雅W配置, 爱蜜莉雅被动配置, 爱蜜莉雅表现配置, 爱蜜莉雅音效配置 } from "./00．配置";
...
function 二段引爆W(this: void, 施法者: any, 控制器: any, 技能实例ID: number | undefined): void {
  ...
  // 二段引爆音：引爆结算点一次（确认记录：扇形冰片为同步一次齐射，按单次事件播放，不逐冰片/不循环；坐标=引爆中心=冰花区域中心，参数配置驱动）
  Sound3DII_CooPlayReuse(爱蜜莉雅音效配置.W引爆.路径, 数据.目标X, 数据.目标Y, 爱蜜莉雅音效配置.W引爆.高度, 爱蜜莉雅音效配置.W引爆.裁断距离);
...
function 释放W冰花(this: void, _context: any, 施法者: any, 技能实例ID: number | undefined): void {
  ...
  播放英雄技能喊话(施法者, "爱蜜莉雅", 爱蜜莉雅技能配置.W.技能ID);
  ...
  // 冰花区域展开音：区域真实建立后一次（二段引爆/重复进入不会走到这里；坐标=目标点，参数配置驱动）
  Sound3DII_CooPlayReuse(爱蜜莉雅音效配置.W展开.路径, 目标X, 目标Y, 爱蜜莉雅音效配置.W展开.高度, 爱蜜莉雅音效配置.W展开.裁断距离);
  ...
  const 寒气特效 = 创建点特效({
    模型路径: 爱蜜莉雅表现配置.寒气边界.模型路径,
   RGB: 爱蜜莉雅表现配置.寒气边界.RGB,
       X: 目标X,
    Y: 目标Y,
    Z: 爱蜜莉雅表现配置.寒气边界.高度,
    缩放: 爱蜜莉雅W配置.半径 / 爱蜜莉雅表现配置.寒气边界.基准半径 * 爱蜜莉雅表现配置.寒气边界.基准缩放,
    持续秒: 爱蜜莉雅表现配置.寒气边界.持续秒,
  });
  if (寒气特效 != null && 寒气特效 !== 0) {
