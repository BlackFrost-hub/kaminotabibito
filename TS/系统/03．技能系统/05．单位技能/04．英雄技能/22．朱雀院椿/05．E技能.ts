...
function 结算E终点横斩(this: void, 施法者: any, 技能实例ID: number | undefined, 数据: E数据): void {
  ...
  创建点特效({
    模型路径: 朱雀院椿表现配置.E终点主斩.模型路径,
    RGB: 朱雀院椿表现配置.E终点主斩.RGB,
    X: 数据.终点X,
    Y: 数据.终点Y,
    Z: 朱雀院椿表现配置.E终点主斩.高度,
    面向角度: 数据.方向角,
    动画索引: 0,
    缩放: 朱雀院椿表现配置.E终点主斩.缩放,
    持续秒: 朱雀院椿表现配置.E终点主斩.持续秒,
  });
  // 终点横斩音（结算点=落点；二刀追加横斩复用同一音不重复播；参数配置驱动）
  Sound3DII_CooPlayReuse(朱雀院椿音效配置.E终点横斩.路径, 数据.终点X, 数据.终点Y, 朱雀院椿音效配置.E终点横斩.高度, 朱雀院椿音效配置.E终点横斩.裁断距离);
  const 敌人 = 获取扇形区域单位({
    X: 数据.终点X,
    Y: 数据.终点Y,
    半径: E配置.横斩半径,
...
function 释放E间合(this: void, _context: any, 施法者: any, 技能实例ID: number | undefined): void {
  ...
  数据.位移ID = 开始冲锋(施法者, {
    ...
    位移特效高度: 朱雀院椿表现配置.E冲锋主层.高度,
    位移特效持续秒: 朱雀院椿表现配置.E冲锋主层.持续秒,
    附加位移特效缩放: 朱雀院椿表现配置.E冲锋主层.缩放,
    附加位移特效高度: 朱雀院椿表现配置.E冲锋主层.高度,
  ...
  // 间合冲锋音（冲锋位移实际建立时一次；单位=施法者，参数配置驱动）
  if (数据.位移ID !== 0) {
    Sound3DII_UnitPlayReuse(朱雀院椿音效配置.E冲锋.路径, 施法者, 朱雀院椿音效配置.E冲锋.裁断距离);
  }
