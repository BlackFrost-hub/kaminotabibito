...
function R创建终式(
  ...
  if ((R配置.主斩特效 as string) !== "") {
    创建点特效({
      模型路径: R配置.主斩特效,
      X: GetUnitX(施法者),
      Y: GetUnitY(施法者),
      Z: 朱雀院椿表现配置.参数.R主斩.高度,
      面向角度: 方向角,
      动画索引: 0,
      缩放: 朱雀院椿表现配置.参数.R主斩.缩放,
      // death 1000-3000（2s）：必须覆盖完整 Death，否则严重截断
      持续秒: 朱雀院椿表现配置.参数.R主斩.持续秒,
    });
  }
  ...
  if (姿态 === "二刀") {
    ...
    if ((R配置.交错斩特效 as string) !== "") {
      创建点特效({
        模型路径: R配置.交错斩特效,
        X: GetUnitX(施法者),
        Y: GetUnitY(施法者),
        Z: 朱雀院椿表现配置.参数.R交错斩.高度,
        面向角度: 方向角,
        动画索引: 0,
        缩放: 朱雀院椿表现配置.参数.R交错斩.缩放,
        // death 0-333ms，短促播放
        持续秒: 朱雀院椿表现配置.参数.R交错斩.持续秒,
      });
    }
  }
}
...
function 释放R炎姬(this: void, _context: any, 施法者: any, 技能实例ID: number | undefined): void {
  ...
  开始充能(施法者, {
    ...
    开始回调: function R蓄力开始(this: void, _单位: any, _充能ID: number): void {
      if (R配置.蓄力提示特效 != null && R配置.蓄力提示特效 !== "") {
        预警特效 = 创建点特效({
          模型路径: R配置.蓄力提示特效,
          X: 目标X,
          Y: 目标Y,
          Z: 朱雀院椿表现配置.参数.R蓄力提示.高度,
          缩放: 朱雀院椿表现配置.参数.R蓄力提示.缩放,
          持续秒: 朱雀院椿表现配置.参数.R蓄力提示.持续秒,
        });
      }
    },
