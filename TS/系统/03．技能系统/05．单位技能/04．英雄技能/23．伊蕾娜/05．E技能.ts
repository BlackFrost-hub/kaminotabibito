...
function 结算E到达(this: void, 施法者: any, 实例ID: number | undefined, 数据: any): void {
  ...
  // 落地紫柱：飞行落地冲击表现（叠加层，参数配置驱动，候选未迁入则留空不播）
  if ((伊蕾娜表现配置.E落地星柱.模型路径 as string) !== "") {
    创建点特效({
      模型路径: 伊蕾娜表现配置.E落地星柱.模型路径,
      RGB: 伊蕾娜表现配置.E落地星柱.RGB,
      X,
      Y,
      Z: 伊蕾娜表现配置.E落地星柱.高度,
      面向角度: 伊蕾娜表现配置.E落地星柱.面向角度,
      动画索引: 伊蕾娜表现配置.E落地星柱.动画索引,
      缩放: 伊蕾娜表现配置.E落地星柱.缩放,
      持续秒: 伊蕾娜表现配置.E落地星柱.持续秒,
    });
  }
