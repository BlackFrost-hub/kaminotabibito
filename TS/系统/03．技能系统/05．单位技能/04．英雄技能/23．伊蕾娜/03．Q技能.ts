...
function 处理Q命中(this: void, 施法者: any, 目标: any, 数据: any): void {
  ...
  // 命中星柱：光柱砸落在命中目标坐标（叠加层，参数配置驱动，候选未迁入则留空不播）
  if ((伊蕾娜表现配置.Q命中星柱.模型路径 as string) !== "") {
    创建点特效({
      模型路径: 伊蕾娜表现配置.Q命中星柱.模型路径,
      RGB: 伊蕾娜表现配置.Q命中星柱.RGB,
      X: GetUnitX(目标),
      Y: GetUnitY(目标),
      Z: 伊蕾娜表现配置.Q命中星柱.高度,
      面向角度: 伊蕾娜表现配置.Q命中星柱.面向角度,
      动画索引: 伊蕾娜表现配置.Q命中星柱.动画索引,
      缩放: 伊蕾娜表现配置.Q命中星柱.缩放,
      持续秒: 伊蕾娜表现配置.Q命中星柱.持续秒,
    });
  }
  // 见闻在真实命中后记录（A3）
