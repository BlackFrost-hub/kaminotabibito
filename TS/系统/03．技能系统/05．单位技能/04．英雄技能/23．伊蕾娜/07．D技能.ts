 ...
 * - 只维护一套变式状态（见闻容器承载），显示层使用三套互斥变式 Buff。
 * - R 蓄力期间锁定快照：R 开了指令中断，蓄力中按 D 会先打断 R 并还原锁定，随后 D 正常切换变式——锁定仅拦截非指令路径的改写，作为防御层保留。
...
function 显示伊蕾娜变式浮字(this: void, 英雄: any, 变式: string): void {
  const 配置 = 伊蕾娜表现配置.D变式浮字;
  CreateFloatTextOnUnit(英雄, 变式, {
    size: 配置.字体尺寸,
    red: 配置.颜色.红,
    green: 配置.颜色.绿,
    blue: 配置.颜色.蓝,
