...
const { 创建点特效, 销毁点特效, 设置特效颜色 } = require("lib.扩展函数.封装函数.01．通用工具.03．特效") as {
...
export function 创建塞莉亚节点(
  ...
  const 表现 = 塞莉亚克莱尔表现子配置.公式节点;
  // 常驻句柄：持续秒 -1，寿命由节点实例单次销毁；高度走子配置不硬编码
  const 特效句柄 = 创建点特效({
    模型路径: 表现.模型路径,
    X,
    Y,
  ...
  const 颜色 = 塞莉亚克莱尔节点颜色[类型];
  设置特效颜色(特效句柄, 颜色.r, 颜色.g, 颜色.b, 颜色.a);
