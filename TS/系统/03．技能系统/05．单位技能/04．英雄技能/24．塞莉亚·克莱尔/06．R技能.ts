...
function 范围爆发(this: void, 施法者: any, X: number, Y: number, 半径: number, 倍率: number, 数据: R领域数据, 标签: string): void {
  ...
  for (let i = 0; i < 列表.length; i++) {
    const 敌人 = 列表[i];
    if (!单位存活(敌人)) continue;
    debugLogForce("塞莉亚-R", "命中", "玩家", GetPlayerId(GetOwningPlayer(施法者)) + 1, "四码", fourCCToStringSafe(R技能类型ID), "实例", 数据.技能实例ID ?? "-", "目标", GetUnitName(敌人), "handle", 敌人, "X", Math.floor(GetUnitX(敌人)), "Y", Math.floor(GetUnitY(敌人)), "伤害", 伤害);
...
function 执行连接分支(
  ...
  debugLogForce("塞莉亚-R", "连接分支", "玩家", GetPlayerId(GetOwningPlayer(施法者)) + 1, "类型", 已消费连接.A类型 + "+" + 已消费连接.B类型);
  ...
  if (有棱晶 && 有结界) {
    ...
    // 折射冲击分支暂复用贯穿炮音（确认记录：棱晶+结界分支暂复用 r_cannon；坐标=阵心，参数配置驱动）
    Sound3DII_CooPlayReuse(塞莉亚音效配置.R贯穿炮.路径, 数据.中心X, 数据.中心Y, 塞莉亚音效配置.R贯穿炮.高度, 塞莉亚音效配置.R贯穿炮.裁断距离);
  } else if (有结界 && 有锚定) {
    // 范围封锁 + 结界爆裂
    ...
    // 结界爆裂复用 R释放（确认记录：r_release 兼结界+锚定分支的结界爆裂；坐标=阵心，参数配置驱动）
    Sound3DII_CooPlayReuse(塞莉亚音效配置.R释放.路径, 数据.中心X, 数据.中心Y, 塞莉亚音效配置.R释放.高度, 塞莉亚音效配置.R释放.裁断距离);
  } else if (有棱晶 && 有锚定) {
    ...
    // 贯穿炮音（贯穿炮分支真正开火时一次；坐标=阵心，完成瞬间与 R释放 通用爆发叠放，参数配置驱动）
    Sound3DII_CooPlayReuse(塞莉亚音效配置.R贯穿炮.路径, 数据.中心X, 数据.中心Y, 塞莉亚音效配置.R贯穿炮.高度, 塞莉亚音效配置.R贯穿炮.裁断距离);
  }
...
function 执行单节点强化(this: void, 施法者: any, 实例: 战斗技能实例控制器, 数据: R领域数据, 节点类型: 塞莉亚节点类型): void {
  debugLogForce("塞莉亚-R", "单节点强化", "玩家", GetPlayerId(GetOwningPlayer(施法者)) + 1, "类型", 节点类型);
  if (节点类型 === "棱晶") {
    发射R魔弹(施法者, 数据, {
      名称: "塞莉亚-单体棱晶折射",
      标签: "塞莉亚-闭锁·单体棱晶",
...
function 执行R完成结算(this: void, 施法者: any, 实例: 战斗技能实例控制器, 数据: R领域数据): void {
  ...
  const 核心 = 创建点特效({
    ...
    RGB: 塞莉亚克莱尔表现配置.R闭锁核心.RGB,
    X: 数据.中心X,
    Y: 数据.中心Y,
    Z: 塞莉亚克莱尔表现配置.R闭锁核心.高度,
    缩放: 塞莉亚克莱尔表现配置.R闭锁核心.缩放,
    持续秒: 塞莉亚克莱尔表现配置.R闭锁核心.持续秒,
  });
  void 核心;

  // 阵心核心音（完成回调创建阵心核心表现时一次；坐标=阵心，与 R释放 爆发叠放，参数配置驱动）
  Sound3DII_CooPlayReuse(塞莉亚音效配置.R核心.路径, 数据.中心X, 数据.中心Y, 塞莉亚音效配置.R核心.高度, 塞莉亚音效配置.R核心.裁断距离);
  ...
  for (let i = 0; i < 列表.length; i++) {
    const 敌人 = 列表[i];
    if (!单位存活(敌人)) continue;
    debugLogForce("塞莉亚-R", "命中", "玩家", GetPlayerId(GetOwningPlayer(施法者)) + 1, "四码", fourCCToStringSafe(R技能类型ID), "实例", 数据.技能实例ID ?? "-", "目标", GetUnitName(敌人), "handle", 敌人, "X", Math.floor(GetUnitX(敌人)), "Y", Math.floor(GetUnitY(敌人)), "伤害", 攻击力 * 塞莉亚克莱尔R配置.主伤害攻击力倍率);
  ...
  // 自然收束（唯一允许的收束路径）
  const 收束特效 = 创建点特效({
    模型路径: 塞莉亚克莱尔表现配置.R收束.模型路径,
    RGB: 塞莉亚克莱尔表现配置.R收束.RGB,
    X: 数据.中心X,
    Y: 数据.中心Y,
    Z: 塞莉亚克莱尔表现配置.R收束.高度,
    缩放: 塞莉亚克莱尔表现配置.R收束.缩放
      * 塞莉亚克莱尔R配置.领域半径 / 塞莉亚克莱尔表现配置.R高阶法阵.基准半径,
    持续秒: 塞莉亚克莱尔表现配置.R收束.持续秒,
  });
  void 收束特效;

  // 通用完成爆发音（自然收束（唯一收束路径）时一次；坐标=阵心，参数配置驱动）
  Sound3DII_CooPlayReuse(塞莉亚音效配置.R释放.路径, 数据.中心X, 数据.中心Y, 塞莉亚音效配置.R释放.高度, 塞莉亚音效配置.R释放.裁断距离);
...
function 释放R高阶术式(this: void, _context: any, 施法者: any, 技能实例ID: number | undefined): void {
  ...
  if (查询战斗技能实例(施法者, R技能键).length > 0) {
    debugLogForce("塞莉亚-R", "释放被拒", "玩家", GetPlayerId(GetOwningPlayer(施法者)) + 1, "原因", "已有高阶领域进行中");
    return;
  }
  ...
  debugLogForce("塞莉亚-R", "释放", "玩家", GetPlayerId(GetOwningPlayer(施法者)) + 1, "四码", fourCCToStringSafe(R技能类型ID), "实例", 技能实例ID ?? "-", "目标", "点施放", "X", Math.floor(中心X), "Y", Math.floor(中心Y));
  ...
  const 实例 = 创建战斗技能实例({
    ...
    结束回调: function R实例结束(this: void, 原因: string, _控制器: any): void {
      debugLogForce("塞莉亚-R", "结束", "玩家", GetPlayerId(GetOwningPlayer(施法者)) + 1, "四码", fourCCToStringSafe(R技能类型ID), "原因", 原因);
      // 所有结束路径解除 D 锁；蓄力状态由充能系统表达（不进玩家 Buff 栏）
      解除塞莉亚R锁定(施法者);
    },
  });
  ...
  let 法阵特效: any = null;
  debugLogForce("塞莉亚-R", "特效", "路径", 塞莉亚克莱尔表现配置.R高阶法阵.模型路径, "玩家", GetPlayerId(GetOwningPlayer(施法者)) + 1, "X", Math.floor(中心X), "Y", Math.floor(中心Y), "实例", 技能实例ID ?? "-");
  法阵特效 = 创建点特效({
    模型路径: 塞莉亚克莱尔表现配置.R高阶法阵.模型路径,
    RGB: 塞莉亚克莱尔表现配置.R高阶法阵.RGB,
    X: 中心X,
    Y: 中心Y,
    Z: 塞莉亚克莱尔表现配置.R高阶法阵.高度,
    缩放: 塞莉亚克莱尔R配置.领域半径 / 塞莉亚克莱尔表现配置.R高阶法阵.基准半径
      * 塞莉亚克莱尔表现配置.R高阶法阵.基准缩放,
    持续秒: 塞莉亚克莱尔表现配置.R高阶法阵.持续秒,
  });
  ...
  if (充能ID > 0) {
    // R 展开音（充能真正建立时一次；坐标=法阵中心，总长对齐蓄力秒，参数配置驱动）
    Sound3DII_CooPlayReuse(塞莉亚音效配置.R展开.路径, 中心X, 中心Y, 塞莉亚音效配置.R展开.高度, 塞莉亚音效配置.R展开.裁断距离);
    R守护 = 开始塞莉亚循环动作(施法者, "R蓄力");
    // 规划明确「高阶术式蓄力」不进玩家 Buff 栏，蓄力状态由充能系统与读条表达
  } else {
  ...
  // 技能喊话在充能真正建立后播放（蓄力被拒/中断不播；充能ID 作用域在释放函数内）
  if (充能ID > 0) 播放英雄技能喊话(施法者, "塞莉亚·克莱尔", 塞莉亚克莱尔技能配置.R.技能ID);
}
