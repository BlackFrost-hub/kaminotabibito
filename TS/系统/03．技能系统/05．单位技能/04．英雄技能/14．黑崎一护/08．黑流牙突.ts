...
import { 获取黑崎一护状态, 黑崎一护是否卍解, 解除黑崎一护A键武装 } from "./01．状态表";
...
const 玩家黑崎一护表: Record<number, any> = {};
...
export function 注册玩家黑流牙突A键(this: void, caster: any): void {
  ...
  // R每次启动都刷新英雄句柄；A键回调直接取该同步缓存，不再临时枚举玩家单位。
  玩家黑崎一护表[playerId] = caster;
...
const 目标独立冷却表: Record<number, boolean> = {};
...
function 清除黑流牙突标记(this: void, variable: any): void {
  const cooldown = variable as 目标冷却上下文;
  if (cooldown == null || cooldown.目标ID == null) return;
  目标独立冷却表[cooldown.目标ID] = false;
}
...
function 推进黑流牙突(this: void, variable: any): void {
  ...
  // 源 JASS 使用 SetUnitPositionLoc：由引擎处理落点，不因地形检测直接取消黑流牙突。
...
function 目标指令回调(this: void, unit: any, orderId: number, targetUnit: any, _targetItem: any, _targetDestructable: any): void {
  ...
  const 目标ID = GetHandleId(targetUnit);
  const 目标冷却中 = 目标独立冷却表[目标ID] === true;
  if (目标冷却中) return;
