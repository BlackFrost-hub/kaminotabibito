...
function R创建终式(this: void, 施法者: any, 技能实例ID: number | undefined, _目标X: number, 目标Y: number, 方向角: number): void {
  ...
  // 主斩表现（候选未迁入则留空不播）
  if (R配置.主斩特效 != null && R配置.主斩特效 !== "") {
    创建点特效({
      模型路径: R配置.主斩特效,
      X: GetUnitX(施法者),
      Y: GetUnitY(施法者),
      Z: 朱雀院红叶表现配置.参数.R主斩.高度,
      缩放: 朱雀院红叶表现配置.参数.R主斩.缩放,
      持续秒: 朱雀院红叶表现配置.参数.R主斩.持续秒,
    });
  }
    ...
    开始回调: function R蓄力开始(this: void, _单位: any, _充能ID: number): void {
      if (R配置.蓄力提示特效 != null && R配置.蓄力提示特效 !== "") {
        预警特效 = 创建点特效({
          ...
          X: 中心X,
          Y: 中心Y,
          Z: 朱雀院红叶表现配置.参数.R蓄力提示.高度,
          缩放: 朱雀院红叶表现配置.参数.R蓄力提示.缩放,
          持续秒: 朱雀院红叶表现配置.参数.R蓄力提示.持续秒,
        });
      }
    },
