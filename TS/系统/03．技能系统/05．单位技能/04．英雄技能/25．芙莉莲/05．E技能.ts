...
function 结算E落点(this: void, 施法者: any, 技能实例ID: number | undefined, 数据: E数据): void {
  ...
  // 落点观察标记表现（参数配置驱动）
  创建点特效({
    模型路径: 芙莉莲表现配置.E观察落点.模型路径,
        X: 数据.终点X,
    Y: 数据.终点Y,
    Z: 芙莉莲表现配置.E观察落点.高度,
    面向角度: 芙莉莲表现配置.E观察落点.面向角度,
    动画索引: 芙莉莲表现配置.E观察落点.动画索引,
    缩放: 芙莉莲表现配置.E观察落点.缩放,
    持续秒: 芙莉莲表现配置.E观察落点.持续秒,
    RGB: 芙莉莲表现配置.E观察落点.RGB,
  });
  // E 成功 → 提供一次演算普攻
  提供演算普攻(施法者);
}
...
function 释放E(this: void, _context: any, 施法者: any, 技能实例ID: number | undefined): void {
  ...
  播放英雄技能喊话(施法者, "芙莉莲", 芙莉莲技能配置.E.技能ID);
  // 起飞风压表现（参数配置驱动）
  创建点特效({
    模型路径: 芙莉莲表现配置.E起落风压.模型路径,
        X: 起点X,
    Y: 起点Y,
    Z: 芙莉莲表现配置.E起落风压.高度,
    面向角度: 数据.方向角,
    动画索引: 芙莉莲表现配置.E起落风压.动画索引,
    缩放: 芙莉莲表现配置.E起落风压.缩放,
    持续秒: 芙莉莲表现配置.E起落风压.持续秒,
    RGB: 芙莉莲表现配置.E起落风压.RGB,
  });
  ...
  const 起点偏移 = 芙莉莲表现配置.E闪电.起点高度偏移;
  const 终点高度 = 芙莉莲表现配置.E闪电.终点高度;
  数据.观察TickID = addPeriodicCallback(250, function E观察Tick(this: void): void {
    ...
    if (数据.观察闪电列表.length < 5) {
      ...
      while (true) {
        ...
        // 观察定位音：仅在实际"发现/记录新目标"事件播一次（不随观察 tick 循环；坐标=目标位置）
        Sound3DII_CooPlayReuse(芙莉莲音效配置.E观察.路径, GetUnitX(u), GetUnitY(u), 芙莉莲音效配置.E观察.高度, 芙莉莲音效配置.E观察.裁断距离);
        // 观察到目标 → 施加位置解析
        施加解析(施法者, u, "位置");
  ...
  数据.位移ID = 开始冲锋(施法者, {
    ...
    朝向跟随位移: true,
    暂停单位: false,
    位移特效: 芙莉莲表现配置.E起落风压.模型路径,
    RGB: 芙莉莲表现配置.E起落风压.RGB,
    位移特效缩放: 芙莉莲表现配置.E起落风压.缩放,
    位移特效高度: 芙莉莲表现配置.E起落风压.高度,
    位移特效持续秒: 芙莉莲表现配置.E起落风压.持续秒,
