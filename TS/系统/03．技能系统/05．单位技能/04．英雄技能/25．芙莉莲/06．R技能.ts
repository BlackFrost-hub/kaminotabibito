...
function R结算主炮(this: void, 施法者: any, 技能实例ID: number | undefined, 快照: R快照): void {
  ...
  创建点特效({
    ...
    X: 施法者X,
    Y: 施法者Y,
    Z: 芙莉莲表现配置.特效参数.R主炮.高度,
    面向角度: 快照.方向角,
    动画索引: 0,
    缩放: 芙莉莲表现配置.特效参数.R主炮.缩放,
    持续秒: 芙莉莲表现配置.特效参数.R主炮.持续秒,
  });
  ...
  for (let i = 0; i < 命中列表.length; i++) {
    ...
    if (快照.解析完成 && 快照.解析目标 === 目标) {
      if (尝试消费解析完成(施法者, 目标)) {
        ...
        创建点特效({
          ...
          X: GetUnitX(目标),
          Y: GetUnitY(目标),
          Z: 芙莉莲表现配置.特效参数.R破防爆发.高度,
          面向角度: 0,
          动画索引: 0,
          缩放: 芙莉莲表现配置.特效参数.R破防爆发.缩放,
          持续秒: 芙莉莲表现配置.特效参数.R破防爆发.持续秒,
        });
        // 完成爆发追加一次小伤害（同实例）
        造成技能伤害({
          来源: 施法者,
    ...
    创建点特效({
      ...
      X: GetUnitX(目标),
      Y: GetUnitY(目标),
      Z: 芙莉莲表现配置.特效参数.R命中反馈.高度,
      面向角度: 0,
      动画索引: 0,
      缩放: 芙莉莲表现配置.特效参数.R命中反馈.缩放,
      持续秒: 芙莉莲表现配置.特效参数.R命中反馈.持续秒,
    });
  }
  ...
  const 充能ID = 开始充能(施法者, {
    ...
    开始回调: function R蓄力开始(this: void, _单位: any, _充能ID: number): void {
      预警句柄 = 创建点特效({
        ...
        X: GetUnitX(施法者),
        Y: GetUnitY(施法者),
        Z: 芙莉莲表现配置.特效参数.R蓄力预警.高度,
        面向角度: 快照.方向角,
        动画索引: 0,
        缩放: 芙莉莲表现配置.特效参数.R蓄力预警.缩放,
        // 持续秒 配置驱动（默认 -1 常驻，由充能结束回调统一销毁；修改配置即生效，不再依赖公共默认）
        持续秒: 芙莉莲表现配置.特效参数.R蓄力预警.持续秒,
      });
    },
