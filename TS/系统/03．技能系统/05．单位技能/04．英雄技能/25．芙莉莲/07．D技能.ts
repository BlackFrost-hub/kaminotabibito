...
function 启动静止检测(this: void, 花田: 花田数据): void {
  花田.静止检测ID = addPeriodicCallback(500, function 花田静止检测(this: void): void {
    if (花田.已结束) return;
    const 英雄 = 花田.芙莉莲;
...
export function 尝试消费花田盛开(this: void, 芙莉莲: any): boolean {
  ...
  创建点特效({
    ...
    X: 花田.中心X,
    Y: 花田.中心Y,
    Z: 芙莉莲表现配置.特效参数.D盛开.高度,
    面向角度: 0,
    动画索引: 0,
    缩放: (花田.半径 / 芙莉莲表现配置.特效参数.D花瓣.基准半径) * 芙莉莲表现配置.特效参数.D花瓣.基准缩放 * 芙莉莲表现配置.特效参数.D盛开.缩放倍率,
    持续秒: 芙莉莲表现配置.特效参数.D盛开.持续秒,
  });
  return true;
}
...
function 销毁花田(this: void, 花田: 花田数据, 自然结束: boolean): void {
  ...
  if (花田.花瓣句柄 != null && 花田.花瓣句柄 !== 0) {
    if (自然结束) {
      const 句柄 = 花田.花瓣句柄;
      花田.花瓣句柄 = null;
      addDelayedCallback(400, function 花瓣淡出(this: void): void {
...
function 创建花海(this: void, 花田: 花田数据): void {
  ...
  for (let 行 = -配置.网格半径; 行 <= 配置.网格半径; 行++) {
    ...
    for (let 列 = -配置.网格半径; 列 <= 配置.网格半径; 列++) {
      ...
      const 图案索引 = Math.abs(行 * 7 + 列 * 11) % 3;
      const 缩放倍率 = 配置.基准缩放 * (1 + (图案索引 - 1) * 配置.缩放扰动);
      const 网格序号 = (行 + 配置.网格半径) * (配置.网格半径 * 2 + 1) + 列 + 配置.网格半径;
      const 朝向 = (网格序号 * 配置.旋转步进) % 360;
      const 花簇 = DzDoodadCreate(花海装饰物ID, 0, x, y, 配置.高度, 朝向, 缩放倍率);
      if (花簇 != null && 花簇 !== 0) {
...
function 释放D(this: void, _context: any, 施法者: any, 技能实例ID: number | undefined): void {
  ...
  花田.花瓣句柄 = 创建点特效({
    ...
    X: 目标X,
    Y: 目标Y,
    Z: 芙莉莲表现配置.特效参数.D花瓣.高度,
    面向角度: 0,
    动画索引: 0,
    缩放: (D配置.半径 / 芙莉莲表现配置.特效参数.D花瓣.基准半径) * 芙莉莲表现配置.特效参数.D花瓣.基准缩放,
    持续秒: 芙莉莲表现配置.特效参数.D花瓣.持续秒,
  });
  ...
  花田.到期回调ID = addDelayedCallback(D配置.持续秒 * 1000 + 100, function 花田到期(this: void): void {
    if (花田.已结束) return;
    销毁花田(花田, true);
    控制器.完成();
  });
