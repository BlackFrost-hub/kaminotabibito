...
function RE目标合法(this: void, state: RE状态, target: any): boolean {
  if (!单位存活(target) || !jass.IsUnitEnemy(target, jass.GetOwningPlayer(state.施法者))) return false;
  if (jass.IsUnitType(target, jass.UNIT_TYPE_ANCIENT)) return false;
  return state.已命中[jass.GetHandleId(target) as number] !== true;
}

function RE枚举目标(this: void, state: RE状态, x: number, y: number, radius: number): any[] {
  const result: any[] = [];
  jass.GroupClear(state.枚举组);
  jass.GroupEnumUnitsInRange(state.枚举组, x, y, radius, null);
  while (true) {
    const unit = jass.FirstOfGroup(state.枚举组);
    if (unit == null || unit === 0) break;
    jass.GroupRemoveUnit(state.枚举组, unit);
    if (RE目标合法(state, unit)) result.push(unit);
  }
  return result;
}
