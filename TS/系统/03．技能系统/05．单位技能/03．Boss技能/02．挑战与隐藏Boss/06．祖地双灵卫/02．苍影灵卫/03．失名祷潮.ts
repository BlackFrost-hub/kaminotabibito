...
export function 释放失名祷潮(this: void, context: 祖地双灵卫运行时上下文, target?: any): boolean {
  ...
  const resolveId = addDelayedCallback(cfg.预警秒 * 1000, function 失名祷潮结算(this: void): void {
    ...
    for (let i = 0; i < heroes.length; i++) {
      ...
      造成AOE技能伤害({ 来源: boss, 目标: hit, 伤害: damage, attack: false, ranged: true, attackType: ATTACK_TYPE_MAGIC, 伤害类型: DAMAGE_TYPE_MAGIC, weaponType: WEAPON_TYPE_WHOKNOWS, 来源类型: 'Boss技能', 标签: '祖地双灵卫·失名祷潮' });
    }
  });
