...
import { 米亚腐化感染配置, 米亚技能数值配置, 米亚音效配置, 米亚运行时配置 } from "./02．数值与表现配置";
...
function 创建米亚上下文(this: void, boss: any, 清理: 机制清理篮子): 米亚运行时上下文 {
  ...
  播放米亚台词(boss, "开场", 0);
  注册Boss阶段状态(boss, 米亚技能数值配置.阶段阈值, 读取Boss阶段序号, context, 清理);
  return context;
}
