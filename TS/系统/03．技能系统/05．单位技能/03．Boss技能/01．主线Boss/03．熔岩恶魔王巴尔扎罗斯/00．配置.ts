...
export const 巴尔扎罗斯单位技能配置 = {
  ...
  护卫: {
    格鲁姆: {
      ...
      台词: {
        响应召令: ["遵命，吾王。（优先处理格鲁姆，避开他的正面锤击与熔岩火线。）"],
        熔岩重锤: ["都趴下！我要把你们砸进地底！（远离格鲁姆正面90°扇形、半径360码的预警区；命中会眩晕1.25秒并叠2层灼热。）"],
        熔岩火径: ["火路已开。无路可退。（避开长640、宽120码的火线；线上每0.25秒叠1层灼热，穿越火线还会受到额外伤害。）"],
        死亡: ["吾王……格鲁姆不能再战了……"],
      },
      配音台词: {
        响应召令: ["[obedient growl] As you command, my king."],
        熔岩重锤: ["[battle roar] Down! I will hammer you into the earth!"],
        熔岩火径: ["[grim] The path burns. There is no retreat."],
        死亡: ["[dying strain] My king... Grum can fight no longer..."],
      },
      配音资源: {
        响应召令: ["Sound\\Boss\\Balzaroth\\Voice\\balzaroth_guard_grum_answer_command_01_v3.mp3"],
        熔岩重锤: ["Sound\\Boss\\Balzaroth\\Voice\\balzaroth_guard_grum_molten_hammer_01_v3.mp3"],
        熔岩火径: ["Sound\\Boss\\Balzaroth\\Voice\\balzaroth_guard_grum_lava_path_01_v3.mp3"],
        死亡: ["Sound\\Boss\\Balzaroth\\Voice\\balzaroth_guard_grum_death_01_v3.mp3"],
      },
    },
    塞拉: {
      名称: "冰焰巫师·塞拉",
  ...
  主动技能提示: [
    { 技能ID: "A0C9", 提示: "恶魔咆哮波", 扩展提示: "施法硬直1.2秒后，向正面发射宽220、长1200码的直线冲击；命中一次造成Boss当前攻击力×200%+目标最大生命值×5%伤害。8秒内第二次命中同一目标会眩晕1秒，命中护卫会恢复其最大生命值8%（护卫模仿版本宽度与伤害均为本体60%）。离开正面路径即可规避。" },
    { 技能ID: "A0CC", 提示: "熔岩喷发", 扩展提示: "施法硬直1.2秒，目标附近180码内随机落点；1.2秒后在半径260码内爆发，造成Boss攻击力×140%+目标最大生命值×4%伤害并顶飞0.6秒，附加1层灼热。爆发后生成半径240码熔岩残留，持续8秒，每1秒造成目标最大生命值×1%并附加1层灼热。离开红色预警圈并避开残留区。" },
    { 技能ID: "A0CA", 提示: "王者天罚", 扩展提示: "引导5秒，在第1、3、5秒落下三波天罚；基础落点半径260码，P2扩大为299码，P3每波额外增加2个随机落点（半径220码）。每波造成Boss攻击力×120%+目标最大生命值×4%伤害；同一目标连续命中3波会额外叠2层灼热。离开每次脚下预警圈。" },
    { 技能ID: "A0CB", 提示: "火焰锁链", 扩展提示: "施法硬直0.9秒，优先点名距Boss至少600码的最远目标，不足600码时随机点名；目标脚下显示半径180码锁定预警。锁链持续8秒，场中生成可攻击锁链单位（最大生命=Boss最大生命值×7%）；Boss与目标距离超过650码时每1秒造成Boss当前攻击力×80%+目标最大生命值×2%伤害。击杀锁链单位，或保持与Boss不超过650码，可结束威胁。" },
  ],
  广播持续时间Ms: 4200,
  配音裁断距离: 4000,
  配音生成配置: {
...
export type 格鲁姆台词类型 = keyof typeof 巴尔扎罗斯单位技能配置.护卫.格鲁姆.台词;
export type 塞拉台词类型 = keyof typeof 巴尔扎罗斯单位技能配置.护卫.塞拉.台词;
