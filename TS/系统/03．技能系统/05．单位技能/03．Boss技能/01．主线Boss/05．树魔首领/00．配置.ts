...
export const 树魔首领单位技能配置 = {
  ...
  主动技能提示: [
    { 技能ID: "AN04", 提示: "扩散冲击波", 扩展提示: "蓄力1秒后以Boss为中心向360°发射16道外扩弹幕；每道命中一次造成Boss当前攻击力×200%伤害，命中目标的普通攻击伤害降低40%，持续7秒。预警半径720码（离开预警圈）。" },
    { 技能ID: "AN05", 提示: "消耗反击", 扩展提示: "正面防御持续3秒；正面270°内只承受原伤害的10%，受击后向攻击者发射反击弹幕，造成Boss当前攻击力×300%伤害并抽取其本次伤害值20%的魔法值。背后120°内攻击可破招，使本次伤害提高50%并令Boss硬直2秒（优先从背后攻击）。" },
    { 技能ID: "AT16", 提示: "远古诅咒", 扩展提示: "点名目标等待3秒；第一段总伤害=点名目标结算时当前生命值×（60%+20%×N），N为当前有效玩家数且至少按1人计算，点名目标与其400码内玩家均分。第一段后所有有效玩家恢复等于该总伤害的生命值；N≥2时，1.8秒后在玩家中心650码内触发第二段。" },
    { 技能ID: "AN06", 提示: "树魔图腾", 扩展提示: "吟唱0.8秒后按存活随从抽取图腾：巫医→静止陷阱（300码触发，0.75秒后全体眩晕8秒）、猎头者→生命陷阱（持续20秒、影响半径900码、每秒造成目标最大生命3%并降低治疗50%+难度×5%）、投掷者→爆炸陷阱（10秒、被摧毁0.5秒后全体承受当前生命50%+难度×1800魔法伤害）；多种随从按存活数量加权，无随从时随机。" },
  ],
  召唤物ID: {
    猎头者: "ohun",
    巫医: "odoc",
