...
function on检查失败惩罚(this: void, variable?: any): void {
  ...
  for (let i = 0; i < 目标列表.length; i++) {
    ...
    if (结果.是否造成伤害) {
      ...
      播放Boss坐标音效(利尔伯特音效配置.检查.失败惩罚, GetUnitX(target), GetUnitY(target), 利尔伯特音效配置.默认裁断距离);
    }
  }
  结束检查状态(状态);
}
...
export function 释放利尔伯特检查(this: void, 上下文: 利尔伯特运行时上下文, target: any): boolean {
  ...
  播放Boss坐标音效(利尔伯特音效配置.检查.装备抽取, GetUnitX(target), GetUnitY(target), 利尔伯特音效配置.默认裁断距离);
