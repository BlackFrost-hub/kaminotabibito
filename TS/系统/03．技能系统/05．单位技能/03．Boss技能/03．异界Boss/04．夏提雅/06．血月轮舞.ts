...
export function 释放夏提雅血月轮舞(this: void, context: 夏提雅运行时上下文, target: any): boolean {
  ...
  const firstId = addDelayedCallback(cfg.第一段预警秒 * 1000, function 血月轮舞横扫结算(this: void): void {
  ...
  const secondId = addDelayedCallback((cfg.第一段预警秒 + secondDelay) * 1000, function 血月轮舞反刺结算(this: void): void {
    if (!单位有效(boss) || context.挑战已结束) return;
    const heroes = 获取Boss技能敌对英雄列表(boss);
  ...
  context.清理.登记延迟回调('夏提雅-血月轮舞横扫', firstId);
  context.清理.登记延迟回调('夏提雅-血月轮舞反刺', secondId);
  return true;
}
