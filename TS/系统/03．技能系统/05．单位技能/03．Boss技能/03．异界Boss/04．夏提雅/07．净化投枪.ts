...
const { addDelayedCallback, getServerTime } = require('系统.00．核心系统.05．中心计时器') as { addDelayedCallback: (this: void, ms: number, callback: (this: void) => void) => number; getServerTime: (this: void) => number };
...
export function 释放夏提雅净化投枪(this: void, context: 夏提雅运行时上下文, target: any): boolean {
  ...
  if (isP3) 创建技能提示圈({ 类型: '敌方圆形', X: secondX, Y: secondY, 半径: cfg.伤害半径, 持续时间: totalDuration, 来源单位: boss });
  播放限时单位动画({ 单位: boss, 动画编号: cfg.动画编号, 持续秒: totalDuration, 恢复动画编号: 0 });
  const delayedId = addDelayedCallback(cfg.预警秒 * 1000, function 净化投枪落下(this: void): void {
    if (!单位有效(boss) || context.挑战已结束) return;
    结算净化投枪落点(context, x, y, '夏提雅·净化投枪');
    if (!isP3) 尝试安排净化投枪英灵复刻(context, x, y);
  });
  context.清理.登记延迟回调('夏提雅-净化投枪', delayedId);
  if (isP3) {
    const secondId = addDelayedCallback(totalDuration * 1000, function 净化投枪第二枚落下(this: void): void {
      if (!单位有效(boss) || context.挑战已结束 || context.阶段 !== 'P3真祖血宴') return;
      结算净化投枪落点(context, secondX, secondY, '夏提雅·净化投枪-P3第二枚');
    });
    context.清理.登记延迟回调('夏提雅-净化投枪-P3第二枚', secondId);
  }
  return true;
}
