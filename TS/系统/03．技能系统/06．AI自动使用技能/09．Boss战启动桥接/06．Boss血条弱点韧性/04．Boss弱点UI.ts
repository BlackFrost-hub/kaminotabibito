...
function 创建Boss弱点问号UI(this: void, state: Boss血条弱点韧性运行状态): void {
  ...
  for (let i = 0; i < weakList.length; i++) {
    const x = Boss弱点UI常量.弱点起始X + Boss弱点UI常量.弱点间距 * (i + 1);
    const frame = DzCreateFrameByTagName("BACKDROP", "BossWeakQuestion", state.血条Frame, "template", i + 1);
    DzFrameSetSize(frame, Boss弱点UI常量.弱点图标宽, Boss弱点UI常量.弱点图标高);
...
function 创建Boss护盾说明UI(this: void, state: Boss血条弱点韧性运行状态): void {
  ...
  DzFrameSetSize(state.护盾图标Frame, Boss弱点UI常量.护盾图标宽, Boss弱点UI常量.护盾图标高);
  ...
  DzFrameSetSize(state.护盾说明按钮Frame, Boss弱点UI常量.护盾说明按钮宽, Boss弱点UI常量.护盾说明按钮高);
  ...
  DzFrameSetSize(state.破碎护盾Frame, Boss弱点UI常量.破碎护盾宽, Boss弱点UI常量.破碎护盾高);
  ...
  DzFrameSetSize(state.灰色护盾Frame, Boss弱点UI常量.护盾图标宽, Boss弱点UI常量.护盾图标高);
...
export function 显示Boss弱点真实图标(this: void, state: Boss血条弱点韧性运行状态, weakIndex: number): void {
  ...
  const x = state.弱点X轴列表[weakIndex] || (Boss弱点UI常量.弱点起始X + Boss弱点UI常量.弱点间距 * (weakIndex + 1));
  const iconFrame = DzCreateFrameByTagName("BACKDROP", "BossWeakIcon", state.血条Frame, "template", 30 + weakIndex);
  DzFrameSetSize(iconFrame, Boss弱点UI常量.弱点图标宽, Boss弱点UI常量.弱点图标高);
...
export function 设置Boss弱点命中表现(this: void, state: Boss血条弱点韧性运行状态, weakIndex: number, active: boolean): void {
  ...
  DzFrameSetSize(
    frame,
    active ? Boss弱点UI常量.弱点命中图标宽 : Boss弱点UI常量.弱点图标宽,
    active ? Boss弱点UI常量.弱点命中图标高 : Boss弱点UI常量.弱点图标高,
  );
}
