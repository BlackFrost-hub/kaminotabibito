...
const 保留剧情Boss死亡击杀者字段 = "保留剧情Boss死亡击杀者";
...
function onBoss死亡结算事件(this: void, dyingUnit: any, killingUnit: any): void {
  ...
  if (配置.保留原剧情执行 === true) {
    if (killingUnit != null && killingUnit !== 0) {
      YDUserDataSetSafe("unit", dyingUnit, 保留剧情Boss死亡击杀者字段, "unit", killingUnit);
    }
    return;
  }
...
export function 消费保留剧情Boss死亡击杀者(this: void, bossUnit: any): any {
  if (bossUnit == null || bossUnit === 0) return null;
  const killingUnit = YDUserDataGetSafe("unit", bossUnit, 保留剧情Boss死亡击杀者字段, "unit");
  YDUserDataClearSafe("unit", bossUnit, 保留剧情Boss死亡击杀者字段, "unit");
  return killingUnit;
}
