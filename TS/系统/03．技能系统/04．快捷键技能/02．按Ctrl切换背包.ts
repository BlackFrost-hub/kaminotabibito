...
const { UnitItemInSlotBJ } = require("lib.扩展函数.BJ函数.index") as {
  UnitItemInSlotBJ: (whichUnit: any, itemSlot: number) => any;
};
const { UnitRemoveItemSwapped } = require("lib.扩展函数.BJ函数.index") as {
  UnitRemoveItemSwapped: (whichItem: any, whichHero: any) => void;
};
 ...
 */
function swapItems(unit1: any, unit2: any, slot: number): void {
  // 获取单位1的物品
...
function onCtrlSwitchBag(): void {
  ...
  // 检查英雄是否被当前玩家选中
  if (!jass.IsUnitSelected(hero, player)) return;
 ...
 * 初始化背包切换功能
 */
export function initSwitchBag(): void {
