...
const { 米亚技能数值配置 } = require("系统.03．技能系统.05．单位技能.03．Boss技能.02．挑战与隐藏Boss.02．污染之猫米亚.02．数值与表现配置") as {
  米亚技能数值配置: { 阶段阈值: { P2生命比例: number; P3生命比例: number } };
};
...
function 设置米亚测试阶段(this: void, context: any, 阶段: 米亚测试阶段): boolean {
  ...
  const 生命比例 = 阶段 === 1
    ? 1
    : 阶段 === 2
      ? (米亚技能数值配置.阶段阈值.P2生命比例 + 米亚技能数值配置.阶段阈值.P3生命比例) * 0.5
      : 米亚技能数值配置.阶段阈值.P3生命比例 * 0.5;
