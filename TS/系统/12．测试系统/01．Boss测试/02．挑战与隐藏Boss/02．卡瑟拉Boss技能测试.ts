...
const { 获取或创建卡瑟拉上下文, 清理卡瑟拉上下文 } = require("系统.03．技能系统.05．单位技能.03．Boss技能.02．挑战与隐藏Boss.03．深渊巨鱿卡瑟拉.01．运行时上下文") as {
  获取或创建卡瑟拉上下文: (this: void, boss: any) => any;
  清理卡瑟拉上下文: (this: void, boss: any) => void;
};
...
function 获取或创建测试Boss(this: void, player: any): any {
  ...
  const boss = CreateUnit(player, stringToFourCC("N05V"), 临时测试场地中心X, 临时测试场地中心Y, 270);
  if (boss != null && boss !== 0) {
    卡瑟拉测试Boss[pid] = boss;
    标记测试Boss跳过死亡结算(boss);
...
function on卡瑟拉技能1测试命令(this: void, _player: any, context: any): void {
  if (context != null) 释放卡瑟拉深海涡流(context);
}

function on卡瑟拉技能2测试命令(this: void, _player: any, context: any): void {
  if (context != null) 释放卡瑟拉触手鞭笞(context);
}

function on卡瑟拉技能3测试命令(this: void, _player: any, context: any): void {
  if (context != null) 释放卡瑟拉墨汁喷吐(context);
}
