...
import { QUEST_STES_OBJECTIVE_ROWS } from "./任务STES配置表";
import { questDB } from "./任务数据";
...
function applyObjectiveRow(
  ...
  const pdata = questDB.getPlayerData(playerId);
  if (!pdata) {
    debugPrint(`[任务STES] 无玩家数据 playerId=${playerId} event=${eventKey}`);
    return;
  }
  const quest = pdata.quests.get(row.questId);
  if (!quest) {
    debugPrint(`[任务STES] 玩家未接任务 questId=${row.questId} event=${eventKey}`);
    return;
  }
