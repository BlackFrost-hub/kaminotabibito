...
import {
  giveQuestRewards,
  showAbandonedQuestNotice,
  showQuestAcceptedMessage,
  showQuestCompletedMessage,
  showQuestFailedMessage,
  showQuestTrackingNotice,
} from "./02．任务提示与奖励";
...
export class QuestManager {
  ...
  /**
   * 自定义追踪：当前实现为提示 + `triggerUIRefresh`；若需持久「当前追踪 questId」可在 questDB 扩展字段。
   */
  public toggleQuestTracking(playerId: number, questId: string): boolean {
    ...
    questDebugPrint(`已追踪任务 ${questId}`);

    showQuestTrackingNotice(playerId, questData.title);
  ...
  public resetPlayerQuests(playerId: number): void {
    ...
    questDebugPrint(`已重置玩家 ${playerId} 的任务数据`);
  }
