...
export function questTypes(): QuestType[] {
  return [QuestType.MAIN, QuestType.SIDE, QuestType.DAILY];
}

export function createEmptyQuestIdList(): string[] {
  const questIds: string[] = [];
  for (let i = 0; i < ROWS_PER_PAGE; i++) questIds.push("");
  return questIds;
}

export function buildObjectiveText(quest: QuestData, index: number): string {
...
export function buildRewardText(quest: QuestData): string {
...
export function buildInfoText(quest: QuestData): string {
...
export function chunkQuests(quests: QuestData[]): QuestData[][] {
...
export function findExpandedVariantIndex(page: PageLike, expandedQuestId: string | null): number {
  if (!expandedQuestId) return 0;
  for (let i = 0; i < page.questIds.length; i++) {
    if (page.questIds[i] === expandedQuestId) return i + 1;
  }
  return 0;
}

export function hideAllCategoryPages(categoryView: CategoryViewLike, setVisible: SetVisibleLike): void {
...
export function showOnlyPageAndVariant(
  categoryView: CategoryViewLike,
  pageIndex: number,
  variantIndex: number,
