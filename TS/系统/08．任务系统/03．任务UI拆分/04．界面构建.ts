...
import { tryCreateFromFdfOnly, tryCreateFromFdfWithSource } from "./01．通用工具";
...
export function buildTaskEntryIcon(opts: {
  japi: any; parent: number; FrameType: any; FramePoint: any; createFrame: any; createTextLabel: any;
  setFramePosition: any; setFrameSize: any; setFramePointRelative: any; setFrameClickEvent: any; applyDzTextFontAndCenterAlignment: any;
  onClickSound: () => void; onTogglePanel: () => void;
  ...
  const titleRel = { relativeTo: entryFrame, point: FramePoint.CENTER, relativePoint: FramePoint.CENTER, x: 0, y: 0 };
  let entryText: number | null = null;
  const textFrame = createFrame({ type: FrameType.TEXT, name: "TaskEntryText", parent: entryFrame, template: "template", visible: true }) ?? 0;
  ...
  if (entryText != null && entryText !== 0) {
    if (typeof (japi as any).DzFrameSetText === "function") (japi as any).DzFrameSetText(entryText, "|cffffcc00任务(J)|r");
    applyDzTextFontAndCenterAlignment(entryText);
  }
  const btn = createFrame({ type: FrameType.GLUETEXTBUTTON, name: "TaskEntryBtn", parent: entryFrame, template: "template", visible: true, enable: true, alpha: 0 }) ?? 0;
  if (btn && typeof (japi as any).DzFrameSetAllPoints === "function") {
    (japi as any).DzFrameSetAllPoints(btn, entryFrame);
    setFrameClickEvent(btn, () => { onClickSound(); onTogglePanel(); }, false);
  }
  return { entryFrame, entryText };
}

export interface BuildMainPanelResult {
  mainPanel: number | null; listContainer: number | null; tabMainBg: number | null; tabMain: number | null; tabSideBg: number | null; tabSide: number | null; tabDailyBg: number | null; tabDaily: number | null; scrollBarFrame: number | null; scrollThumbFrame: number | null; scrollThumbHitBtn: number | null; vScrollTrack: VerticalScrollbarTrack | null;
}

function createTaskTab(opts: {
  japi: any; tabParent: number; bgName: string; tabName: string; labelName: string; x: number; labelText: string; category: QuestType; tooltip: string;
  FramePoint: any; setFramePointRelative: any; setFrameSize: any; setFrameHoverEvents: any; setFrameClickEvent: any; setButtonText: any;
  createTabLabelTextOnBackdrop: any; setupTransparentGlueHitLayer: any; onClickSound: () => void; onSwitchCategory: (type: QuestType) => void; onShowTabTooltip: (msg: string) => void;
  ...
  if (tab) {
    ...
    if (bg) setupTransparentGlueHitLayer(bg, tab);
    else {
    ...
    if (!bg) {
      setButtonText(tab, "");
      if (typeof (japi as any).DzFrameSetAlpha === "function") (pcall as any)(() => (japi as any).DzFrameSetAlpha(tab, 0));
    }
    if (typeof (japi as any).DzFrameSetLevel === "function") (japi as any).DzFrameSetLevel(tab, 9);
    setFrameClickEvent(tab, () => { onClickSound(); onSwitchCategory(category); }, false);
...
export function buildTaskMainPanel(opts: {
  japi: any; parent: number; entryFrame: number | null; FrameType: any; FramePoint: any; createFrame: any; setFramePosition: any; setFrameSize: any; setFramePointRelative: any; setFrameTexture: any; setFrameHoverEvents: any; setFrameClickEvent: any; setButtonText: any; createTabLabelTextOnBackdrop: any; setupTransparentGlueHitLayer: any; onClickSound: () => void; onSwitchCategory: (type: QuestType) => void; onShowTabTooltip: (msg: string) => void; getTotalContentHeight: () => number; getScrollOffset: () => number; setScrollOffset: (v: number) => void; isVisible: () => boolean; onScrollChanged: () => void;
  ...
  if (!mainPanel) return { mainPanel: null, listContainer: null, tabMainBg: null, tabMain: null, tabSideBg: null, tabSide: null, tabDailyBg: null, tabDaily: null, scrollBarFrame: null, scrollThumbFrame: null, scrollThumbHitBtn: null, vScrollTrack: null };
  if (typeof (japi as any).DzFrameClearAllPoints === "function") (japi as any).DzFrameClearAllPoints(mainPanel);
  if (entryFrame) setFramePointRelative(mainPanel, FramePoint.TOPLEFT, entryFrame, FramePoint.TOPLEFT, PANEL_REL_TO_ENTRY_X, PANEL_REL_TO_ENTRY_Y);
  else setFramePosition(mainPanel, { point: FramePoint.TOPLEFT, x: ENTRY_X + PANEL_REL_TO_ENTRY_X, y: ENTRY_Y + PANEL_REL_TO_ENTRY_Y });
  ...
  const mainResult = createTaskTab({ japi, tabParent, bgName: "TaskTabMainBg", tabName: "TaskTabMain", labelName: "TaskTabMainLabel", x: 0.02, labelText: "|cffffcc00主线(1)|r", category: QuestType.MAIN, tooltip: "按 1 切换主线任务", FramePoint, setFramePointRelative, setFrameSize, setFrameHoverEvents, setFrameClickEvent, setButtonText, createTabLabelTextOnBackdrop, setupTransparentGlueHitLayer, onClickSound, onSwitchCategory, onShowTabTooltip });
  const sideResult = createTaskTab({ japi, tabParent, bgName: "TaskTabSideBg", tabName: "TaskTabSide", labelName: "TaskTabSideLabel", x: 0.135, labelText: "|cffffcc00支线(2)|r", category: QuestType.SIDE, tooltip: "按 2 切换支线任务", FramePoint, setFramePointRelative, setFrameSize, setFrameHoverEvents, setFrameClickEvent, setButtonText, createTabLabelTextOnBackdrop, setupTransparentGlueHitLayer, onClickSound, onSwitchCategory, onShowTabTooltip });
  const dailyResult = createTaskTab({ japi, tabParent, bgName: "TaskTabDailyBg", tabName: "TaskTabDaily", labelName: "TaskTabDailyLabel", x: 0.25, labelText: "|cffffcc00小任务(3)|r", category: QuestType.DAILY, tooltip: "按 3 切换小任务", FramePoint, setFramePointRelative, setFrameSize, setFrameHoverEvents, setFrameClickEvent, setButtonText, createTabLabelTextOnBackdrop, setupTransparentGlueHitLayer, onClickSound, onSwitchCategory, onShowTabTooltip });
  const sbSrc = tryCreateFromFdfWithSource("TaskScrollBar", mainPanel, () => (createFrame({ type: FrameType.BACKDROP, name: "TaskScrollBarBtn", parent: mainPanel, template: "template", visible: true }) ?? 0));
  ...
  const scrollThumbFrame = createFrame({ type: FrameType.BACKDROP, name: "TaskScrollThumbDyn", parent: mainPanel, template: "template", visible: true }) ?? 0;
  ...
  if (scrollThumbFrame && scrollThumbFrame !== 0 && scrollBarFrame && scrollBarFrame !== 0) {
    vScrollTrack = new VerticalScrollbarTrack({ trackFrame: scrollBarFrame, thumbFrame: scrollThumbFrame, hitButtonName: "TaskScrollThumbHit", listViewHeightNorm: LIST_VIEW_H, trackHeightNorm: LIST_VIEW_H, thumbSizeNorm: SCROLL_THUMB_SIZE, topCompensation: SCROLL_THUMB_TOP_COMPENSATION, bottomCompensation: SCROLL_THUMB_BOTTOM_COMPENSATION, dragTick: THUMB_DRAG_TICK, sensitivity: THUMB_DRAG_SENSITIVITY, getTotalContentHeight, getScrollOffset, setScrollOffset, isInteractionEnabled: isVisible, onScrollChanged, skipManualThumbSync: () => false });
    vScrollTrack.attach();
    scrollThumbHitBtn = vScrollTrack.getHitButtonFrame();
  }
  return { mainPanel, listContainer, tabMainBg: mainResult.bg, tabMain: mainResult.tab, tabSideBg: sideResult.bg, tabSide: sideResult.tab, tabDailyBg: dailyResult.bg, tabDaily: dailyResult.tab, scrollBarFrame: scrollBarFrame ?? null, scrollThumbFrame: scrollThumbFrame ?? null, scrollThumbHitBtn, vScrollTrack };
}
