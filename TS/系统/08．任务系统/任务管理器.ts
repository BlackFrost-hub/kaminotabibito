...
import { questDB, QuestType, QuestStatus, QuestData, createTestQuests } from "./任务数据";
...
export class QuestManager {
  ...
  private setupTimeLimit(playerId: number, questId: string): void {
    const quest = questDB.getPlayerData(playerId)?.quests.get(questId);
  ...
  public onQuestAbandoned(playerId: number, questId: string): boolean {
    ...
    const nativeHandle = questDB.getPlayerData(playerId)?.quests.get(questId)?.nativeHandle;
  ...
  public toggleQuestTracking(playerId: number, questId: string): boolean {
    const questData = questDB.getPlayerData(playerId)?.quests.get(questId);
