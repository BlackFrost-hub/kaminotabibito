...
function parseItemIdList(spec: string): string[] {
  const trimmed = spec.trim();
  const prefix = "itemId(";
  if (trimmed.indexOf(prefix) !== 0) return [];
  let end = -1;
  ...
  for (const p of parts) {
    const code = p.trim();
    if (code.length === 4) out.push(code);
  }
  return out;
}

function pickRandomDistinct(list: string[], count: number): string[] {
  const out: string[] = [];
...
// ========== 虚拟分区：动作执行 ==========
function execRemoveItemFromStock(whichUnit: any, arg: string, modifiers: string[]): void {
  ...
  for (const code of targets) {
    ...
    if (id !== 0) RemoveItemFromStockBJ(id, whichUnit);
  }
}
 ...
 */
export function runNpcInitAction(whichUnit: any, initAction: string | undefined | null): void {
  if (!whichUnit || !initAction) return;
