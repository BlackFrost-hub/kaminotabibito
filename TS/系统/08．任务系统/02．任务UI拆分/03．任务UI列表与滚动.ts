...
export function isTaskScrollBarTrackHit(
  ...
  const f = typeof getMouseFocus === "function" ? getMouseFocus() : 0;
  if (!f || f === 0) return false;
  if (!scrollBarFrame || scrollBarFrame === 0) return false;
  if (!(f === scrollBarFrame || isDescendantOf(japi, f, scrollBarFrame))) return false;
  if (scrollThumbHitBtn && (f === scrollThumbHitBtn || isDescendantOf(japi, f, scrollThumbHitBtn))) return false;
  if (scrollThumbFrame && (f === scrollThumbFrame || isDescendantOf(japi, f, scrollThumbFrame))) return false;
  return true;
}
