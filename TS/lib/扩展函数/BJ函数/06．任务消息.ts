...
export function QuestMessageBJ(f: any, messageType: number, message: string): void {
    ...
    if (messageType === bj_QUESTMESSAGE_DISCOVERED) {
        ...
        play(bj_questDiscoveredSound); flash(); return;
    }
    if (messageType === bj_QUESTMESSAGE_UPDATED) {
        ...
        play(bj_questUpdatedSound); flash(); return;
    }
    if (messageType === bj_QUESTMESSAGE_COMPLETED) {
        ...
        play(bj_questCompletedSound); flash(); return;
    }
    if (messageType === bj_QUESTMESSAGE_FAILED) {
        ...
        play(bj_questFailedSound); flash(); return;
    }
    if (messageType === bj_QUESTMESSAGE_REQUIREMENT) {
        jass.DisplayTimedTextToPlayer(lp, 0, 0, bj_TEXT_DELAY_QUESTREQUIREMENT, message); return;
    }
    if (messageType === bj_QUESTMESSAGE_MISSIONFAILED) {
        ...
        play(bj_questFailedSound); return;
    }
    ...
    if (messageType === bj_QUESTMESSAGE_SECRET) {
        ...
        play(bj_questSecretSound); return;
    }
    ...
    if (messageType === bj_QUESTMESSAGE_ITEMACQUIRED) {
        ...
        play(bj_questItemAcquiredSound); return;
    }
