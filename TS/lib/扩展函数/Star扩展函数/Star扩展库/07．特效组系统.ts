 ...
 * @returns 特效组ID
 */
export function EG_CreateEffectGroup(): number {
 ...
 * @param id 特效组ID
 */
export function EG_RemoveGroup(id: number): void {
  delete groups[id];
}
 ...
 * @param id 特效组ID
 */
export function EG_ClearGroup(id: number): void {
  const g = groups[id];
  if (g == null) return;
  g.length = 0;
}
 ...
 * @returns 是否添加成功
 */
export function EG_GroupAddEffect(e: any, id: number): boolean {
  const g = groups[id];
  if (g == null) return false;

  for (let i = 0; i < g.length; i++) {
    if (g[i] === e) return false;
  }
 ...
 * @returns 是否添加成功
 */
export function EG_GroupAddEffectEx(e: any, id: number): boolean {
  const g = groups[id];
  if (g == null) return false;
 ...
 * @returns 是否移除成功
 */
export function EG_RemoveEffectOfGroup(e: any, id: number): boolean {
  const g = groups[id];
  if (g == null) return false;
 ...
 */
export function EG_ForGroup(id: number, callback: (e: any) => void): void {
  const g = groups[id];
  if (g == null) return;
 ...
 */
export function EG_GetFirstOfGroup(id: number): any {
  const g = groups[id];
 ...
 */
export function EG_GetRandomOfGroup(id: number): any {
  const g = groups[id];
  if (g == null || g.length === 0) return null;
 ...
 */
export function EG_IsEffectOnGroup(e: any, id: number): number {
  const g = groups[id];
  if (g == null) return -1;
 ...
 * @returns 是否存在
 */
export function EG_IsGroupHaveEffect(e: any, id: number): boolean {
 ...
 * @returns 是否为空
 */
export function EG_IsGroupEmpty(id: number): boolean {
  const g = groups[id];
 ...
 * @returns 特效数量
 */
export function EG_GetCount(id: number): number {
  const g = groups[id];
  if (g == null) return 0;
  return g.length;
}
 ...
 * @param destId 源特效组ID
 */
export function EG_GroupAddGroup(srcId: number, destId: number): void {
 ...
 * 整数转特效组ID（恒等转换）
 */
export function EG_I2EG(id: number): number {
  return id;
}
 ...
 * 特效组ID转整数（恒等转换）
 */
export function EG_EG2I(eg: number): number {
  return eg;
}
