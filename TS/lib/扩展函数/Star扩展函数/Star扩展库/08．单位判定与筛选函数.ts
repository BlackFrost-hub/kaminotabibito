...
export function SUC_IsValidUnit(u: any): boolean {
  return u != null && u !== 0;
}
...
// 判断单位是否还活着。
export function SUC_IsUnitAlive(u: any): boolean {
...
// 统一基础目标过滤。
export function SUC_MatchBasicTarget(target: any, source: any, wantEnemy: boolean): boolean {
  if (!SUC_IsValidUnit(target) || !SUC_IsValidUnit(source)) return false;
  if (SUC_IsUnitInvincible(target)) return false;
  if (SUC_IsUnitStructure(target)) return false;
  if (!SUC_IsUnitAlive(target)) return false;
