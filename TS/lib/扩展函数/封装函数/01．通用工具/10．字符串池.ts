...
const 字符串池映射: Record<number, 字符串池数据 | undefined> = {};
let 下一个字符串池ID = 0;

function 取字符串池(this: void, pool: number): 字符串池数据 | undefined {
...
function 删除下标(this: void, data: 字符串池数据, index: number): void {
...
function 随机取下标(this: void, data: 字符串池数据): number {
  let totalWeight = 0;
  let i = 0;
...
export function 创建字符串池(this: void): number {
  下一个字符串池ID += 1;
  const id = 下一个字符串池ID;
  字符串池映射[id] = {
    id,
...
export function 删除字符串池(this: void, pool: number): void {
  delete 字符串池映射[pool];
}

export function 字符串池是否存在(this: void, pool: number): boolean {
  return 取字符串池(pool) != null;
}

export function 字符串池是否为空(this: void, pool: number): boolean {
  const data = 取字符串池(pool);
  return data == null || data.values.length === 0;
}

export function 获取字符串池数量(this: void, pool: number): number {
  const data = 取字符串池(pool);
 ...
 */
export function 字符串池查找(this: void, pool: number, value: string): number {
  const data = 取字符串池(pool);
...
export function 字符串池添加(this: void, pool: number, value: string, weight?: number): boolean {
  const data = 取字符串池(pool);
...
export function 字符串池删除字符串(this: void, pool: number, value: string): boolean {
  const data = 取字符串池(pool);
  if (data == null) return false;
...
export function 字符串池随机取字符串(this: void, pool: number): string {
  const data = 取字符串池(pool);
  if (data == null) return "";
...
export function 字符串池随机取出字符串(this: void, pool: number): string {
  const data = 取字符串池(pool);
  if (data == null) return "";
