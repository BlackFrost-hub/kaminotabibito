...
const { ceil } = require("lib.扩展函数.封装函数.01．通用工具.07．数学运算");
 ...
 */
export function createDelayedCall(delaySec, callback) {
    const delayMs = delaySec <= 0 ? 0 : ceil(delaySec * 1000);
    return { id: addDelayedCallback(delayMs, callback) };
}
export function cancelDelayedCall(handle) {
    if (handle == null)
        return;
 ...
 */
export function runTimerOnce(timer, delaySec, callback) {
    if (!timer) {
        callback();
        return;
    }
