...
const { X_GDBC, X_GAFC, X_IsTerrainWalkable, X_GetAbleX, X_GetAbleY } = require("lib.扩展函数.Star扩展函数.Star扩展库.06．X库函数");
...
function hid(h) {
    return jass.GetHandleId(h) || 0;
}
function getUnitX(u) {
    return jass.GetUnitX(u) || 0;
}
function getUnitY(u) {
    return jass.GetUnitY(u) || 0;
}
function getUnitFacing(u) {
    return jass.GetUnitFacing(u) || 0;
}
...
let isRunning = false;
const triggerUnitSpeedEntryUidByTriggerHid = {};
const speedBreakTempTimerCtxByHid = {};
function onSpeedBreakTempTimerExpire() {
 ...
 */
function syncEntryOrderDestination(e) {
    const tgtU = jass.GetOrderTargetUnit();
    if (tgtU != null && tgtU !== 0) {
        e.tx = getUnitX(tgtU);
        e.ty = getUnitY(tgtU);
    ...
    if (tgtIt != null && tgtIt !== 0) {
        e.tx = jass.GetItemX(tgtIt) || 0;
        e.ty = jass.GetItemY(tgtIt) || 0;
        return;
    }
    e.tx = jass.GetOrderPointX() || 0;
    e.ty = jass.GetOrderPointY() || 0;
}
function onSpeedEntryOrderTargetChanged() {
...
function createTriggerForEntry(entry) {
    ...
    const evTarget = jass.EVENT_UNIT_ISSUED_TARGET_ORDER;
    if (evTarget != null) {
 ...
 */
export function SOS_SetUnitSpeed(u, speed) {
    if (u == null || u === 0)
        return;
 ...
 */
export function SOS_SetUnitSpeedTemp(u, speed, duration) {
    if (u == null || u === 0)
        return;
    if (duration <= 0) {
 ...
 * @returns 移动速度
 */
export function SOS_GetUnitSpeed(u) {
    if (u == null || u === 0)
        return 0;
    const uid = hid(u);
    const entry = entryMap[uid];
 ...
 * @param u 目标单位
 */
export function SOS_UnSetUnitSpeed(u) {
    if (u == null || u === 0)
        return;
    const uid = hid(u);
    removeEntry(uid);
