...
// ============================================================================
/** 方位：正前方（0° ± 30°） */
export const 方位_正前方 = 1;
/** 方位：正后方（180° ± 30°） */
export const 方位_正后方 = 2;
/** 方位：侧面（非前后方） */
export const 方位_侧面 = 3;
/** 方位：左前方（45° ± 45°） */
export const 方位_左前方 = 4;
/** 方位：左后方（135° ± 45°） */
export const 方位_左后方 = 5;
// ============================================================================
// 核心判断函数
// ============================================================================
/**
 ...
 * @returns 是否在指定角度范围内
 */
export function 是否在指定角度范围内(unit, target, angleRange) {
    if (!SUC_IsValidUnit(unit) || !SUC_IsValidUnit(target))
        return false;
 ...
 */
export function 是否在前方角度内(unit, target, angleRange = 45) {
    if (!SUC_IsValidUnit(unit) || !SUC_IsValidUnit(target))
        return false;
    const x1 = jass.GetUnitX(unit);
    const y1 = jass.GetUnitY(unit);
 ...
 */
export function 是否在后方角度内(unit, target, angleRange = 45) {
    if (!SUC_IsValidUnit(unit) || !SUC_IsValidUnit(target))
        return false;
    const x1 = jass.GetUnitX(unit);
    const y1 = jass.GetUnitY(unit);
 ...
 * 判断目标是否在单位正前方（0° ± 30°）
 */
export function 是否在正前方(unit, target) {
    return 是否在前方角度内(unit, target, 30);
}
/**
 * 判断目标是否在单位正后方（180° ± 30°）
 */
export function 是否在正后方(unit, target) {
    return 是否在后方角度内(unit, target, 30);
}
/**
 * 判断目标是否在单位左侧（0° - 90°）
 */
export function 是否在左侧(unit, target) {
    if (!SUC_IsValidUnit(unit) || !SUC_IsValidUnit(target))
        return false;
    const x1 = jass.GetUnitX(unit);
    const y1 = jass.GetUnitY(unit);
 ...
 * 判断目标是否在单位右侧（0° - -180°）
 */
export function 是否在右侧(unit, target) {
    if (!SUC_IsValidUnit(unit) || !SUC_IsValidUnit(target))
        return false;
    const x1 = jass.GetUnitX(unit);
    const y1 = jass.GetUnitY(unit);
 ...
 */
export function 是否在前方(unit, target) {
    return 是否在前方角度内(unit, target, 90);
}
/**
 * 判断目标是否在单位后方（余弦 < 0，即 >90° 范围内）
 */
export function 是否在后方(unit, target) {
 ...
 */
export function 获取方位区间(unit, target) {
    if (!SUC_IsValidUnit(unit) || !SUC_IsValidUnit(target))
        return 3;
    const x1 = jass.GetUnitX(unit);
    const y1 = jass.GetUnitY(unit);
    ...
    const c = CosBJ(angle);
    if (c >= 0.866025)
        return 方位_正前方; // cos(30°)
    if (c >= 0.707106)
        return 方位_左前方; // cos(45°)
    if (c <= -0.866025)
        return 方位_正后方; // cos(150°)
    if (c <= -0.707106)
        return 方位_左后方; // cos(135°)
    return 方位_侧面;
}
// ============================================================================
// 旧函数兼容导出（别名）
...
export const SU_DotBehindUnit = (fac, x, y, a, b) => {
    const angle = 计算两点角度(x, y, a, b) - fac;
    return CosBJ(angle) <= -0.707106;
};
/** @deprecated 使用 获取方位区间() 代替 */
export const SU_GetUnitOfUnit = (u, tu) => 获取方位区间(u, tu);
/** @deprecated 使用 是否在前方() 代替 */
export const SU_IsUnitInfrontUnit2 = (u, tu) => 是否在前方(u, tu);
/** @deprecated 使用 是否在正前方() 代替 */
export const SU_IsUnitInfrontUnit = (u, tu) => 是否在正前方(u, tu);
/** @deprecated 使用 是否在正后方() 代替 */
export const SU_IsUnitBehindUnit = (u, tu) => 是否在正后方(u, tu);
