...
// 保留旧接口名的无敌判断。
export function SU_IsUnitInvincible(u) {
    return SUC_IsUnitInvincible(u);
}
// 通过乌鸦形态调整飞行高度。
export function SU_SetUnitFlyHeight(whichUnit, newHeight, rate) {
    if (!SUC_IsValidUnit(whichUnit))
        return;
...
// 读取英雄三围总和。
export function SU_GetHeroAllState(u, b) {
...
// 读取单位损失生命百分比。
export function SU_GetUnitLostHPPercent(u) {
    if (!SUC_IsValidUnit(u))
        return 0;
...
// 读取单位损失生命值。
export function SU_GetUnitLostHP(u) {
    if (!SUC_IsValidUnit(u))
        return 0;
    const maxLife = jass.GetUnitState(u, jass.UNIT_STATE_MAX_LIFE);
    const life = jass.GetUnitState(u, jass.UNIT_STATE_LIFE);
...
// 按数值或百分比增减生命上限。
export function UnitAddHp(u, value, b) {
    if (!SUC_IsValidUnit(u))
        return;
    const life = jass.GetUnitState(u, jass.UNIT_STATE_LIFE);
    const maxLife = jass.GetUnitState(u, jass.UNIT_STATE_MAX_LIFE);
...
// 保留旧语义的存活判断。
export function SU_IsUnitDie(u) {
    return SUC_IsUnitAlive(u);
}
// 用透明度和高度控制显隐。
export function SU_ShowOrHideUnit(u, isShow) {
    if (!SUC_IsValidUnit(u))
        return;
    if (isShow) {
...
// 判断单位是否为水元素。
export function IsWaterElement(u) {
    if (!SUC_IsValidUnit(u))
        return false;
...
// 识别单位的生命周期类型。
export function GetUnitTimedLifeID(u) {
    if (!SUC_IsValidUnit(u))
        return TIMED_LIFE_NONE;
    if (jass.GetUnitAbilityLevel(u, 0x4255616e) !== 0)
        return TIMED_LIFE_RAISE_DEAD;
    if (jass.GetUnitAbilityLevel(u, 0x4261706c) !== 0)
        return TIMED_LIFE_DISEASE_CLOUD;
    if (jass.GetUnitAbilityLevel(u, 0x42456666) !== 0)
        return TIMED_LIFE_FORCE_OF_NATURE;
    if (jass.GetUnitAbilityLevel(u, 0x42687764) !== 0)
        return TIMED_LIFE_HEALING_WARD;
    if (jass.GetUnitAbilityLevel(u, 0x42726169) !== 0)
        return TIMED_LIFE_ANIMATE_DEAD;
    if (jass.GetUnitAbilityLevel(u, 0x42487765) !== 0)
        return TIMED_LIFE_WATER_ELEMENTAL;
    if (jass.GetUnitAbilityLevel(u, 0x42544c46) !== 0)
        return TIMED_LIFE_TIMED;
    return TIMED_LIFE_NONE;
}
// GUI 用的整数透传封装。
export function I2TimedLifeID(i) {
    return i;
}
