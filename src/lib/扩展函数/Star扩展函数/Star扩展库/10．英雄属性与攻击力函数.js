...
function getUnitPrimaryTypeFromSlk(u) {
...
function GAFC(x1, y1, x2, y2) {
    return jass.Atan2(y2 - y1, x2 - x1) / BJ_DEGTORAD;
}
function getHeroPrimaryGreenValue(u) {
...
// 读取单位模型路径。
export function SU_GetUnitModel(u) {
    if (!SUC_IsValidUnit(u))
        return "";
    const unitId = jass.GetUnitTypeId(u);
    let file = "";
...
// 读取英雄主属性类型。
export function SU_GetHeroParmary(u) {
    return getUnitPrimaryTypeFromSlk(u);
}
// 增加或设置指定英雄属性。
export function SU_AddHeroState(u, id, typ, value) {
    if (!SUC_IsValidUnit(u))
        return;
    const isAdd = typ === 0;
    if (id === PRIMARY_STR) {
...
// 读取主属性的当前值。
export function SU_GetHeroParmaryValue(u) {
    if (!SUC_IsValidUnit(u))
        return -1;
...
// 判断是否匹配指定主属性。
export function SU_HeroISParmary(u, i) {
    return SU_GetHeroParmary(u) === i;
}
// 计算单位白字攻击力。
export function SU_GetUnitWhiteAtk(uOrA, aMaybe) {
    let u = uOrA;
    let a = aMaybe;
