...
function 设置底层暂停状态(u, 是否暂停) {
    if (u == null || u === 0)
        return;
    if (japi != null && typeof japi.EXPauseUnit === "function") {
        const EXPauseUnit = japi.EXPauseUnit;
        EXPauseUnit(u, 是否暂停);
        return;
    }
    PauseUnit(u, 是否暂停);
}
function 生成暂停来源键(单位ID, 来源) {
    return `${单位ID}:${来源}`;
}
export function 申请单位暂停占用(u, 来源) {
    if (u == null || u === 0 || 来源 == null || 来源 === "")
        return false;
    const 单位ID = hid(u);
    if (单位ID === 0)
        return false;
...
export function 单位是否存在其他暂停占用(u, 自身来源) {
    if (u == null || u === 0)
        return false;
    const 单位ID = hid(u);
    if (单位ID === 0)
...
function onHardStraightTimerExpire() {
 ...
 * @param time 暂停时间（秒）
 */
export function GS_Suspend(u, time) {
    if (u == null || u === 0)
        return;
    const uid = hid(u);
    let T = jass.LoadTimerHandle(HS_S, uid, 1);
 ...
 * @returns 是否正在暂停中
 */
export function GS_IsUnitSuspending(u) {
    if (u == null || u === 0)
        return false;
 ...
 * @returns 剩余暂停时间（秒）
 */
export function GS_LoadSuspend(u) {
    if (u == null || u === 0)
        return 0;
    const T = jass.LoadTimerHandle(HS_S, hid(u), 1);
    if (T == null)
 ...
 * @param r 时间值（秒）
 */
export function GS_UnitSuspend(u, i, r) {
    if (u == null || u === 0)
        return;
