...
--- 实数符号 - RSignBJ（返回 ±1）
function ____exports.RSignBJ(a)
...
function ____exports.AngleBetweenPoints(locA, locB)
...
function ____exports.DistanceBetweenPoints(locA, locB)
    if locA == nil or locB == nil then
        return 0
    end
    local dx = jass.GetLocationX(locB) - jass.GetLocationX(locA)
...
--- 整数最小值 - IMinBJ
function ____exports.IMinBJ(a, b)
    return a <= b and a or b
end
--- 实数最大值 - RMaxBJ
function ____exports.RMaxBJ(a, b)
    return a < b and b or a
end
--- 实数最小值 - RMinBJ
function ____exports.RMinBJ(a, b)
    return a < b and a or b
end
--- 百分比转整数 - PercentToInt
...
function ____exports.PercentTo255(percentage)
    return ____exports.PercentToInt(percentage, 255)
end
return ____exports
