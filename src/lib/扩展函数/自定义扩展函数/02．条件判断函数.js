...
function isInvincibleUnit(unit) {
    if (!unit)
        return false;
    return jass.IsUnitInvulnerable(unit);
}
function isAncientUnit(unit) {
    if (!unit)
        return false;
 ...
 */
export function isUnitEnemy(targetUnit, sourceUnit) {
 ...
 */
export function isSameUnit(unitA, unitB) {
    if (!unitA || !unitB)
        return false;
    return unitA === unitB;
}
 ...
 */
export function isUnitAlly(targetUnit, sourceUnit) {
    if (!targetUnit || !sourceUnit)
        return false;
 ...
 */
export function isValidAllyUnit(targetUnit, sourceUnit) {
 ...
 */
export function isValidAllyUnitExcludeSelf(targetUnit, sourceUnit) {
 ...
 */
export function isNotUsingInventoryItem(unit) {
    if (!unit)
        return true;
 ...
 * @param targetUnit 目标单位
 */
export function isSingleTargetSpell(targetUnit) {
    return targetUnit != null && targetUnit !== 0;
}
export function matchUnitFilter(targetUnit, sourceUnit, options) {
    if (!targetUnit)
        return false;
...
export function createUnitFilter(options) {
    return (targetUnit, sourceUnit) => matchUnitFilter(targetUnit, sourceUnit, options);
}
