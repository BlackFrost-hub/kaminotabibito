...
export function getSKey_PIndex() {
    const jg = jglobals;
...
export function getSKey_Trigger() {
    const jg = jglobals;
...
function setG_SIndex(v) {
    if (jglobals)
        jglobals.G_SIndex = v;
    globalThis.G_SIndex = v;
}
function getG_LIndex() {
    const idx = sym("G_LIndex");
    return typeof idx === "number" ? idx : 0;
}
function setG_LIndex(v) {
    if (jglobals)
        jglobals.G_LIndex = v;
    globalThis.G_LIndex = v;
}
function sh(s) {
 ...
 */
export function YDLocalInitialize() {
 ...
 */
export function YDLocal1Release() {
    const YDLOC = ydlocHandle();
 ...
 */
export function YDLocal5Set(type, name, value) {
    const h = ydlocHandle();
    if (!h)
        return;
    const p = globalThis.ydl_triggerstep ?? 0;
    const c = sh(name);
    saveByHash(type, h, p, c, value);
}
/**
 ...
 */
export function YDLocal5Get(type, name) {
    const h = ydlocHandle();
    if (!h)
        return defaultForType(type);
    const p = globalThis.ydl_triggerstep ?? 0;
 ...
 */
export function flushYDLocal5ParamPage() {
    const h = ydlocHandle();
    if (!h)
        return;
    const p = globalThis.ydl_triggerstep ?? 0;
    if (typeof p !== "number" || p === 0 || p !== p)
        return;
 ...
 */
export function YDLocal7Set(type, name, value) {
    const h = ydlocHandle();
    if (!h)
        return;
 ...
 */
export function YDLocal7Get(type, name) {
    const h = ydlocHandle();
    if (!h)
        return defaultForType(type);
    const p = loadStar_PIndex();
 ...
 */
export function getParentYdlocPageForReturnValue(_self) {
 ...
 */
export function clearStar_PIndex() {
    const YDHT = ydhtHandle();
    if (!YDHT)
        return;
    const trig = jass.GetTriggeringTrigger();
