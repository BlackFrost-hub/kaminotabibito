...
const { ceil } = require("lib.扩展函数.封装函数.01．通用工具.index");
...
// ==========================================================================================
/** 待销毁特效列表 */
const _pendingEffects = [];
const { onTick10ms } = require("系统.00．核心系统.05．中心计时器");
/** 是否已注册到中心计时器 */
let _effectRecycleRegistered = false;
/** 每10毫秒检查待销毁特效 */
const _tickEffectRecycle = () => {
    globalThis.__ydweTickCount = (globalThis.__ydweTickCount || 0) + 1;
    ...
    if (oldLen !== newLen) {
        globalThis.__ydweDestroyed = (globalThis.__ydweDestroyed || 0) + (oldLen - newLen);
    }
    globalThis.__ydweEffectDebug = newLen;
};
/**
 ...
 * @param effect 特效句柄
 */
export function YDWETimerDestroyEffect(duration, effect) {
    if (!effect)
        return;
    if (duration <= 0) {
    ...
    if (!_effectRecycleRegistered) {
        ...
        globalThis.__ydweRegistered = true;
    }
    // 计算tick数（每10毫秒一个tick）
    const ticks = ceil(duration / 0.01);
    _pendingEffects.push({ eff: effect, ticksLeft: ticks });
    globalThis.__ydweEffectDebug = _pendingEffects.length;
}
