...
function ensureMoveSmartOrderIds() {
    if (cachedOrderMove !== 0)
        return;
    const m = jass.OrderId("move");
    const s = jass.OrderId("smart");
...
function faceUnitTowardGroundItem(u, item) {
    if (u === null || u === 0 || item === null || item === 0)
        return;
    const angleDeg = jass.Atan2(jass.GetItemY(item) - jass.GetUnitY(u), jass.GetItemX(item) - jass.GetUnitX(u)) * BJ_RADTODEG;
...
function tryMergeGroundItemIntoInventory(wp, u) {
    for (let i = 0; i < 6; i++) {
...
function CheakPickUpForTrigger(triggeringTrigger, fromTimer) {
    ...
    if (triggeringTrigger === null)
        return false;
    const tid = jass.GetHandleId(triggeringTrigger);
    if (tempTrigUnregisters[tid] === undefined)
        return false;
...
function StarItem_UnitOrderCond() {
    return isHeroTriggerUnit() && StarItem_ItemStack_Cond();
}
function StarItem_ItemPickUpCond() {
...
function ensureStackEventTriggers() {
    if (StackRegd)
        return;
    StackRegd = true;
    ...
    playerUnitEvent.registerPlayerUnitEventForPlayerIds(StarTrig_UnitOrder, STACK_EVENT_PLAYER_IDS, jass.EVENT_PLAYER_UNIT_ISSUED_TARGET_ORDER, heroFilter);
    playerUnitEvent.registerPlayerUnitEventForPlayerIds(StarTrig_ItemPickUP, STACK_EVENT_PLAYER_IDS, jass.EVENT_PLAYER_UNIT_PICKUP_ITEM, heroFilter);
}
...
export function StarItem_ItemStack_Act() {
    const wp = jass.GetOrderTargetItem();
    const u = jass.GetTriggerUnit();
    if (wp === null || u === null || !tryMergeGroundItemIntoInventory(wp, u))
        return;
    StackStatus = false;
    suppressPendingMoveAfterGroundStack(u);
    StackStatus = true;
