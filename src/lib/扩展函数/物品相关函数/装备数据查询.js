...
export function findStatKey(raw) {
...
export function getItemDataEntry(item) {
    if (item == null || item === 0)
        return null;
...
export function getItemDataEntryByIdStr(idStr) {
    if (!idStr)
        return null;
    const entry = itemsData[idStr];
    if (!entry)
...
export function getItemDataEntryByTypeId(itemTypeId) {
