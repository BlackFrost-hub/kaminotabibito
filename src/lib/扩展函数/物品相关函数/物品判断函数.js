...
export function UnitHasItemOfTypeBJ(whichUnit, itemTypeId) {
    if (!whichUnit || typeof jass.UnitItemInSlot !== "function" || typeof jass.GetItemTypeId !== "function")
        return false;
...
export function GetItemTypeTotalCountByChargesBJ(whichUnit, itemId) {
    ...
    for (let i = 0; i < 6; i++) {
        ...
        if (jass.GetItemTypeId(item) !== itemId)
            continue;
        const ch = typeof jass.GetItemCharges === "function" ? jass.GetItemCharges(item) : 0;
        total += ch > 0 ? ch : 1;
    }
    return total;
}
...
export function TryGiveItemToUnitBJ(targetUnit, item) {
    if (!targetUnit || !item || typeof jass.UnitAddItem !== "function")
        return false;
...
export function ReturnItemToHeroOrDropBJ(item, fromUnit, hero) {
    ...
    if (TryGiveItemToUnitBJ(hero, item))
        return "added";
    if (typeof jass.UnitRemoveItem !== "function" || typeof jass.SetItemPosition !== "function")
        return "failed";
...
export function UnitRemoveItemSwapped(whichItem, whichHero) {
    if (!whichItem || !whichHero || typeof jass.UnitRemoveItem !== "function")
        return;
    globalThis.bj_lastRemovedItem = whichItem;
