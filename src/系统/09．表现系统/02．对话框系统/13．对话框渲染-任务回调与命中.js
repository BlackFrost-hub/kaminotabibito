...
import { onDialogFinished, resetDialogActiveFlagsKeepOnFinish } from "./02．对话框业务逻辑";
import { resetActivePlayerIdIfMatch, setActivePlayerId } from "./10．对话框渲染-Dz与状态";
import { setDialogPanelHitBinder } from "./11．对话框渲染-创建帧";
import { DEFAULT_BODY_FONT_SIZE, DEFAULT_FONT, DEFAULT_TITLE_FONT_SIZE, dzGetLocalPlayer, dzGetPlayerId, dzPlayer, dzSetFont, dzSetText, dzSetTexture, dzShow, dzTimerPause, findFirstQuestEntryIndex, g_questCallbacksByPlayer, g_states, japi, KEY_SKIP_DIALOG, MAX_PLAYERS, } from "./10．对话框渲染-Dz与状态";
...
// ========== 虚拟分区：队列索引/当前页查找辅助 ==========
function getCurrentEntry(state) {
    return state.queue[state.currentIndex];
}
function findLastNormalEntryIndex(state) {
    for (let i = state.queue.length - 1; i >= 0; i--) {
        if (!state.queue[i].isQuest)
            return i;
    }
    return -1;
}
/** 统一渲染当前页（标题/正文/字体/立绘），不包含 quest buttons / continue hint（由调用方决定） */
function renderCurrentEntry(state, revealFullText) {
...
function finishDialogAndCleanup(state) {
...
// ========== 虚拟分区：任务接受/拒绝按钮回调调度 ==========
function resolveQuestCallbackByPlayerId(playerId) {
...
function runQuestAcceptForPlayer(playerId) {
...
function runQuestRejectForPlayer(playerId) {
    const ctx = resolveQuestCallbackByPlayerId(playerId);
    if (!ctx)
        return;
...
function questAcceptCallbackP0() { runQuestAcceptForPlayer(0); }
function questAcceptCallbackP1() { runQuestAcceptForPlayer(1); }
function questAcceptCallbackP2() { runQuestAcceptForPlayer(2); }
function questAcceptCallbackP3() { runQuestAcceptForPlayer(3); }
function questRejectCallbackP0() { runQuestRejectForPlayer(0); }
function questRejectCallbackP1() { runQuestRejectForPlayer(1); }
function questRejectCallbackP2() { runQuestRejectForPlayer(2); }
function questRejectCallbackP3() { runQuestRejectForPlayer(3); }
globalThis.QuestAcceptCallbackP0 = questAcceptCallbackP0;
globalThis.QuestAcceptCallbackP1 = questAcceptCallbackP1;
globalThis.QuestAcceptCallbackP2 = questAcceptCallbackP2;
globalThis.QuestAcceptCallbackP3 = questAcceptCallbackP3;
globalThis.QuestRejectCallbackP0 = questRejectCallbackP0;
globalThis.QuestRejectCallbackP1 = questRejectCallbackP1;
globalThis.QuestRejectCallbackP2 = questRejectCallbackP2;
globalThis.QuestRejectCallbackP3 = questRejectCallbackP3;
// ========== 虚拟分区：面板点击交互处理 ==========
function handleDialogPanelClick(state) {
    // 情况 A：打字中 → 只补全
...
function runDialogPanelHitForPlayer(playerId) {
    if (playerId < 0 || playerId >= MAX_PLAYERS)
        return;
    const state = g_states[playerId];
    if (!state)
...
function dialogPanelHitCallbackP0() { runDialogPanelHitForPlayer(0); }
function dialogPanelHitCallbackP1() { runDialogPanelHitForPlayer(1); }
function dialogPanelHitCallbackP2() { runDialogPanelHitForPlayer(2); }
function dialogPanelHitCallbackP3() { runDialogPanelHitForPlayer(3); }
globalThis.DialogPanelHitCallbackP0 = dialogPanelHitCallbackP0;
globalThis.DialogPanelHitCallbackP1 = dialogPanelHitCallbackP1;
globalThis.DialogPanelHitCallbackP2 = dialogPanelHitCallbackP2;
globalThis.DialogPanelHitCallbackP3 = dialogPanelHitCallbackP3;
function showContinueHintLocal(state, visible) {
    const localPlayer = dzGetLocalPlayer();
    const targetPlayer = dzPlayer(state.playerId);
    if (localPlayer !== targetPlayer)
        return;
...
export function bindDialogPanelHitFrame(_hitFrame) {
    return;
}
// ========== 虚拟分区：~ 键跳过冷却 timer ==========
const SKIP_KEY_COOLDOWN_SECONDS = 0.12;
const g_skipKeyCooldown = [];
const g_skipKeyCooldownTimers = [];
function finishSkipKeyCooldownForPlayer(pid) {
...
function skipKeyCooldownCallbackP0() { finishSkipKeyCooldownForPlayer(0); }
function skipKeyCooldownCallbackP1() { finishSkipKeyCooldownForPlayer(1); }
function skipKeyCooldownCallbackP2() { finishSkipKeyCooldownForPlayer(2); }
function skipKeyCooldownCallbackP3() { finishSkipKeyCooldownForPlayer(3); }
function startSkipKeyCooldown(pid) {
    if (pid < 0 || pid >= MAX_PLAYERS)
        return;
...
function skipDialogLocal() {
    const triggerPlayer = japi.DzGetTriggerKeyPlayer();
    if (!triggerPlayer)
        return;
    const triggerPid = dzGetPlayerId(triggerPlayer);
    if (triggerPid == null || triggerPid < 0 || triggerPid >= MAX_PLAYERS)
        return;
    if (g_skipKeyCooldown[triggerPid])
        return;
    ...
    if (state.strNow < state.strLen) {
        ...
        if (currentEntry !== undefined)
            dzSetText(state.frames[3], currentEntry.text);
    }
...
export function initSkipKeyListener() {
    if (g_skipKeyInitialized)
        return;
...
export function bindQuestSyncHandlersImpl(state) {
