...
export function ensureQuestConfigsRegistered() {
    const g = globalThis;
    if (g.__questConfigsRegistered)
        return;
...
export function setQuestState(playerId, questId, state, playerName) {
    if (state === 1) {
        ...
        if (playerName) {
            ...
            const globalData = questDB.globalData;
    ...
    if (state === 2) {
        ...
        if (playerName) {
            ...
            if (def) {
                ...
                if (savedAccepterName)
                    def.accepterName = savedAccepterName;
            }
        }
    }
}
export function hasPlayerAcceptedQuest(playerId, questId) {
    return getQuestState(playerId, questId) === 1;
}
export function hasPlayerCompletedQuest(playerId, questId) {
...
export function findEnabledNpcConfigBySelectedUnit(unit, unitName) {
    if (!unit || !unitName)
        return null;
    const selectedUnitCode = fourCCToString(GetUnitTypeId(unit));
    for (const npc of NPC_CONFIGS) {
        if (npc.enabled !== true)
            continue;
        if (npc.unitcode && npc.unitcode !== selectedUnitCode)
            continue;
