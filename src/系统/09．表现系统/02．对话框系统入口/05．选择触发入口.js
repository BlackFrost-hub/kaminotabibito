...
import { buildDialogData, buildQuestCompletedDialog, buildQuestInProgressDialog, buildQuestOfferDialog, } from "./04．对话构建";
const { openNpcDialog } = UI函数;
// ========== 虚拟分区：初始化 ==========
export function initDialogEntrySelectionTrigger() {
    ...
    jass.TriggerAddAction(trg, () => {
        ...
        if (!hero)
            return;
        if (!jass.IsUnitInRange(hero, u, 350))
            return;
