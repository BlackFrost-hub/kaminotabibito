...
const 封装函数 = require("系统.00．核心系统.01．封装函数");
import { getItemName } from "../../../lib/扩展函数/YDWE函数/00．YDWE函数";
import { ConsumeItemTypeCountByChargesBJ, GetItemTypeTotalCountByChargesBJ, ReturnItemToHeroOrDropBJ, UnitGetItemByTypeId, UnitHasItemOfTypeBJ } from "../../../lib/扩展函数/物品相关函数/物品判断函数";
...
function tryConsumeRequiredResources(player, requiredResources, requireCount) {
    if (!requiredResources || requiredResources === "")
        return true;
    const cost = normalizeRequireCount(requireCount);
    if (typeof jass.GetPlayerState !== "function" || typeof jass.SetPlayerState !== "function")
        return false;
...
function isKillQuestObjectiveCompleted(playerId, questId, requireCount) {
    ...
    for (const q of active) {
        ...
        if (required <= 0)
            required = requireCount > 0 ? requireCount : 1;
        return current >= required;
    }
    return false;
}
function parseAllowedEquipLevels(requireItem) {
    const out = new Set();
    const marker = "装备等级:";
...
function resolveSubmitItem(hero, requireItem) {
    if (!hero || !requireItem)
    ...
    if (requireItem.indexOf("装备等级:") === 0) {
        for (let slot = 0; slot < 6; slot++) {
            ...
            if (!item || typeof jass.GetItemTypeId !== "function")
                continue;
            const itemId = jass.GetItemTypeId(item);
            const itemCode = 封装函数.fourCCToString(itemId);
            const data = itemsData[itemCode];
            if (!data)
                continue;
            if ((data.type || "") !== "道具/戒指/饰品")
                continue;
    ...
    if (requireItem.length === 4) {
        ...
        const data = itemsData[requireItem];
        return { itemId, itemCode: requireItem, itemLevel: data?.level || "" };
    }
    if (requireItem.indexOf("|") >= 0) {
        ...
        for (const code of parts) {
            ...
            if (UnitHasItemOfTypeBJ(hero, testId)) {
                const data = itemsData[c];
                return { itemId: testId, itemCode: c, itemLevel: data?.level || "" };
            }
        }
    }
    return { itemId: 0, itemCode: "", itemLevel: "" };
}
function isSubmitItemMatchedRequire(submitInfo, requireItem) {
    if (!requireItem || submitInfo.itemId === 0)
        return false;
    if (requireItem.indexOf("装备等级:") === 0) {
...
function pickNpcCompleteTextByBranch(raw, branchIndex) {
    if (!raw || raw === "")
        return raw;
    if (branchIndex < 0)
        return raw;
    const lines = raw.split("\n").map(s => s.trim()).filter(s => s !== "");
    if (branchIndex >= lines.length)
        return raw;
    return lines[branchIndex];
}
function shouldUseGenericGiveFailHint(quest) {
    if (quest.type !== "给予")
        return false;
...
export function handleQuestSubmit(params) {
    ...
    function broadcastQuestComplete() {
    ...
    if (requireItem) {
        ...
        if (itemCount >= requireCount) {
            ...
            showLocalHint(dialogOwnerId, `|cffffff00『系统提示』：|r你只有 |cffff9900${itemCount}|r 个 |cffffcc00${itemDisplayName}|r，还需要 |cffff4444${requireCount - itemCount}|r 个`);
        }
        return;
    }
    setQuestState(questId, 2, playerName);
