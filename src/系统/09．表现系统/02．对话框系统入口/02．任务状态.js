...
export function setQuestState(questId, state, playerName) {
    if (state === 1) {
        ...
        if (playerName) {
            ...
            const active = questDB.globalData?.quests.get(questId);
            if (active)
...
export function hasPlayerAcceptedQuest(_playerId, questId) {
    return getQuestState(questId) === 1;
}
export function hasPlayerCompletedQuest(_playerId, questId) {
