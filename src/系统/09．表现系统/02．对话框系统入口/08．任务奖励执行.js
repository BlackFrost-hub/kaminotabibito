...
import { bindRewardParseHeroResolver, isConditionMatchedWithContext, } from "./06．任务奖励解析";
function getUserPlayers() {
    const out = [];
    for (let i = 0; i < 4; i++) {
        const p = jass.Player(i);
        if (p && jass.GetPlayerController(p) === jass.MAP_CONTROL_USER)
            out.push(p);
    }
    return out;
}
export function getPlayerFirstHero(player) {
    if (!player || typeof jass.CreateGroup !== "function")
        return null;
...
function gainGold(players, value) {
...
function gainLumber(players, value) {
    for (const p of players) {
...
function gainExp(players, value) {
    for (const p of players) {
...
function gainLevel(players, value) {
    for (const p of players) {
        const hero = getPlayerFirstHero(p);
...
function gainHeroStat(players, statName, value) {
    for (const p of players) {
        const hero = getPlayerFirstHero(p);
        if (!hero)
            continue;
...
function resolveAmountExpr(expr, triggerPlayerId) {
    ...
    if (text.indexOf("IMaxBJ(") === 0) {
        ...
        const level = hero && typeof jass.GetHeroLevel === "function" ? jass.GetHeroLevel(hero) : 1;
        const a = 20000 - (level - 20) * 1000;
        return IMaxBJ(a, 10000);
    }
    return readFirstNumber(text);
}
function executeOneRewardExpr(expr, triggerPlayerId) {
    const text = expr.trim();
    if (text === "" || text === "null")
        return;
    const allPlayers = getUserPlayers();
    const targetPlayers = text.indexOf("完成任务的玩家") >= 0 || text.indexOf("Player") >= 0
    ...
    for (const p of prefixes) {
        if (payload.indexOf(p) === 0) {
            ...
            while (payload.charAt(0) === "+" || payload.charAt(0) === "＋")
    ...
    if (payload.indexOf("经验") >= 0 || payload.indexOf("exp") >= 0) {
        ...
        if (value > 0)
            gainExp(targetPlayers, value);
        return;
    }
    if (payload.indexOf("金币") >= 0 || payload.indexOf("gold") >= 0) {
        const value = resolveAmountExpr(payload, triggerPlayerId);
        if (value > 0)
            gainGold(targetPlayers, value);
        return;
    }
    if (payload.indexOf("能量碎片") >= 0 || payload.indexOf("木头") >= 0 || payload.indexOf("木材") >= 0 || payload.indexOf("wood") >= 0) {
        const value = resolveAmountExpr(payload, triggerPlayerId);
        if (value > 0)
            gainLumber(targetPlayers, value);
        return;
    }
    if (payload.indexOf("等级") >= 0 || payload.indexOf("level") >= 0) {
        const value = resolveAmountExpr(payload, triggerPlayerId);
        if (value > 0)
            gainLevel(targetPlayers, value);
        return;
    }
    if (payload.indexOf("力量") >= 0) {
        const value = resolveAmountExpr(payload, triggerPlayerId);
        if (value > 0)
            gainHeroStat(targetPlayers, "力量", value);
        return;
    }
    if (payload.indexOf("敏捷") >= 0) {
        const value = resolveAmountExpr(payload, triggerPlayerId);
        if (value > 0)
            gainHeroStat(targetPlayers, "敏捷", value);
        return;
    }
    if (payload.indexOf("智力") >= 0) {
        const value = resolveAmountExpr(payload, triggerPlayerId);
        if (value > 0)
            gainHeroStat(targetPlayers, "智力", value);
    }
}
export function applyRewardWithContext(rewardRaw, ctx) {
    if (!rewardRaw || rewardRaw === "")
    ...
    for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
        ...
        if (colon > 0) {
            ...
            for (const p of parts)
                executeOneRewardExpr(p, ctx.triggerPlayerId);
...
export function previewRewardMatchWithContext(rewardRaw, ctx) {
    if (!rewardRaw || rewardRaw === "")
        return { matchedRuleIndex: -1, matchedCondition: "" };
    const lines = rewardRaw.split("\n");
    for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
...
export function giveRewardToPlayers(rewardRaw, triggerPlayerId) {
