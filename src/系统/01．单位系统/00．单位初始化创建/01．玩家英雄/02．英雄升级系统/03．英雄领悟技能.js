...
function 显示领悟提示(whichHero, abilityId) {
    const owner = jass.GetOwningPlayer(whichHero);
    if (!owner || owner === 0)
        return;
    const heroName = jass.GetUnitName(whichHero);
    const abilityName = YDWEGetUnitAbilityDataString(whichHero, abilityId, 1, 215) || "未知技能";
    jass.DisplayTimedTextToPlayer(owner, 0, 0, 20, "|cffffff00【系统提示】：|r|cffffffcc【" + heroName + "】|r领悟了技能|cffff99cc【" + abilityName + "】|r");
}
export function 应用英雄领悟技能(whichHero) {
