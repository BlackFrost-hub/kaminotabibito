...
function 按总池上限重分配(list) {
    if (list == null || list.length === 0)
        return;
    if (list.length < 2)
        return;
    const 当前总仇恨 = 获取总仇恨值(list);
    if (当前总仇恨 <= 0 || 当前总仇恨 <= 仇恨上限)
        return;
...
function 触发当前目标清除(敌人ID, 目标ID) {
    if (_清除当前目标 != null)
        _清除当前目标(敌人ID, 目标ID);
}
function 移除条目并联动(list, 敌人ID, index) {
...
function 清理过期条目(list, 敌人ID, 当前时间) {
    if (list == null || list.length === 0)
        return;
    for (let i = list.length - 1; i >= 0; i--) {
...
function 清理列表并重分配(敌人ID, list, 当前时间) {
    if (list == null)
        return;
...
export function 注册当前目标清除回调(fn) {
    _清除当前目标 = fn;
}
/** 增加仇恨（累加），同时维护敌人引用表 */
export function addThreat(敌人, 仇恨目标, 数值) {
    const 敌人ID = 取单位ID(敌人);
...
/** 设置仇恨（覆盖），同时维护敌人引用表 */
export function setThreat(敌人, 仇恨目标, 数值) {
    const 敌人ID = 取单位ID(敌人);
    const 目标ID = 取单位ID(仇恨目标);
    if (敌人ID === 0 || 目标ID === 0)
        return;
...
export function getThreatByHid(敌人, 目标ID) {
    const 敌人ID = 取单位ID(敌人);
    if (敌人ID === 0 || 目标ID === 0)
        return 0;
    ...
    for (let i = 0; i < list.length; i++) {
        if (list[i].targetHid === 目标ID)
            return list[i].threat;
    }
    return 0;
}
export function getThreat(敌人, 仇恨目标) {
    const 敌人ID = 取单位ID(敌人);
    const 目标ID = 取单位ID(仇恨目标);
    if (敌人ID === 0 || 目标ID === 0)
        return 0;
...
export function getHighestThreat(敌人, filter) {
    const 敌人ID = 取单位ID(敌人);
    if (敌人ID === 0)
        return null;
    const list = threatTables[敌人ID];
...
export function removeTarget(敌人, 仇恨目标) {
    const 敌人ID = 取单位ID(敌人);
    const 目标ID = 取单位ID(仇恨目标);
    if (敌人ID === 0 || 目标ID === 0)
        return;
...
export function clearAllThreat(敌人) {
    const 敌人ID = 取单位ID(敌人);
    if (敌人ID === 0)
        return;
    delete threatTables[敌人ID];
...
export function clearAllThreatById(敌人ID) {
    if (敌人ID === 0)
        return;
    delete threatTables[敌人ID];
    delete enemyRefTable[敌人ID];
...
/** 该敌人是否有仇恨记录 */
export function isEnemyTracked(敌人) {
    const 敌人ID = 取单位ID(敌人);
    if (敌人ID === 0)
        return false;
...
/** 仇恨表中的目标数量 */
export function getEnemyThreatCount(敌人) {
    const 敌人ID = 取单位ID(敌人);
    if (敌人ID === 0)
        return 0;
    const list = threatTables[敌人ID];
...
/** 获取该敌人所有仇恨目标的只读快照 */
export function getEnemyThreats(敌人) {
    const 敌人ID = 取单位ID(敌人);
    if (敌人ID === 0)
        return [];
    const list = threatTables[敌人ID];
...
export function getAllTrackedEnemyIds() {
    return 获取有序敌人ID列表();
}
/** 按 ID 取敌人仇恨表是否还有记录（驱动层用） */
export function hasThreatTable(敌人ID) {
    return threatTables[敌人ID] != null;
}
/** 从敌人引用表获取敌人单位引用 */
export function getEnemyRef(敌人ID) {
    return enemyRefTable[敌人ID] ?? null;
}
/** 获取最近一次收到伤害/更新仇恨的服务器时间（毫秒） */
export function getEnemyLastThreatUpdateTimeById(敌人ID) {
    return enemyLastThreatUpdateMs[敌人ID] ?? 0;
}
export function 清理敌人过期仇恨条目ById(敌人ID) {
    if (敌人ID === 0)
        return;
    const list = threatTables[敌人ID];
    if (list == null)
