...
const { calcBaseLifeRegen, calcBaseManaRegen, calcTotalLifeRegen, calcTotalManaRegen, calcBossTotalLifeRegen, getPercentLifeRegen, getPercentManaRegen, } = require("系统.01．单位系统.02．恢复系统.01．恢复计算");
const { calcItemLifeRegenBonus } = require("系统.01．单位系统.02．恢复系统.02．装备恢复效果");
const { getUnitLifeRegenMultiplier } = require("系统.01．单位系统.02．恢复系统.03．单位恢复特性");
const { REGEN_THRESHOLD, PERCENT_REGEN_THRESHOLD, } = require("系统.01．单位系统.02．恢复系统.00．恢复常量");
//=============================================================================
// 一、单位恢复执行
//=============================================================================
/**
 * 执行单位生命恢复
 */
function applyLifeRegen(unit, regen) {
    if (regen <= 0)
        return;
 ...
 * 执行单位魔法恢复
 */
function applyManaRegen(unit, regen) {
    if (regen <= 0)
        return;
 ...
 * 与 JASS 源代码逻辑完全一致
 */
export function processPlayerHeroRegen(unit) {
 ...
 * 处理Boss单位恢复
 */
export function processBossRegen(unit) {
