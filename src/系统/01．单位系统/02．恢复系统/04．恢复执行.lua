...
--- 执行单位生命恢复
local function applyLifeRegen(unit, regen)
    if regen <= 0 then
        return
    end
...
--- 执行单位魔法恢复
local function applyManaRegen(unit, regen)
    if regen <= 0 then
        return
    end
...
--- 处理玩家英雄恢复
-- 与 JASS 源代码逻辑完全一致
function ____exports.processPlayerHeroRegen(unit)
...
--- 处理Boss单位恢复
function ____exports.processBossRegen(unit)
    local totalLifeRegen = calcBossTotalLifeRegen(unit)
    if totalLifeRegen > REGEN_THRESHOLD then
        applyLifeRegen(unit, totalLifeRegen)
    end
end
--- 获取玩家英雄组
local function getPlayerHeroGroup()
...
--- 每秒恢复处理主函数
function ____exports.onRegenTimer()
    local heroGroup = getPlayerHeroGroup()
    if heroGroup ~= nil then
        forEachUnitInGroup(
            ...
            function(____, unit)
                if unit ~= nil then
                    ____exports.processPlayerHeroRegen(unit)
                end
            end
        )
    end
    local bossGroup = getBossGroup()
    if bossGroup ~= nil then
        forEachUnitInGroup(
            ...
            function(____, unit)
                if unit ~= nil then
                    ____exports.processBossRegen(unit)
                end
            end
        )
    end
...
--- 注册恢复系统到中心计时器
local function registerToCenterTimer()
    if _registered then
        return
    end
    _registered = true
