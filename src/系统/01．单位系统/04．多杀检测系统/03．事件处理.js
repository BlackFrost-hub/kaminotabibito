 ...
 */
import { MULTI_KILL_SYSTEM_ENABLED, MULTI_KILL_EVENT, } from "./00．常量定义";
import { startMultiKillMonitor, } from "./01．核心功能";
...
const { ydlStes_syncTriggerStep } = require("lib.扩展函数.YDWE函数.05．STES子触发公共工具");
const { CountUnitsInGroup } = require("lib.扩展函数.BJ函数.02．单位与英雄");
 ...
 */
export function fireMultiKillEvent() {
    ...
    // 调试输出读取到的参数
    dbg(`参数读取结果:`);
    dbg(`  effectSource=${effectSource === 0 || effectSource == null ? "nil/0" : "有效"} → 使用=${resolvedEffectSource === 0 || resolvedEffectSource == null ? "nil/0" : "有效"}`);
    ...
    dbg(`  killGroup内单位数=${killGroup !== 0 && killGroup != null ? jass.CountUnitsInGroup(killGroup) : "N/A"}`);
...
// STES事件处理
// ==========================================================================================
let multiKillTrigger = null;
function onMultiKillEvent() {
    fireMultiKillEvent();
}
// ==========================================================================================
// 初始化
// ==========================================================================================
export function initMultiKillSystem() {
    if (!MULTI_KILL_SYSTEM_ENABLED)
        return;
    if (multiKillTrigger != null)
        return;
    multiKillTrigger = registerStesListener(MULTI_KILL_EVENT, onMultiKillEvent);
}
export function isMultiKillSystemInitialized() {
