...
import { MULTI_KILL_SYSTEM_ENABLED, } from "./00．常量定义";
const { YDUserDataGet, YDUserDataSet, YDUserDataClear, } = require("lib.扩展函数.YDWE函数.index");
...
const { GroupAddGroup } = require("lib.扩展函数.BJ函数.02．单位与英雄");
// ==========================================================================================
// 全局存储
// ==========================================================================================
let groupMonitors = [];
let groupUnitMap = new Map();
let finalDamageListenerRegistered = false;
let damageModifierRegistered = false;
// ==========================================================================================
// 导入成功回调
// ==========================================================================================
const { onMultiKillSuccess } = require("系统.01．单位系统.04．多杀检测系统.04．成功回调");
// ==========================================================================================
// 辅助函数
// ==========================================================================================
function getGameTime() {
    const { getServerTime } = globalThis;
    // 直接使用服务器时间（毫秒），转换为秒
...
function killAllInGroup(instance) {
...
function removeGroupMonitor(instance) {
    if (instance.killGroup != null) {
...
function 处理多杀致命计数(instance, targetUnit, damage) {
...
// 核心逻辑：伤害处理
// ==========================================================================================
function onUnitDamage(targetUnit, sourceUnit, damage, _snapshot) {
    const instance = groupUnitMap.get(getUnitId(targetUnit));
    if (instance == null)
        return;
    ...
    if (!(damage > 0))
        return;
    if (instance.pendingFinish !== true)
        return;
    if (instance.pendingTarget !== targetUnit)
        return;
...
// API函数
// ==========================================================================================
export function startMultiKillMonitor(config) {
    if (!MULTI_KILL_SYSTEM_ENABLED)
        return;
    ...
    const existingIdx = groupMonitors.findIndex((m) => m.effectSource === config.effectSource);
    if (existingIdx >= 0) {
...
export function stopMultiKillMonitor(effectSource) {
    const instance = groupMonitors.find((m) => m.effectSource === effectSource);
    if (instance != null) {
        removeGroupMonitor(instance);
    }
}
export function addToKillGroup(effectSource, unit) {
    const instance = groupMonitors.find((m) => m.effectSource === effectSource);
    if (instance == null)
        return;
    if (instance.killGroup == null) {
...
export function removeFromKillGroup(effectSource, unit) {
    const instance = groupMonitors.find((m) => m.effectSource === effectSource);
    if (instance == null || instance.killGroup == null)
        return;
    jass.GroupRemoveUnit(instance.killGroup, unit);
    groupUnitMap.delete(getUnitId(unit));
}
export function isMultiKillMonitored(unit) {
    return groupUnitMap.has(getUnitId(unit));
}
export function getMultiKillMonitorCount() {
