...
const { withTimer } = require("系统.00．核心系统.01．封装函数");
...
function checkRegionCondition(cond, _unit) {
    if (!cond || cond === "always")
        return true;
...
function runRegionRule(rule, unit, owner) {
    if (!rule)
        return;
    ...
    if (chosen.action === "KillUnit") {
        if (typeof jass.KillUnit === "function") {
            jass.KillUnit(unit);
        }
        const msg = formatText(chosen.text);
        if (msg && owner != null && typeof jass.DisplayTimedTextToPlayer === "function") {
            jass.DisplayTimedTextToPlayer(owner, 0, 0, 8, msg);
        }
    }
    else if (chosen.action === "Teleport") {
        if (typeof jass.SetUnitPosition === "function" && chosen.x != null && chosen.y != null) {
...
function initRegionTeleport() {
    ...
    for (const k in 区域传送配置) {
        ...
        if (cfg == null || !cfg.enabled)
            continue;
        if (typeof jass.CreateRegion !== "function")
            continue;
        const region = jass.CreateRegion();
        // dbg("已创建区域: " + cfg.id);
        if (typeof jass.Rect !== "function")
            continue;
        const rect = jass.Rect(cfg.left, cfg.bottom, cfg.right, cfg.top);
        if (typeof jass.RegionAddRect === "function") {
            jass.RegionAddRect(region, rect);
        }
        if (typeof jass.TriggerRegisterEnterRegion === "function") {
    ...
    const onEnter = () => {
        const unit = typeof jass.GetTriggerUnit === "function"
            ? jass.GetTriggerUnit()
            : null;
        const region = typeof jass.GetTriggeringRegion === "function"
        ...
        if (unit == null || region == null)
            return;
        // 前置条件：不处理中立敌对单位
        const owner = typeof jass.GetOwningPlayer === "function"
            ? jass.GetOwningPlayer(unit)
            : null;
        if (owner != null &&
            typeof jass.Player === "function" &&
            jass.PLAYER_NEUTRAL_AGGRESSIVE != null) {
            const neutralAgg = jass.Player(jass.PLAYER_NEUTRAL_AGGRESSIVE);
            if (owner === neutralAgg)
                return;
        }
        ...
        if (!checkRegionCondition(cfg.condition, unit))
            return;
        ...
        if (typeof jass.SetUnitPosition === "function") {
            jass.SetUnitPosition(unit, cfg.teleportX, cfg.teleportY);
        }
        if (typeof jass.IssueImmediateOrder === "function") {
        ...
        if (player != null) {
            ...
            if (typeof jass.DisplayTimedTextToPlayer === "function") {
                jass.DisplayTimedTextToPlayer(player, 0, 0, 8, cfg.text);
            }
        }
    };
