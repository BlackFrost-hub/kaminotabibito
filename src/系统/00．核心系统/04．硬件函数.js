...
export const KEY_STATE = { DOWN: 1, UP: 2 };
/** 鼠标按键（BzAPI：1=左，2=右，3=中） */
export const MOUSE_BUTTON = { LEFT: 1, RIGHT: 2, MIDDLE: 3 };
/** A-Z */
export const KEY = {
...
export function isHardwareAPIAvailable() {
    return (typeof japi.DzIsKeyDown === "function" &&
...
export function registerKeyDown(keyCode, callback) {
...
export function registerKeyUp(keyCode, callback) {
...
function registerKeyEventRawStatus(keyCode, status, sync, action) {
    return registerKeyEventByCode(keyCode, status, sync, action);
}
...
export function registerMouseWheel(sync, action) {
    const trig = createTriggerOrNull();
    if (!trig)
        return null;
...
export function frameFindByName(name, id) {
...
export function frameSetScriptByCode(frame, eventId, action, sync) {
    const f = japiFn("DzFrameSetScriptByCode");
    if (f)
        f(frame, eventId, action, sync);
}
// -------------------- 测试：B 键广播 9999 --------------------
function initTestKeyB() {
    ...
    const hook = (st) => {
        registerKeyEventRawStatus(KEY.B, st, false, () => {
            ...
            if (last && !down) {
                for (let i = 0; i < 12; i++) {
                    jass.DisplayTimedTextToPlayer(jass.Player(i), 0, 0, 3, "9999");
                }
                if (typeof jass.GetPlayerName === "function" && p) {
                    jass.DisplayTimedTextToPlayer(jass.Player(0), 0, 0, 3, "from=" + jass.GetPlayerName(p));
                }
            }
        });
    };
