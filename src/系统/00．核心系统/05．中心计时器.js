...
function updateDate(y, remainSec, dayBy2015) {
...
const TIME_GET_KEYS = ["year", "month", "day", "hour", "minute", "second", "weekday", "millisecond"];
function removeFnFromArray(arr, fn) {
...
const _periodicCallbacks = [];
let _delayedCallbackIdCounter = 0;
const _delayedCallbacks = [];
function runPeriodicCallbacks() {
    const now = nowMs();
...
export function getTime(i) {
    if (i < 0 || i > 7)
        return 0;
    return _timeCache[TIME_GET_KEYS[i]];
}
export function getGameTime() {
    return nowMs() - (_initialized ? _serverTime : 0);
}
export function getGameElapsedTime() {
    return _gameElapsedTime;
}
export function getGameTimeHMS() {
...
export function getGameTimeStringWithMs() {
...
export function getDateTimeStringWithMs() {
...
export function addPeriodicCallback(intervalMs, callback) {
    const id = ++_periodicCallbackIdCounter;
    _periodicCallbacks.push({ id, intervalMs, lastRunTime: nowMs(), callback });
    return id;
}
export function removePeriodicCallback(id) {
    const idx = _periodicCallbacks.findIndex((c) => c.id === id);
    if (idx > -1)
        _periodicCallbacks.splice(idx, 1);
}
export function addDelayedCallback(delayMs, callback) {
...
export function removeDelayedCallback(id) {
    for (const d of _delayedCallbacks) {
        if (d.id === id)
            d.active = false;
    }
}
export function onSecond(callback) {
    _secondCallbacks.push(callback);
}
export function onTick10ms(callback) {
    _tickCallbacks.push(callback);
}
export function offSecond(callback) {
    removeFnFromArray(_secondCallbacks, callback);
}
export function offTick10ms(callback) {
    removeFnFromArray(_tickCallbacks, callback);
}
export function initCenterTimer() {
    if (_initialized)
        return;
    if (bootstrapTimer) {
