...
const YDWE模块 = require("lib.扩展函数.YDWE函数.01．YDUserData兼容");
...
const { StarOther_PanCameraToTimedForPlayer } = require("lib.扩展函数.Star扩展函数.Star扩展库.00．镜头函数");
const { onTick10ms, offTick10ms } = globalThis;
function 移动镜头到玩家(玩家, x, y) {
...
const DzFrameGetHeroBarButton = japi.DzFrameGetHeroBarButton;
...
const DzFrameSetPriority = japi.DzFrameSetPriority;
const DzFrameShow = japi.DzFrameShow;
const GetUnitX = jass.GetUnitX;
const GetUnitY = jass.GetUnitY;
const GetUnitName = jass.GetUnitName;
const ReviveHeroLoc = jass.ReviveHeroLoc;
const GetRandomDirectionDeg = jass.GetRandomDirectionDeg;
const Cos = jass.Cos;
const Sin = jass.Sin;
...
const 英雄栏倒计时底阴影框体表 = [0, 0, 0, 0, 0];
const 英雄栏倒计时左描边框体表 = [0, 0, 0, 0, 0];
const 英雄栏倒计时右描边框体表 = [0, 0, 0, 0, 0];
const 英雄栏倒计时阴影框体表 = [0, 0, 0, 0, 0];
const 英雄栏倒计时框体表 = [0, 0, 0, 0, 0];
const 英雄栏倒计时剩余秒表 = [0, 0, 0, 0, 0];
function 是否有效(handle) {
    return handle != null && handle !== 0;
}
function 取英雄栏槽位(unit) {
    if (!是否有效(unit))
        return -1;
    ...
    if (playerId < 0 || playerId >= 英雄栏文本框体数量)
        return -1;
    return playerId;
}
function 十倍精度文本(value) {
...
function 隐藏英雄栏倒计时(槽位) {
    if (槽位 < 0 || 槽位 >= 英雄栏倒计时框体表.length)
        return;
    英雄栏倒计时剩余秒表[槽位] = 0;
...
function 刷新英雄栏倒计时文本(槽位) {
    const 底阴影框体 = 英雄栏倒计时底阴影框体表[槽位];
    const 左描边框体 = 英雄栏倒计时左描边框体表[槽位];
    const 右描边框体 = 英雄栏倒计时右描边框体表[槽位];
    const 阴影框体 = 英雄栏倒计时阴影框体表[槽位];
    const 文字框体 = 英雄栏倒计时框体表[槽位];
    if (底阴影框体 === 0 || 左描边框体 === 0 || 右描边框体 === 0 || 阴影框体 === 0 || 文字框体 === 0)
        return;
...
function 创建英雄栏倒计时框体(槽位) {
    ...
    DzFrameSetPoint(底阴影框体, 帧点中心, button, 帧点中心, 英雄栏倒计时偏移X + 英雄栏倒计时底阴影偏移X, 英雄栏倒计时偏移Y + 英雄栏倒计时底阴影偏移Y);
    ...
    DzFrameSetPoint(左描边框体, 帧点中心, button, 帧点中心, 英雄栏倒计时偏移X + 英雄栏倒计时左描边偏移X, 英雄栏倒计时偏移Y + 英雄栏倒计时左描边偏移Y);
    ...
    DzFrameSetPoint(右描边框体, 帧点中心, button, 帧点中心, 英雄栏倒计时偏移X + 英雄栏倒计时右描边偏移X, 英雄栏倒计时偏移Y + 英雄栏倒计时右描边偏移Y);
    ...
    DzFrameSetPoint(阴影框体, 帧点中心, button, 帧点中心, 英雄栏倒计时偏移X + 英雄栏倒计时阴影偏移X, 英雄栏倒计时偏移Y + 英雄栏倒计时阴影偏移Y);
...
function 启动英雄栏倒计时(unit) {
    ...
    const 阴影框体 = 英雄栏倒计时阴影框体表[槽位];
    if (底阴影框体 !== 0)
        DzFrameShow(底阴影框体, true);
    if (左描边框体 !== 0)
        DzFrameShow(左描边框体, true);
    if (右描边框体 !== 0)
        DzFrameShow(右描边框体, true);
    if (阴影框体 !== 0)
        DzFrameShow(阴影框体, true);
...
function 执行复活(dyingUnit) {
    if (!是否有效(dyingUnit))
        return;
    if (!是玩家英雄(dyingUnit))
        return;
    if (jass.IsUnitType(dyingUnit, jass.UNIT_TYPE_DEAD) !== true)
        return;
    隐藏英雄栏倒计时(取英雄栏槽位(dyingUnit));
    const 剩余次数 = YDWE模块.YDUserDataGet("string", 复活次数表, 复活次数属性, "integer");
    if (剩余次数 != null && 剩余次数 <= 0)
        return;
    if (剩余次数 != null) {
...
function 英雄死亡延迟复活(dyingUnit, 击杀者) {
    if (!是玩家英雄(dyingUnit))
        return;
    启动英雄栏倒计时(dyingUnit);
    addDelayedCallback(复活延迟秒 * 1000, function () {
        执行复活(dyingUnit);
    });
}
export function 初始化英雄复活() {
    if (已注册死亡)
        return;
    已注册死亡 = true;
    初始化英雄栏倒计时框体();
    ...
    死亡模块.registerDeathListener(英雄死亡延迟复活);
}
