...
function Boss脱战完成() {
    ...
    if (boss == null || boss === 0)
        return;
    if (jass.IsUnitType(boss, jass.UNIT_TYPE_DEAD))
        return;
    设置生命百分比(boss, 100);
    设置魔法百分比(boss, 100);
    const bossName = jass.GetUnitName(boss);
    jass.QuestMessageBJ(jass.GetPlayersAll(), jass.bj_QUESTMESSAGE_WARNING, "|cffffff00『系统消息』|r：|cffff0000Boss|r|cffff6600『" + bossName + "』|r由于太久没受到玩家伤害脱战回血了。");
}
function 检查移除脱战Buff(unit, damage) {
    if (!拥有Buff(unit, 脱战BuffID))
        return;
...
function 单位受伤事件(unit, _attacker, damage, _snapshot) {
