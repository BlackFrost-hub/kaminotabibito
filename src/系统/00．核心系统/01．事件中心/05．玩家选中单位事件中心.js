...
const selectedUnit = {};
const selectedCount = {};
const selectedUnitsByPlayer = {};
const selectionListeners = [];
const registeredPlayers = {};
let selectedTrigger = null;
let deselectedTrigger = null;
let initialized = false;
function isValidPlayer(whichPlayer) {
    return !!whichPlayer && whichPlayer !== 0;
}
function isRealUnit(whichUnit) {
    return !!whichUnit && whichUnit !== 0 && jass.GetUnitTypeId(whichUnit) !== 0;
}
function getUnitHandleId(whichUnit) {
    if (!whichUnit || whichUnit === 0)
        return 0;
    return typeof jass.GetHandleId === "function" ? jass.GetHandleId(whichUnit) : 0;
}
function getSelectedUnitList(playerId) {
...
function refreshPlayerSelectionSummary(playerId) {
...
function findSelectedUnitIndex(list, whichUnit) {
    const hid = getUnitHandleId(whichUnit);
    if (hid === 0)
        return -1;
    for (let i = 0; i < list.length; i++) {
        if (getUnitHandleId(list[i]) === hid)
            return i;
    }
    return -1;
}
function dispatchSelectionListeners(player, playerId, unit, isSelected) {
    for (let i = 0; i < selectionListeners.length; i++) {
        selectionListeners[i](player, playerId, unit, isSelected);
    }
}
function handleSelectionEvent(isSelected) {
...
function onPlayerUnitSelectedAction() {
    handleSelectionEvent(true);
}
function onPlayerUnitDeselectedAction() {
    handleSelectionEvent(false);
}
function ensureSelectionTriggers() {
...
function registerSelectionTriggersForPlayer(whichPlayer) {
 ...
 */
export function initPlayerSelectionCenter(whichPlayer) {
    registerSelectionTriggersForPlayer(whichPlayer);
}
function normalizeSelectionListener(arg1, arg2) {
 ...
 */
export function addSelectionListener(arg1, arg2) {
 ...
 * 移除之前注册的选中状态监听。
 */
export function removeSelectionListener(arg1, arg2) {
    const listener = normalizeSelectionListener(arg1, arg2);
    if (typeof listener !== "function")
        return;
 ...
 */
export function getSoleSelectedUnitForPlayer(playerId) {
    if (!initialized)
        return null;
 ...
 */
export function seedSoleSelectedUnitForPlayer(whichPlayer, whichUnit) {
    if (!isValidPlayer(whichPlayer))
        return;
    const playerId = jass.GetPlayerId(whichPlayer);
 ...
 */
export function getSelectionDebugForPlayer(playerId) {
    const count = selectedCount[playerId] || 0;
    const unit = selectedUnit[playerId];
