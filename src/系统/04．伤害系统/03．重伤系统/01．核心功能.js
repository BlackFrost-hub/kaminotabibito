...
const { buffs } = require("系统.05．Buff系统.01．Buff表");
import { 重伤系统开关, 重伤上限, 重伤下限, 重伤BuffID, 重伤效果系数, 重伤默认持续时间, } from "./00．常量定义";
...
const { registerManualBuff, getBuffRuntime, 移除单位指定Buff } = require("系统.05．Buff系统.00．Buff系统");
const { 创建Dz绑定单位特效, 销毁Dz绑定单位特效, 是否已有Dz绑定单位特效, } = require("lib.扩展函数.封装函数.01．通用工具.03．特效");
...
function 清理重伤附着特效(unit) {
    if (unit == null || unit === 0)
        return;
    销毁Dz绑定单位特效(unit, 重伤特效Key);
}
function 重伤Buff移除回调(unit) {
    清理重伤附着特效(unit);
}
function 刷新重伤附着特效(unit) {
    if (unit == null || unit === 0)
        return;
    if (重伤特效模型 === "")
        return;
    if (是否已有Dz绑定单位特效(unit, 重伤特效Key))
        return;
    创建Dz绑定单位特效(unit, 重伤特效挂点, 重伤特效模型, 重伤特效Key);
}
/** 限制重伤值在有效范围内 */
function 限制重伤值(value) {
    if (value < 重伤下限)
        return 重伤下限;
    if (value > 重伤上限)
        return 重伤上限;
 ...
 * @param 持续时间 秒，默认3秒
 */
export function 施加重伤(unit, 重伤值, 持续时间 = 重伤默认持续时间, source) {
    if (unit == null || unit === 0)
        return;
    if (持续时间 <= 0)
        return;
...
/** 移除单位重伤 */
export function 移除单位重伤(unit) {
    if (unit == null || unit === 0)
        return;
    移除单位指定Buff(unit, 重伤BuffID);
}
/** 计算重伤后的治疗量 */
function 计算重伤治疗量(source, target, amount) {
    if (!重伤系统开关)
        return amount;
    if (amount <= 0)
        return 0;
...
function on伤害事件(target, source, applied, _snapshot) {
    if (!重伤系统开关)
        return;
    if (!(applied > 0))
        return;
...
/** 注册重伤回调到治疗系统和伤害事件 */
export function initWoundSystem() {
    if (!重伤系统开关)
        return;
