...
// ========== 虚拟分区：DOT 子模块导入 ==========
import { dotEffectModelFromBuffRow, } from "./02．DOT定义/01．DOT配置";
import { registerBuiltInDotTypes } from "./02．DOT定义/03．DOT类型定义";
import { getDotSourceDisplayName, isValidDotStateRow, tabDeleteHid, tabRowForHid, tabSetHid, unitHid, } from "./02．DOT定义/04．DOT工具";
...
const damageEventModule = require("系统.04．伤害系统.01．伤害事件");
const leakCore = require("系统.00．核心系统.05．泄露审计");
const LeakWatcher = leakCore.LeakWatcher ?? leakCore;
// ========== 虚拟分区：DOT 类型配置与注册 ==========
const dotTypes = [];
/** 注册一种 DOT，后续伤害回调会按配置解析装备并施加/覆盖 */
export function registerDotType(config) {
    dotTypes.push(config);
}
// ========== 虚拟分区：Buff 池桥接 ==========
/** Buff 池同步：避免顶层 require 循环，运行时加载 05．Buff系统.00．Buff系统 */
function notifyBuffPool(typeId, target, state) {
    pcall(() => {
        const m = require("系统.05．Buff系统.00．Buff系统");
        if (m != null && typeof m.syncDotBuff === "function")
...
const ignoredTargetByType = {};
// ========== 虚拟分区：装备数据源 ==========
const equipDataMod = require("系统.02．物品系统.01．装备数据");
...
// ========== 虚拟分区：dotTicks 清理 ==========
function removeDotTicksForTargetHid(typeId, tgtHid) {
 ...
 */
export function syncDotRemainingFromBuffPool() {
    dotStateSync.syncDotRemainingFromBuffPool();
}
/** Buff 池判定某 DOT 到期时调用（池行已删，勿再 syncDotBuff null） */
export function clearDotByBuffPoolExpire(buffID, hid) {
 ...
 */
export function tryApplyHeroAttackGearDots(source, target, _damage) {
...
export function notifyDotTickBatchDamageDisplayed() {
    dotExecutor.notifyDotTickBatchDamageDisplayed();
}
// ========== 虚拟分区：伤害回调入口（委托策略模块） ==========
function onDamage(target, damage, damageType, fromDotTickBatch, source, isNormalAttackHit) {
...
export function dealSpiritDamage(source, target, amount) {
    dotExecutor.dealDamageForType("antiHeal", source, target, amount);
}
/** 造成火焰伤害（外部技能与 burn DOT 同源类型时可调用） */
export function dealBurnDamage(source, target, amount) {
