...
// 导入HOT系统
const { startHot, isHotActive } = require("系统.04．伤害系统.02．治疗系统.04．持续治疗效果");
 ...
 */
function canBeHealed(unit, sourcePlayer, source) {
    if (!isValidUnit(unit))
        return false;
    if (!jass.IsUnitAlly(unit, sourcePlayer) && unit !== source)
        return false;
    return true;
}
//=============================================================================
 ...
 * @param healMP 治疗MP量
 */
export function doHealItemEffect(abilId, target, healHP, healMP) {
    if (!HEAL_ITEM_SYSTEM_ENABLED)
        return;
    if (target == null)
        return;
 ...
 * @param healMP 治疗MP量
 */
export function doHealItemEffectById(abilIdStr, target, healHP, healMP) {
    if (typeof abilIdStr !== "string" || abilIdStr.length !== 4)
        return;
 ...
 * 检查技能ID是否为物品治疗技能
 */
export function isHealItemAbility(abilId) {
    return (abilId === ABIL_A002 ||
        abilId === ABIL_A0LF ||
        abilId === ABIL_A015 ||
        abilId === ABIL_A0B8 ||
        abilId === ABIL_A08C);
