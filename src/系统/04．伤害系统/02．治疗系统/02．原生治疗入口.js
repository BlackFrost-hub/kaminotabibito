...
const { GetSpellAbilityId } = require("lib.扩展函数.BJ函数.07．杂项");
 ...
 * 当任意马甲单位施放治疗技能时触发
 */
export function initNativeHealEntry() {
    if (!NATIVE_HEAL_ENTRY_ENABLED)
        return;
    if (nativeHealEntryInitialized)
        return;
 ...
 * 检查系统是否已初始化
 */
export function isNativeHealEntryInitialized() {
