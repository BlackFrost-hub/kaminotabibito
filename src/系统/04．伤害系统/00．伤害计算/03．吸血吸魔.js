...
const { getRealAttr, getRealAttrWithLimit, isPlayerUnit, canBreakManaStealLimit, } = require("系统.04．伤害系统.00．伤害计算.01．属性读取");
const { STAT_LIMITS, ENEMY_STAT_LIMITS } = require("系统.04．伤害系统.00．伤害计算.00．伤害常量");
const 漂浮文字模块 = require("lib.扩展函数.封装函数.03．漂浮文字.index");
const CreateFloatTextOnUnit = 漂浮文字模块.CreateFloatTextOnUnit;
const { isAncientUnit, formatNumber, forEachUnitInGroup } = require("lib.扩展函数.封装函数.01．通用工具.index");
//=============================================================================
 ...
 */
export function getPlayerHeroGroup() {
    try {
 ...
 * 遍历玩家英雄组，找到属于同一玩家的英雄
 */
export function getHeroForAncientUnit(ancientUnit) {
    if (ancientUnit == null)
        return null;
 ...
 * - 马甲单位：返回所属玩家的英雄
 */
export function getStealBeneficiary(source) {
    if (source == null)
        return null;
    // 马甲单位：找玩家英雄
 ...
 * @returns 吸血百分比
 */
export function calcLifeSteal(attacker, isPlayer) {
 ...
 */
export function calcTotalLifeSteal(attacker, isPlayer, isMagic, isNormalAttack) {
 ...
 * @returns 回复值
 */
export function calcLifeStealHeal(attacker, damage, isMagic, isNormalAttack) {
 ...
 */
export function applyLifeSteal(attacker, heal, showText = true) {
    if (heal <= 0 || attacker == null)
        return;
    // 获取当前生命
 ...
 * @returns 吸魔值
 */
export function calcManaSteal(attacker, damage) {
    if (!jass.IsUnitType(attacker, jass.UNIT_TYPE_HERO))
        return 0;
    const isPlayer = isPlayerUnit(attacker);
    let manaStealPercent = getRealAttr(attacker, "伤害吸魔", 0);
    if (manaStealPercent <= 0)
        return 0;
 ...
 * @param showText 是否显示漂浮文字
 */
export function applyManaSteal(attacker, mana, showText = true) {
    if (mana <= 0 || attacker == null)
        return;
    // 获取当前魔法
 ...
 * @param showText 是否显示漂浮文字
 */
export function applyLifeAndManaSteal(attacker, damage, isMagic, isNormalAttack, showText = true) {
    // 获取实际受益单位（马甲单位 -> 玩家英雄）
