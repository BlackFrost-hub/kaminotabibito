...
const { STAT_LIMITS, ENEMY_STAT_LIMITS, BREAKABLE_LIMITS } = require("系统.04．伤害系统.00．伤害计算.00．伤害常量");
const { YDWEGetUnitArmor } = require("lib.扩展函数.YDWE函数.06．护甲获取");
const { isPlayerUnit: isPlayerUnitBase } = require("lib.扩展函数.封装函数.01．通用工具.index");
//=============================================================================
// 一、玩家判定
//=============================================================================
/**
 ...
 * 获取单位所属玩家ID
 */
export function getPlayerId(unit) {
    if (unit == null)
        return -1;
 ...
 */
export function getUnitAttr(unit, attrName, valueType, defaultValue = 0) {
    if (unit == null)
        return defaultValue;
    // 先尝试读取单位属性
    ...
    if (valueType === "real" || valueType === "integer") {
        ...
        if (unitValue === true || unitValue === 1)
            return true;
        if (unitValue === false || unitValue === 0)
            return false;
    }
    // 单位没有该属性，尝试读取玩家属性
    ...
    if (player != null) {
        ...
        if (valueType === "real" || valueType === "integer") {
            ...
            if (playerValue === true || playerValue === 1)
                return true;
        }
    }
    return defaultValue;
 ...
 * 读取实数属性
 */
export function getRealAttr(unit, attrName, defaultValue = 0) {
    return Number(getUnitAttr(unit, attrName, "real", defaultValue));
}
/**
 * 读取布尔属性
 */
export function getBoolAttr(unit, attrName, defaultValue = false) {
 ...
 */
export function getRealAttrWithLimit(unit, attrName, isPlayer) {
    ...
    // 玩家可突破上限检查
    if (isPlayer) {
        const breakAttr = BREAKABLE_LIMITS[attrName];
        if (breakAttr != null) {
...
//=============================================================================
/** 读取攻击者的伤害加成属性 */
export function getAttackerDamageBonus(attacker) {
    return getRealAttr(attacker, "伤害%", 0);
}
/** 读取受击者的伤害减少% */
export function getTargetDamageReduction(target, isPlayer) {
    return getRealAttrWithLimit(target, "伤害减少%", isPlayer);
}
/** 读取受击者的魔抗（应用上限） */
export function getTargetMagicResist(target, isPlayer) {
    return getRealAttrWithLimit(target, "魔抗", isPlayer);
}
/** 读取受击者的物理抗性（应用上限） */
export function getTargetPhysResist(target, isPlayer) {
    return getRealAttrWithLimit(target, "物理抗性", isPlayer);
}
/** 读取攻击者的护甲穿透 */
export function getAttackerArmorPierce(attacker) {
    return getRealAttr(attacker, "护甲穿透", 0);
}
/** 读取攻击者的魔法穿透 */
export function getAttackerMagicPierce(attacker) {
    return getRealAttr(attacker, "魔法穿透", 0);
}
/** 读取受击者护甲 */
export function getTargetArmor(target) {
...
//=============================================================================
/** 是否免疫伤害 */
export function isImmuneDamage(unit) {
    return getBoolAttr(unit, "免疫伤害", false);
}
/** 是否免疫普攻 */
export function isImmuneNormalAttack(unit) {
    return getBoolAttr(unit, "免疫普攻", false);
}
/** 是否减伤关闭 */
export function isDamageReduceDisabled(unit) {
    return getBoolAttr(unit, "减伤关闭", false);
}
/** 是否无视护甲 */
export function isIgnoreArmor(attacker) {
    return getBoolAttr(attacker, "无视护甲", false);
}
/** 是否无视魔抗 */
export function isIgnoreMagicResist(attacker) {
    return getBoolAttr(attacker, "无视魔抗", false);
}
/** 是否伤害吸魔突破 */
export function canBreakManaStealLimit(unit) {
