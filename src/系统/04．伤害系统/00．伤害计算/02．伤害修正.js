...
const { getRealAttr, getRealAttrWithLimit, getAttackerArmorPierce, getAttackerMagicPierce, getTargetArmor, isIgnoreArmor, isIgnoreMagicResist, isPlayerUnit, } = require("系统.04．伤害系统.00．伤害计算.01．属性读取");
const { calcArmorReduction, calcPiercedArmorReduction, } = require("lib.扩展函数.封装函数.06．伤害函数.index");
const { OperatorRealMultiply, OperatorResistReduction, createValueHolder, } = require("lib.扩展函数.封装函数.01．通用工具.index");
//=============================================================================
// 一、护甲穿透计算
//=============================================================================
/**
 ...
 * @param attacker 攻击者
 */
export function applyArmorPenetration(damage, target, attacker) {
 ...
 * @param attacker 攻击者
 */
export function applyMagicResist(damage, target, attacker) {
 ...
 * @returns 加法叠加的伤害加成
 */
export function calcElementalDamageBonus(attacker, damageAttr) {
 ...
 * @returns 乘法叠加的减伤比例
 */
export function calcElementalResistReduction(target, resistAttr, isPlayer) {
 ...
 * 获取物理伤害修正
 */
export function getPhysicalDamageModifier(attacker, target, isPlayer) {
 ...
 * 获取技能伤害修正
 */
export function getSkillDamageModifier(attacker, target, isPlayer) {
 ...
 * 获取普攻伤害修正
 */
export function getNormalAttackModifier(attacker, target, isPlayer) {
 ...
 * 获取魔法伤害修正
 */
export function getMagicDamageModifier(attacker) {
 ...
 * 获取强化伤害修正
 */
export function getEnhancedDamageModifier(attacker) {
 ...
 * 获取最终伤害加成
 */
export function getFinalDamageBonus(attacker) {
 ...
 * 条件：目标非英雄且非恶魔种族
 */
export function getAntMasteryBonus(attacker, target) {
    // 检查目标是否为英雄
 ...
 * 条件：目标是英雄或恶魔种族
 */
export function getBossMasteryBonus(attacker, target) {
    // 检查目标是否为英雄
    const isHero = jass.IsUnitType(target, jass.UNIT_TYPE_HERO);
    // 检查目标是否为恶魔种族
    const isDemon = jass.GetUnitRace(target) === jass.RACE_DEMON;
 ...
 * 检查单位是否为召唤物
 */
export function isSummonedUnit(unit) {
    return jass.IsUnitType(unit, jass.UNIT_TYPE_SUMMONED);
}
/**
 * 获取召唤物伤害修正
 */
export function getSummonDamageModifier(attacker, target, isPlayer) {
    // 攻击者不是召唤物，不生效
