...
const { getRealAttr, getRealAttrWithLimit, isPlayerUnit, isImmuneDamage, isImmuneNormalAttack, isDamageReduceDisabled, } = require("系统.04．伤害系统.00．伤害计算.01．属性读取");
const { applyArmorPenetration, applyMagicResist, getPhysicalDamageModifier, getSkillDamageModifier, getNormalAttackModifier, getMagicDamageModifier, getEnhancedDamageModifier, getFinalDamageBonus, getAntMasteryBonus, getBossMasteryBonus, getSummonDamageModifier, calcElementalDamageBonus, calcElementalResistReduction, } = require("系统.04．伤害系统.00．伤害计算.02．伤害修正");
const { applyLifeAndManaSteal } = require("系统.04．伤害系统.00．伤害计算.03．吸血吸魔");
const { applyDamageModifiers } = require("系统.04．伤害系统.00．伤害计算.06．伤害修正回调");
const { createDelayedCall } = require("lib.扩展函数.封装函数.01．通用工具.02．计时器");
const ConvertDamageType = jass.ConvertDamageType;
const ConvertAttackType = jass.ConvertAttackType;
const ConvertWeaponType = jass.ConvertWeaponType;
const 伤害函数 = require("lib.扩展函数.封装函数.06．伤害函数.index");
const appliedFinalDamageListeners = [];
export function 延后一帧执行伤害派生效果(callback) {
    createDelayedCall(0.0, callback);
}
/** 在 `onDamageEvent` 完成 `YDWESetEventDamage`（或免疫置 0）后收到 `(target, attacker, applied)` */
export function registerAppliedFinalDamageListener(cb) {
    for (let i = 0; i < appliedFinalDamageListeners.length; i++) {
        if (appliedFinalDamageListeners[i] === cb)
            return;
    }
    appliedFinalDamageListeners.push(cb);
}
function notifyAppliedFinalDamageListeners(target, attacker, applied, snapshot) {
    for (let i = 0; i < appliedFinalDamageListeners.length; i++) {
...
function captureDamageTypeSnapshot() {
    return {
 ...
 * 检查是否免疫伤害
 */
function checkImmune(target, isNormalAtk) {
    // 免疫伤害（且未关闭减伤）
 ...
 * @returns 伤害计算结果
 */
export function calculateDamage(target, attacker, baseDamage) {
    // 初始化
 ...
 * 在伤害回调中调用
 */
export function onDamageEvent(target, attacker, baseDamage) {
    if (target == null || baseDamage < 0.1)
        return;
