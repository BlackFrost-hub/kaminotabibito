...
export function splitItemBuffSegments(buff) {
...
export function readNumberFromString(s, startIdx) {
    let numEnd = startIdx;
...
export function parseStandardDotBuff(buffStr, keyword, createResult, requireValuePositive = true) {
    ...
    if (duration <= 0)
        return null;
    if (requireValuePositive && value <= 0)
        return null;
