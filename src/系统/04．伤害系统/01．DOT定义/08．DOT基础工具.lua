...
function ____exports.createDotBaseUtils(deps)
    ...
    local function isDebuffDotTargetOk(source, target)
        if source == nil or target == nil or target == 0 then
            return false
        end
        local utStruct = getStructureUnitTypeHandle()
    ...
    local function heroUnitTypeForIsUnitType()
    ...
    local function isSourceHeroPlayer1to4(unit)
        if not unit then
            return false
        end
        ...
        local utHero = heroUnitTypeForIsUnitType()
    ...
    local function getBestDotFromUnit(unit, parseBuff, getProduct)
        local best = nil
        do
            local slot = 0
            while slot <= 5 do
                do
                    ...
                    local segments = (entry and entry.Buff) ~= nil and splitItemBuffSegments(entry.Buff) or ({})
                    do
                        local si = 0
                        while si < #segments do
                            do
                                local parsed = parseBuff(segments[si + 1])
                                if not parsed then
                                    goto __continue28
                                end
                                local product = getProduct(parsed)
    ...
    local function getUnitMaxHp(targetUnit)
    ...
    local function getTargetRegenHP(targetUnit)
        if not targetUnit then
            return 0
        end
