...
export function createDotBaseUtils(deps) {
    ...
    function getStructureUnitTypeHandle() {
        ...
        if (direct != null)
            return direct;
        if (typeof deps.jass.ConvertUnitType === "function")
    ...
    function isDebuffDotTargetOk(source, target) {
        ...
        if (typeof deps.jass.IsUnitType === "function" && utStruct != null) {
            if (deps.jass.IsUnitType(target, utStruct) === true)
                return false;
        }
        if (typeof deps.jass.GetOwningPlayer !== "function")
            return false;
    ...
    function isSourceHeroPlayer1to4(unit) {
        ...
        if (hasIsUnitType && utHero != null && deps.jass.IsUnitType(unit, utHero) === true)
            return true;
        if (hasHeroLevel && deps.jass.GetHeroLevel(unit) > 0)
            return true;
        return false;
    }
    // ========== 虚拟分区：装备读取 ==========
    function unitItemInSlot(unit, slot) {
        if (typeof deps.jass.UnitItemInSlot !== "function")
            return null;
        return deps.jass.UnitItemInSlot(unit, slot);
    }
    function getItemTypeId(item) {
        if (typeof deps.jass.GetItemTypeId !== "function")
            return 0;
    ...
    function getUnitMaxHp(targetUnit) {
        ...
        if (typeof deps.jass.GetUnitState !== "function")
            return 0;
        let maxLifeState = null;
        if (deps.jass.UNIT_STATE_MAX_LIFE != null)
            maxLifeState = deps.jass.UNIT_STATE_MAX_LIFE;
        else if (deps.g.UNIT_STATE_MAX_LIFE != null)
            maxLifeState = deps.g.UNIT_STATE_MAX_LIFE;
        else if (typeof deps.jass.ConvertUnitState === "function")
    ...
    function getTargetRegenHP(targetUnit) {
        if (typeof deps.jass.GetUnitTypeId !== "function" || !targetUnit)
            return 0;
