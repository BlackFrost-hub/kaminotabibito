...
function QuestManager.prototype.setupTimeLimit(self, playerId, questId)
    local ____opt_0 = questDB:getPlayerData(playerId)
    local quest = ____opt_0 and ____opt_0.quests:get(questId)
...
function QuestManager.prototype.onQuestAbandoned(self, playerId, questId)
    ...
    local ____opt_8 = questDB:getPlayerData(playerId)
    local ____opt_6 = ____opt_8 and ____opt_8.quests:get(questId)
    local nativeHandle = ____opt_6 and ____opt_6.nativeHandle
...
function QuestManager.prototype.toggleQuestTracking(self, playerId, questId)
    ...
    local questData = ____opt_10 and ____opt_10.quests:get(questId)
...
function QuestManager.prototype.updateQuestObjective(self, playerId, questId, objectiveId, progress)
    ...
    if success then
        self:triggerUIRefresh(playerId, questId)
        local ____opt_13 = questDB:getPlayerData(playerId)
        local quest = ____opt_13 and ____opt_13.quests:get(questId)
...
function QuestManager.prototype.syncToWar3Quest(self, playerId, questId)
    ...
    local questData = ____opt_15 and ____opt_15.quests:get(questId)
    if not questData then
        return
    end
    ...
    repeat
        local ____switch71 = questData.status
        local ____cond71 = ____switch71 == QuestStatus.IN_PROGRESS
        if ____cond71 then
        ...
        ____cond71 = ____cond71 or ____switch71 == QuestStatus.COMPLETED
        if ____cond71 then
        ...
        ____cond71 = ____cond71 or ____switch71 == QuestStatus.FAILED
        if ____cond71 then
...
function QuestManager.prototype.giveQuestRewards(self, playerId, questId)
    ...
    for ____, reward in ipairs(quest.rewards) do
        repeat
            local ____switch79 = reward.type
            local ____cond79 = ____switch79 == "experience"
            if ____cond79 then
            ...
            ____cond79 = ____cond79 or ____switch79 == "gold"
            if ____cond79 then
            ...
            ____cond79 = ____cond79 or ____switch79 == "item"
            if ____cond79 then
            ...
            ____cond79 = ____cond79 or ____switch79 == "attribute"
            if ____cond79 then
