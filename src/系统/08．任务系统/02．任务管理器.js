...
const { findHeroOfPlayer } = require("系统.00．核心系统.01．封装函数");
import { questDB, QuestType, QuestStatus, createTestQuests } from "./01．任务数据";
// 调试输出
...
export class QuestManager {
    ...
    setupTimeLimit(playerId, questId) {
        ...
        const timerData = globalThis.__questTimers || (globalThis.__questTimers = new Map());
     ...
     */
    toggleQuestTracking(playerId, questId) {
        const questData = questDB.globalData?.quests.get(questId);
        if (!questData)
            return false;
     ...
     * 同步任务状态到War3原生任务
     */
    syncToWar3Quest(playerId, questId) {
        if (typeof jass.CreateQuest !== "function")
            return;
        const questData = questDB.globalData?.quests.get(questId);
        if (!questData)
            return;
 ...
 */
export function handleQuestAccepted() {
    const playerId = g.udg_QuestPlayerId;
    const questId = g.udg_QuestId;
 ...
 * - udg_Progress: 进度值
 */
export function handleObjectiveUpdated() {
    const playerId = g.udg_QuestPlayerId;
    const questId = g.udg_QuestId;
    const objectiveId = g.udg_ObjectiveId;
    const progress = g.udg_Progress;
 ...
 * - udg_QuestId: 任务ID字符串
 */
export function handleQuestFailed() {
    const playerId = g.udg_QuestPlayerId;
    const questId = g.udg_QuestId;
    if (playerId === undefined || questId === undefined) {
 ...
 * - udg_QuestId: 任务ID字符串
 */
export function handleQuestAbandoned() {
    const playerId = g.udg_QuestPlayerId;
    const questId = g.udg_QuestId;
    if (playerId === undefined || questId === undefined) {
