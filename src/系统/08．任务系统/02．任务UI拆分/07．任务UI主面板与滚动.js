...
import { ENTRY_X, ENTRY_Y, PANEL_REL_TO_ENTRY_X, PANEL_REL_TO_ENTRY_Y, PANEL_W, PANEL_H, LIST_CONTAINER_REL_TO_PANEL_X, LIST_CONTAINER_REL_TO_PANEL_Y, SCROLLBAR_REL_X, SCROLLBAR_TOP_INSET, SCROLLBAR_BOTTOM_INSET, SCROLLBAR_W, LIST_VIEW_H, SCROLL_THUMB_SIZE, ENABLE_TASK_UI_RIGHT_SCROLLBAR, } from "./01．任务UI常量";
import { tryCreateFromFdfOnly, tryCreateFromFdfWithSource, pcallDzFrameShow } from "./02．任务UI辅助";
import { buildTaskPanelCategoryTabs } from "./06．任务UI分类标签";
// ========== 虚拟分区：主面板 + 列表容器 + 右侧滚动条构建 ==========
export function buildTaskMainPanel(opts) {
    const { japi, parent, entryFrame, FrameType, FramePoint, createFrame, setFramePosition, setFrameSize, setFramePointRelative, setFrameTexture, setFrameHoverEvents, setFrameClickEvent, setButtonText, createTabLabelTextOnBackdrop, setupTransparentGlueHitLayer, onClickSound, onSwitchCategory, onShowTabTooltip, slotId, contextId, } = opts;
    const empty = {
        mainPanel: null,
        listContainer: null,
        tabMainBg: null,
    ...
    let scrollBarHitBtn = null;
    let scrollThumbFrame = null;
    ...
    if (ENABLE_TASK_UI_RIGHT_SCROLLBAR) {
        ...
        if (scrollThumbFrame && scrollThumbFrame !== 0) {
            ...
            if (scrollBarFrame && scrollBarFrame !== 0) {
                setFramePointRelative(scrollThumbFrame, FramePoint.TOPLEFT, scrollBarFrame, FramePoint.TOPLEFT, (SCROLLBAR_W - SCROLL_THUMB_SIZE) * 0.5, 0);
            }
    ...
    const result = {
        ...
        scrollBarHitBtn: scrollBarHitBtn ?? null,
