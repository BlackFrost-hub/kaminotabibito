...
const { round, clampMin, clampRange } = require("lib.扩展函数.封装函数.01．通用工具.index");
import { ENABLE_MOUSE_WHEEL_SCROLL, ENTRY_Y, PANEL_REL_TO_ENTRY_Y, LIST_VIEW_H, SCROLLBAR_W, SCROLLBAR_TOP_INSET, SCROLL_THUMB_SIZE, SCROLL_THUMB_TOP_COMPENSATION, SCROLL_THUMB_BOTTOM_COMPENSATION, } from "./01．任务UI常量";
import { isWheelTargetForTaskList as isWheelTargetForTaskListByJapi, isTaskScrollThumbDragHit, isTaskScrollBarTrackHit, } from "./03．任务UI列表与滚动";
import { createTriggerOrNull, getClientHeight, getMouseY, getMouseYRelative, getWindowHeight, getScrollbarTrackThumbTravelPx, registerMouseButtonEventByCode, registerMouseMoveEventByCode, } from "../../../lib/扩展函数/封装函数/04．硬件输入/index";
...
function findVisibleWheelCtx() {
    for (let i = 0; i < allWheelCtxs.length; i++) {
        const ctx = allWheelCtxs[i];
        if (ctx.isOwnedByLocalPlayer() && ctx.isVisible())
            return ctx;
    }
    return null;
}
function taskUIWheelEventPcallBody() {
    const ctx = findVisibleWheelCtx();
    if (!ctx || !ctx.isVisible())
        return;
    if (!isWheelTargetForTaskListByJapi(japi, ctx.getMouseFocus, ctx.listContainer, ctx.scrollBarHitBtn || ctx.scrollBarFrame, ctx.scrollThumbFrame, ctx.scrollThumbHitBtn))
        return;
...
function setTaskScrollThumbByRatio(ctx, ratio) {
    if (!ctx.scrollBarFrame || !ctx.scrollThumbFrame)
        return;
...
function updateTaskUIScrollThumbPosition(ctx, pageCount) {
    if (pageCount <= 1) {
...
export function isTaskUIWheelTarget(ctx) {
    if (!ctx.mainPanel)
        return false;
...
export function handleTaskUIListWheel(ctx) {
    ...
    if (delta < 0)
        nextPage = currentPage + 1 < pageCount ? currentPage + 1 : pageCount - 1;
...
function ratioFromThumbDragMouseY(pageCount, mouseYPx) {
    const travelNorm = thumbTravelNorm();
    if (travelNorm <= 0 || pageCount <= 1)
        return 0;
...
function onThumbDragStart() {
    if (!dragCtx)
        return;
    if (dragCtx.getCurrentPageCount() <= 1)
        return;
...
function onThumbDragMove() {
    if (!thumbDragActive || !dragCtx)
        return;
...
function onScrollBarTrackClick(ctx) {
    const pageCount = ctx.getCurrentPageCount();
    if (pageCount <= 1)
        return;
...
function taskUIThumbPressPcallBody() {
    const ctx = findVisibleWheelCtx();
    if (!ctx || !ctx.isVisible())
        return;
...
function onGlobalThumbLeftPress() {
    taskUIThumbPressPcallBody();
}
function taskUIThumbReleasePcallBody() {
    onThumbDragEnd();
}
function onGlobalThumbLeftRelease() {
    taskUIThumbReleasePcallBody();
}
function taskUIThumbMovePcallBody() {
    onThumbDragMove();
}
function onGlobalThumbDragMove() {
    taskUIThumbMovePcallBody();
}
// ── 虚拟分区：全局鼠标事件注册与滚轮注册 ──
function ensureTaskThumbGlobalMouseRegistered() {
    if (taskThumbGlobalMouseRegistered)
        return;
...
export function updateTaskUIScrollBarVisibility(ctx, pageCount, hasQuestRows) {
    ...
    for (const frame of [ctx.scrollBarFrame, ctx.scrollBarHitBtn, ctx.scrollThumbFrame, ctx.scrollThumbHitBtn]) {
        if (frame && frame !== 0)
            pcallDzFrameShow(frame, visible);
    }
    if (visible)
        updateTaskUIScrollThumbPosition(ctx, pageCount);
}
