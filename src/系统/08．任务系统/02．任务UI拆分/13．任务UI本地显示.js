 ...
 */
import { setVisible } from "./08．任务UI列表控制";
/**
 ...
 */
// ========== 虚拟分区：展开/翻页本地帧显隐 ==========
export function toggleExpandLocal(pool, category, pageIndex, expandedQuestId, questId) {
    if (!pool)
        return;
 ...
 */
export function switchPageLocal(pool, category, prevPageIndex, nextPageIndex) {
    if (!pool)
        return;
    const cv = pool.categories[category];
    if (!cv)
