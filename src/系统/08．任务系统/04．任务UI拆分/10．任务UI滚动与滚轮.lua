...
function updateTaskUIScrollThumbPosition(self, ctx, pageCount)
    ...
    local travelRange = LIST_VIEW_H - SCROLL_THUMB_SIZE - SCROLL_THUMB_TOP_COMPENSATION - SCROLL_THUMB_BOTTOM_COMPENSATION
    if travelRange < 0 then
        travelRange = 0
    end
    local topOffset = SCROLL_THUMB_TOP_COMPENSATION
    if pageCount > 1 then
        ...
        topOffset = topOffset + travelRange * ratio
    end
