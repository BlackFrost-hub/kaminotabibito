...
local registerMouseWheel = _____786C_4EF6_51FD_6570.registerMouseWheel
...
local _____5782_76F4_6EDA_52A8_6761_8F68_9053 = require("系统.09．表现系统.垂直滚动条轨道")
...
local _____97F3_6548_51FD_6570 = require("系统.00．核心系统.音效函数")
local SoundUI_ClickPlay = _____97F3_6548_51FD_6570.SoundUI_ClickPlay
local ____UI_51FD_6570 = require("系统.00．核心系统.UI函数")
local DZ_TEXT_ALIGN_CENTER = ____UI_51FD_6570.DZ_TEXT_ALIGN_CENTER
local DZ_TEXT_ALIGN_LEFT = ____UI_51FD_6570.DZ_TEXT_ALIGN_LEFT
local applyDzTextFontAndAlignment = ____UI_51FD_6570.applyDzTextFontAndAlignment
local applyDzTextFontAndCenterAlignment = ____UI_51FD_6570.applyDzTextFontAndCenterAlignment
local createTabLabelTextOnBackdrop = ____UI_51FD_6570.createTabLabelTextOnBackdrop
local setupTransparentGlueHitLayer = ____UI_51FD_6570.setupTransparentGlueHitLayer
--- 任务系统 - 全新任务 UI（魔兽原生风格）
...
function TaskUI.prototype.setupThumbDrag(self)
    ...
    local ____opt_1 = self.vScrollTrack
...
function TaskUI.prototype.refreshList(self)
    ...
    if #quests == 0 then
        ...
        if empty then
            ...
            ____self_listItemFrames_3[#____self_listItemFrames_3 + 1] = empty
...
function TaskUI.prototype.createListItem(self, quest, rowTopRel, expanded)
    ...
    ____self_listItemFrames_4[#____self_listItemFrames_4 + 1] = rowBackdrop
    ...
    ____self_listItemFrames_5[#____self_listItemFrames_5 + 1] = titleFrame
    ...
    if showMainRowIcon then
        ...
        if iconFr ~= 0 then
            ...
            ____self_listItemFrames_7[#____self_listItemFrames_7 + 1] = iconFr
        end
    end
    if expanded then
        ...
        for ____, obj in ipairs(quest.objectives) do
            do
                ...
                repeat
                    ...
                    ____self_listItemFrames_8[#____self_listItemFrames_8 + 1] = objFrame
                    objYRel = objYRel - LIST_ITEM_H * 0.25
                    __continue226 = true
                until true
                if not __continue226 then
                    break
                end
            end
        end
        if quest.timeLimit and quest.timeLimit > 0 then
            ...
            ____self_listItemFrames_9[#____self_listItemFrames_9 + 1] = failFrame
        end
    end
    return nil
end
...
function TaskUI.prototype.hide(self)
    ...
    local ____opt_10 = self.vScrollTrack
    if ____opt_10 ~= nil then
        ____opt_10:cancelDrag()
    end
    self.isVisible = false
    ...
    registerKeyDown(
        ...
        function(____, player)
            ...
            if type(jass.GetPlayerId) == "function" then
                ____temp_12 = jass.GetPlayerId
            else
                ____temp_12 = nil
            end
            local getPid = ____temp_12
