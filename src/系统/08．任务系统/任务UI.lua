...
function TaskUI.prototype.createMainPanel(self, parent)
    ...
    if self.entryFrame then
        ...
        debugPrint(nil, "createMainPanel: 无 entryFrame，主面板用 ENTRY+相对偏移 绝对坐标兜底")
...
function TaskUI.prototype.createListItem(self, quest, rowTopRel, expanded)
    ...
    debugPrint(
        nil,
        (((((("createListItem questId=" .. quest.id) .. " expanded=") .. tostring(expanded)) .. " rowTopRel=") .. tostring(rowTopRel)) .. " title=") .. quest.title
    )
    ...
    local textXRel = isMain0102Icon and rowLeftRel + QUEST_ROW_ICON_PAD_LEFT + iconHLayout + QUEST_ROW_TEXT_GAP_AFTER_ICON or rowLeftRel + 0.03
