...
const { ceil, forEachSorted } = require("lib.扩展函数.封装函数.01．通用工具.index");
const { stringToFourCC } = require("lib.扩展函数.封装函数.01．通用工具.index");
const BJ_RADTODEG = jglobals.bj_RADTODEG ?? 57.29577951308232;
const { CHEST_TYPES, DEFAULT_OPEN_TIME, INTERACT_RANGE, UPDATE_INTERVAL, PROGRESS_BAR_HEIGHT_OFFSET, YDLOCAL_VAR_OPENER, YDLOCAL_VAR_CHEST, YDLOCAL_VAR_PRE_OPENER, YDLOCAL_VAR_PRE_CHEST, isChestType, getChestConfig, } = require("系统.06．经济系统.00．宝箱系统.00．常量定义");
const { 触发宝箱准备开启回调 } = require("系统.06．经济系统.00．宝箱系统.04．准备开启回调");
const { 触发宝箱开启中回调 } = require("系统.06．经济系统.00．宝箱系统.05．开启中回调");
const { 触发宝箱开启完成回调 } = require("系统.06．经济系统.00．宝箱系统.06．开启完成回调");
const { 创建进度条特效, 销毁进度条特效 } = require("系统.03．技能系统.00．技能模板+函数.01．技能函数.06．施法·蓄力·充能.进度条特效");
const { 广播单位类型提示 } = require("系统.06．经济系统.00．宝箱系统.07．主人广播");
const { dropItemsFromChest } = require("系统.06．经济系统.00．宝箱系统.01．宝箱掉落配置");
const { dropItemsFromChestConfig } = require("系统.06．经济系统.00．宝箱系统.01．宝箱掉落配置");
const { 查找宝箱主人 } = require("系统.06．经济系统.00．宝箱系统.08．宝箱主人");
const { debugLogForce } = require("lib.扩展函数.自定义扩展函数.03．调试输出");
const 漂浮文字模块 = require("lib.扩展函数.封装函数.03．漂浮文字.03．创建漂浮文字");
const CreateFloatTextOnUnit = 漂浮文字模块.CreateFloatTextOnUnit;
const 调试模块 = "宝箱系统-核心";
...
function isSamePoint(ax, ay, bx, by) {
    return ax <= bx + 1 && ax >= bx - 1 && ay <= by + 1 && ay >= by - 1;
}
export function onUnitTargetChestPointOrder(unit, x, y) {
    if (!unit)
        return;
...
export function onUnitTargetChestImmediateOrder(unit, orderId) {
    if (!unit)
        return;
...
// STES事件触发
// ==========================================================================================
function fireStesEvent(_eventName, _opener, _target) { }
// ==========================================================================================
// 开启核心逻辑
// ==========================================================================================
...
function onChestCenterTimerTick() {
    if (openingMap.size === 0 && movingMap.size === 0)
        return;
 ...
 * @param target 目标可破坏物
 */
export function onUnitTargetInteractable(unit, target) {
    if (!unit || !target) {
 ...
 * 检查单位是否正在开启
 */
export function isUnitOpening(unit) {
    if (!unit)
        return false;
    const unitId = getUnitId(unit);
 ...
 * 中断单位开启
 */
export function interruptOpening(unit) {
    if (!unit)
        return;
    const unitId = getUnitId(unit);
...
export { YDLOCAL_VAR_OPENER, YDLOCAL_VAR_CHEST, YDLOCAL_VAR_PRE_OPENER, YDLOCAL_VAR_PRE_CHEST, isInteractable, getOpenTime, };
