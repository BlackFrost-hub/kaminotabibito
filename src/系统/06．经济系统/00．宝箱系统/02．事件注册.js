...
const { onUnitTargetInteractable, onUnitTargetChestPointOrder, onUnitTargetChestImmediateOrder, isInteractable } = require("系统.06．经济系统.00．宝箱系统.03．宝箱核心");
const { 宝箱系统开关 } = require("系统.06．经济系统.00．宝箱系统.00．常量定义");
...
const orderEventCenter = require("系统.00．核心系统.01．事件中心.11．单位指令事件中心");
// ==========================================================================================
// 常量
// ==========================================================================================
...
function onGlobalTargetOrder(unit, _orderId, _targetUnit, _targetItem, targetDestructable) {
    if (unit == null || unit === 0 || targetDestructable == null || targetDestructable === 0)
        return;
    const unitId = jass.GetHandleId(unit);
    if (!已注册宝箱英雄.has(unitId))
        return;
...
function onGlobalPointOrder(unit, _orderId, x, y) {
    if (unit == null || unit === 0)
        return;
    const unitId = jass.GetHandleId(unit);
    if (!已注册宝箱英雄.has(unitId))
...
function onGlobalImmediateOrder(unit, orderId) {
    if (unit == null || unit === 0)
        return;
    const unitId = jass.GetHandleId(unit);
    if (!已注册宝箱英雄.has(unitId))
 ...
 * 注册单位目标命令事件监听
 */
function tryRegisterTargetOrderStes() {
    const g = globalThis;
    if (g[REG_GUARD])
        return;
    if (g[TRIG_KEY] == null) {
...
function ensureGlobalTargetOrderListener() {
    const g = globalThis;
    if (g[GLOBAL_ORDER_GUARD])
        return;
 ...
 * 当英雄被登记时调用
 */
export function registerChestSystemHero(hero) {
    if (!宝箱系统开关)
        return;
    if (!hero)
        return;
    已注册宝箱英雄.add(jass.GetHandleId(hero));
 ...
 * 初始化宝箱系统
 */
export function initChestSystem() {
    if (!宝箱系统开关)
        return;
    ensureGlobalTargetOrderListener();
    tryRegisterTargetOrderStes();
