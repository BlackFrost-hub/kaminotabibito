...
const { SHARE_RANGE } = require("系统.06．经济系统.01．杀敌金币平分.00．常量定义");
const { getObjectPropertyInteger } = require("lib.扩展函数.YDWE函数.index");
const { YDUserDataGet } = require("lib.扩展函数.YDWE函数.01．YDUserData兼容");
const { AdjustPlayerStateBJ } = require("lib.扩展函数.封装函数.01．通用工具.index");
const { Sound3DII_Mp3PlayReuse } = require("lib.扩展函数.封装函数.02．音效系统.index");
const 漂浮文字模块 = require("lib.扩展函数.封装函数.03．漂浮文字.index");
const CreateFloatTextOnUnit = 漂浮文字模块.CreateFloatTextOnUnit;
const { getUnitOwnerId } = require("lib.扩展函数.封装函数.01．通用工具.index");
...
// ==========================================================================================
/** 金币获取回调列表 */
const goldGainCallbacks = [];
// ==========================================================================================
// 工具函数
// ==========================================================================================
/** 获取单位赏金 */
function getUnitBounty(unitType) {
    return getObjectPropertyInteger(2, unitType, "bountyplus");
}
/** 检查单位是否为玩家英雄 */
function isPlayerHero(unit) {
...
function isValidDyingUnit(dyingUnit) {
 ...
 * 给予玩家金币（带音效和漂浮文字）
 */
function giveGoldToPlayer(unit, player, baseGold, isShared) {
    let params = { unit, player, baseGold, isShared };
    // 通知回调，让回调决定最终金币
    for (const cb of goldGainCallbacks) {
        const pcallResult = pcall(cb, params);
 ...
 * 处理单位死亡事件
 */
function onUnitDeathHandler(dyingUnit, killer) {
    // 检查死亡单位是否有效
    if (!isValidDyingUnit(dyingUnit))
        return;
    // 获取死亡单位赏金
    ...
    if (killerPlayer == null || heroGroup == null)
        return;
    // 遍历玩家英雄
    ...
    for (let i = 0; i < heroCount; i++) {
        ...
        if (hero === killer)
            continue;
        // 检查是否为友方
        if (jass.IsUnitAlly(hero, killerPlayer) !== true)
            continue;
        // 检查是否在范围内
 ...
 * @param cb 回调函数
 */
export function registerGoldGainCallback(cb) {
 ...
 * 初始化杀敌金币平分系统
 */
export function initGoldShareSystem() {
    if (_initialized)
        return;
    _initialized = true;
