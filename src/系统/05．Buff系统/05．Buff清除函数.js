 ...
 */
const buffPool = require("系统.05．Buff系统.00．Buff系统");
...
function isBuffTypeMatched(buffID, typePrefix, onlyPurgable) {
    ...
    if (typeof typeName !== "string" || typeName === "")
        return false;
    if (typeName.substring(0, typePrefix.length) !== typePrefix)
        return false;
    if (onlyPurgable && meta.canPurge !== true)
        return false;
    return true;
}
function isBuffDispelMatched(buffID, typePrefix, maxDispelLevel, onlyPurgable) {
    const meta = BUFF_TABLE[buffID];
    if (meta == null)
        return false;
    const typeName = meta.type;
    if (typeof typeName !== "string" || typeName === "")
        return false;
    if (typePrefix !== "" && typeName.substring(0, typePrefix.length) !== typePrefix)
        return false;
    if (onlyPurgable && meta.canPurge !== true)
        return false;
    const dispelLevel = meta.dispelLevel;
    if (typeof dispelLevel !== "number")
        return false;
    if (dispelLevel < 0)
        return false;
 ...
 */
export function 移除单位指定类型Buff(unit, typePrefix, onlyPurgable = false) {
    if (unit == null || unit === 0 || typePrefix === "")
        return 0;
    const ids = getBuffIdsOnUnit(unit);
    let removed = 0;
    for (let i = 0; i < ids.length; i++) {
        const buffID = ids[i];
        if (!isBuffTypeMatched(buffID, typePrefix, onlyPurgable))
            continue;
        if (移除单位指定Buff(unit, buffID))
            removed = removed + 1;
    }
    return removed;
}
/** 清除单位增益 Buff（Buff 表 type 以 `Buff:` 开头）。 */
export function 移除单位增益Buff(unit, onlyPurgable = false) {
    return 移除单位指定类型Buff(unit, "Buff:", onlyPurgable);
}
/** 清除单位负面 Buff（Buff 表 type 以 `Debuff:` 开头）。 */
export function 移除单位负面Buff(unit, onlyPurgable = false) {
 ...
 */
export function 按驱散等级移除单位Buff(unit, maxDispelLevel, typePrefix = "", onlyPurgable = true) {
    if (unit == null || unit === 0)
        return 0;
    if (typeof maxDispelLevel !== "number" || maxDispelLevel < 0)
        return 0;
    const ids = getBuffIdsOnUnit(unit);
    let removed = 0;
    for (let i = 0; i < ids.length; i++) {
        const buffID = ids[i];
        if (!isBuffDispelMatched(buffID, typePrefix, maxDispelLevel, onlyPurgable))
            continue;
        if (移除单位指定Buff(unit, buffID))
            removed = removed + 1;
    }
...
export function 一级驱散单位Buff(unit, typePrefix = "", onlyPurgable = true) {
    return 按驱散等级移除单位Buff(unit, 1, typePrefix, onlyPurgable);
}
/** 2 级驱散：强驱散。默认只驱散 `canPurge: true`。 */
export function 二级驱散单位Buff(unit, typePrefix = "", onlyPurgable = true) {
