...
const 数学运算 = require("lib.扩展函数.封装函数.01．通用工具.07．数学运算");
...
function formatDotTooltip(template, durationForDisplay, effectValue, effectValue2, sourceName, intervalSec) {
...
export function buildBuffBarViewModel(unit) {
    const slots = [];
    ...
    for (let i = 0; i < MAX_SLOTS && i < rows.length; i++) {
        ...
        if (meta !== undefined) {
            const tooltipParts = formatDotTooltip(meta.tooltip, durationForTip, row.state.effect, row.state.effect2 ?? 0, row.state.sourceName, meta.interval);
