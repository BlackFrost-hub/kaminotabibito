...
const { EXCLUDED_UNIT_TYPES } = require("系统.05．Buff系统.01．控制抗性.00．控制抗性常量");
//=============================================================================
// 一、排除单位检测
//=============================================================================
/**
 * 检查单位是否被排除
 */
export function isExcludedFromControlResist(unit) {
 ...
 * 检查技能命令是否为排除类型
 */
function isExcludedOrder(abilityId) {
 ...
 * 获取技能英雄持续时间
 */
export function getHeroDuration(abilityId) {
 ...
 * 2. 英雄持续时间 > 0.02秒
 */
export function isControlAbility(abilityId) {
    if (isExcludedOrder(abilityId))
        return false;
 ...
 * 条件：当前命令为stop或麻痹状态
 */
export function isUnitControlled(unit) {
    const currentOrder = jass.GetUnitCurrentOrder(unit);
    // 检查是否为stop命令
    if (currentOrder === STOP_ORDER_ID)
        return true;
    // 检查是否为麻痹状态
    if (currentOrder === PARALYSIS_ORDER_ID)
        return true;
    // 检查是否发出了stop命令
    const issuedOrder = jass.GetIssuedOrderId();
    if (jass.OrderId2String(issuedOrder) === "stop")
        return true;
    return false;
}
