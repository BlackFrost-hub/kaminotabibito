...
const { CONTROL_REDUCTION_CAP, BOSS_CONTROL_LIMITS, } = require("系统.05．Buff系统.01．控制抗性.00．控制抗性常量");
const { getHeroDuration } = require("系统.05．Buff系统.01．控制抗性.01．控制检测");
//=============================================================================
// 一、控制抗性属性读取
//=============================================================================
/**
 ...
 * 优先级：单位属性 > 玩家属性
 */
export function getControlReduction(unit) {
    // 先读取单位属性
 ...
 * 应用控制时间削减上限
 */
export function applyControlReductionCap(reduction) {
    return reduction < CONTROL_REDUCTION_CAP ? reduction : CONTROL_REDUCTION_CAP;
}
/**
 * 检查并应用Boss控制时间上限
 */
export function applyBossControlLimit(unit, duration) {
 ...
 */
export function calcReducedControlDuration(target, originalDuration) {
 ...
 * @returns 实际控制时间
 */
export function calcReducedControlTime(target, abilityId) {
    // 获取原始控制时间
