--[[ Generated with https://github.com/TypeScriptToLua/TypeScriptToLua ]]
local ____exports = {}
local ____03_FF0E_7956_5730_53CC_7075_536B_8BD5_70BC = require("系统.11．剧情系统.03．世界线变动.01．灵心归源世界线.04．精灵往事.03．祖地双灵卫试炼")
local ____init_7956_5730_53CC_7075_536B_8BD5_70BC = ____03_FF0E_7956_5730_53CC_7075_536B_8BD5_70BC["init祖地双灵卫试炼"]
local ____04_FF0E_7956_5730_53CC_7075_536B_5165_53E3_4E0E_5BF9_8BDD = require("系统.11．剧情系统.03．世界线变动.01．灵心归源世界线.04．精灵往事.04．祖地双灵卫入口与对话")
local ____init_7956_5730_53CC_7075_536B_5165_53E3_4E0E_5BF9_8BDD = ____04_FF0E_7956_5730_53CC_7075_536B_5165_53E3_4E0E_5BF9_8BDD["init祖地双灵卫入口与对话"]
local ____05_FF0E_7956_5730_53CC_7075_536B_4F20_9001_4E0E_95F8_95E8 = require("系统.11．剧情系统.03．世界线变动.01．灵心归源世界线.04．精灵往事.05．祖地双灵卫传送与闸门")
local ____init_7956_5730_53CC_7075_536B_4F20_9001_4E0E_95F8_95E8 = ____05_FF0E_7956_5730_53CC_7075_536B_4F20_9001_4E0E_95F8_95E8["init祖地双灵卫传送与闸门"]
local ____06_FF0E_7956_5730_53CC_7075_536BBoss_573A_666F = require("系统.11．剧情系统.03．世界线变动.01．灵心归源世界线.04．精灵往事.06．祖地双灵卫Boss场景")
local ____init_7956_5730_53CC_7075_536BBoss_573A_666F = ____06_FF0E_7956_5730_53CC_7075_536BBoss_573A_666F["init祖地双灵卫Boss场景"]
local ____07_FF0E_7956_5730_53CC_7075_536B_5956_52B1_63D0_4EA4 = require("系统.11．剧情系统.03．世界线变动.01．灵心归源世界线.04．精灵往事.07．祖地双灵卫奖励提交")
local ____init_7956_5730_53CC_7075_536B_5956_52B1_63D0_4EA4 = ____07_FF0E_7956_5730_53CC_7075_536B_5956_52B1_63D0_4EA4["init祖地双灵卫奖励提交"]
local ____08_FF0E_7956_5730_53CC_7075_536B_9053_4E2D_5173_5361 = require("系统.11．剧情系统.03．世界线变动.01．灵心归源世界线.04．精灵往事.08．祖地双灵卫道中关卡")
local ____init_7956_5730_53CC_7075_536B_9053_4E2D_5173_5361 = ____08_FF0E_7956_5730_53CC_7075_536B_9053_4E2D_5173_5361["init祖地双灵卫道中关卡"]
function ____exports.init()
    ____init_7956_5730_53CC_7075_536B_8BD5_70BC()
    ____init_7956_5730_53CC_7075_536B_5165_53E3_4E0E_5BF9_8BDD()
    ____init_7956_5730_53CC_7075_536B_4F20_9001_4E0E_95F8_95E8()
    ____init_7956_5730_53CC_7075_536BBoss_573A_666F()
    ____init_7956_5730_53CC_7075_536B_5956_52B1_63D0_4EA4()
    ____init_7956_5730_53CC_7075_536B_9053_4E2D_5173_5361()
end
do
    local ____export = require("系统.11．剧情系统.03．世界线变动.01．灵心归源世界线.04．精灵往事.01．祖地双灵卫副本配置")
    for ____exportKey, ____exportValue in pairs(____export) do
        if ____exportKey ~= "default" then
            ____exports[____exportKey] = ____exportValue
        end
    end
end
do
    local ____export = require("系统.11．剧情系统.03．世界线变动.01．灵心归源世界线.04．精灵往事.02．祖地双灵卫副本状态")
    for ____exportKey, ____exportValue in pairs(____export) do
        if ____exportKey ~= "default" then
            ____exports[____exportKey] = ____exportValue
        end
    end
end
do
    local ____export = require("系统.11．剧情系统.03．世界线变动.01．灵心归源世界线.04．精灵往事.03．祖地双灵卫试炼")
    for ____exportKey, ____exportValue in pairs(____export) do
        if ____exportKey ~= "default" then
            ____exports[____exportKey] = ____exportValue
        end
    end
end
do
    local ____export = require("系统.11．剧情系统.03．世界线变动.01．灵心归源世界线.04．精灵往事.04．祖地双灵卫入口与对话")
    for ____exportKey, ____exportValue in pairs(____export) do
        if ____exportKey ~= "default" then
            ____exports[____exportKey] = ____exportValue
        end
    end
end
do
    local ____export = require("系统.11．剧情系统.03．世界线变动.01．灵心归源世界线.04．精灵往事.05．祖地双灵卫传送与闸门")
    for ____exportKey, ____exportValue in pairs(____export) do
        if ____exportKey ~= "default" then
            ____exports[____exportKey] = ____exportValue
        end
    end
end
do
    local ____export = require("系统.11．剧情系统.03．世界线变动.01．灵心归源世界线.04．精灵往事.06．祖地双灵卫Boss场景")
    for ____exportKey, ____exportValue in pairs(____export) do
        if ____exportKey ~= "default" then
            ____exports[____exportKey] = ____exportValue
        end
    end
end
do
    local ____export = require("系统.11．剧情系统.03．世界线变动.01．灵心归源世界线.04．精灵往事.07．祖地双灵卫奖励提交")
    for ____exportKey, ____exportValue in pairs(____export) do
        if ____exportKey ~= "default" then
            ____exports[____exportKey] = ____exportValue
        end
    end
end
do
    local ____export = require("系统.11．剧情系统.03．世界线变动.01．灵心归源世界线.04．精灵往事.08．祖地双灵卫道中关卡")
    for ____exportKey, ____exportValue in pairs(____export) do
        if ____exportKey ~= "default" then
            ____exports[____exportKey] = ____exportValue
        end
    end
end
return ____exports
