--[[ Generated with https://github.com/TypeScriptToLua/TypeScriptToLua ]]
local ____exports = {}
local ____05_FF0E_7D27_51D1_5267_60C5_7247_6BB5_7F16_8BD1 = require("系统.11．剧情系统.01．主线任务.00．剧情系统核心工具.05．紧凑剧情片段编译")
local _____7F16_8BD1_7D27_51D1_5267_60C5_7247_6BB5 = ____05_FF0E_7D27_51D1_5267_60C5_7247_6BB5_7F16_8BD1["编译紧凑剧情片段"]
____exports["王城门禁紧凑剧情片段"] = {
    ["片段ID"] = "elven_city_gate_open",
    ["名称"] = "王城门禁开启",
    ["触发条件"] = "剧情进度 == 21 且玩家靠近王城门禁",
    ["可Esc整段跳过"] = true,
    ["默认倍速"] = 1,
    ["默认对白持续时间"] = 3,
    ["对白列表"] = {{["序号"] = 1, ["说话者"] = "玩家", ["文本"] = "我等是奥斯特里帝国异端调查团，这是阿尔文阁下赐下的门扉之印。", ["持续时间"] = 4}, {["序号"] = 2, ["说话者"] = "精灵禁军", ["文本"] = "印记无误。诸位帝国使者，请入城吧。城中的去向与门扉之印的指引，皆已为诸位备妥。", ["持续时间"] = 5}, {["序号"] = 3, ["说话者"] = "玩家", ["文本"] = "多谢告知，我等这便前往觐见。", ["持续时间"] = 3}},
    ["动作时间线"] = {{
        ["序号"] = 1,
        ["挂点"] = "beforeDialog",
        ["对白序号"] = 1,
        ["动作ID"] = "JLC精灵城_王城门禁开启",
        ["名称"] = "王城门禁开启",
        ["参数"] = {
            ["触发进度"] = 21,
            ["目标进度"] = 22,
            ["闸门单位"] = "gg_unit_n04R_0048",
            ["触发范围"] = 999,
            ["触发单位发布命令"] = "stop",
            ["开门延时秒数"] = 2.5,
            ["开门对象"] = "gg_dest_LTe1_11879",
            ["隐藏阻挡"] = "gg_dest_B00K_5466",
            ["解锁视野"] = "gg_rct______________121, gg_rct______________122, gg_rct______________123",
            ["旧JASS功能清单"] = "TimerStart / ModifyGateBJ / ShowDestructable(false) / CreateFogModifierRectBJ"
        }
    }, {
        ["序号"] = 2,
        ["挂点"] = "afterDialog",
        ["对白序号"] = 3,
        ["动作ID"] = "JLC精灵城_前往王宫外庭",
        ["名称"] = "前往王宫外庭",
        ["参数"] = {["任务更新提示"] = "|cffffff00『主线目标』：|r觐见|cffff99cc『克林姆德国王』|r。", ["小地图X"] = -10900.6, ["小地图Y"] = -10601.8}
    }}
}
____exports["王城门禁剧情片段"] = _____7F16_8BD1_7D27_51D1_5267_60C5_7247_6BB5(____exports["王城门禁紧凑剧情片段"])
return ____exports
