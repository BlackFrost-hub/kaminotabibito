...
const YD读取用户数据 = YD安全模块.YDUserDataGetSafe;
const YD设置技能冷却数据 = YD安全模块.YDWESetUnitAbilityDataRealSafe;
const YD读取对象实数属性 = YD安全模块.getObjectPropertyRealSafe;
const 转四字节 = 通用工具模块.stringToFourCC;
const 技能对象类型 = 0;
const { COOLDOWN_REDUCTION_CAP, SKILL_COOLDOWN_CAPS, COOLDOWN_BLACKLIST, EXCLUDED_COOLDOWN_UNIT, } = require("系统.03．技能系统.01．技能冷却.00．冷却常量");
 ...
 */
function getCooldownAttrValue(unit, attrName) {
    if (unit == null)
        return 0;
 ...
 * 获取冷却缩减属性
 */
export function getCooldownReduction(unit) {
    return getCooldownAttrValue(unit, "冷却缩减");
}
/**
 * 获取冷却缩减加成属性（突破上限）
 */
export function getCooldownReductionBonus(unit) {
 ...
 * @returns 冷却上限
 */
export function getCooldownCap(abilityId, hasBonus) {
 ...
 * 应用冷却缩减上限
 */
export function applyCooldownCap(reduction, abilityId, bonus) {
    const entries = Object.entries(SKILL_COOLDOWN_CAPS).sort(([a], [b]) => a < b ? -1 : a > b ? 1 : 0);
    // 检查技能独立上限
    for (const [配置键名, cap] of entries) {
        if (转四字节(提取内部ID(配置键名)) === abilityId) {
 ...
 * 获取技能原始冷却时间
 */
export function getBaseCooldown(abilityId, level) {
    // 使用YDWE函数读取技能冷却
 ...
 * @returns 实际冷却时间
 */
export function calcActualCooldown(baseCooldown, reduction) {
    if (reduction <= 0)
        return baseCooldown;
 ...
 * 设置技能冷却时间
 */
export function setAbilityCooldown(unit, abilityId, level, cooldown) {
