...
const { YDUserDataGet, YDWEGetUnitAbilityDataInteger, YDWEGetUnitAbilityDataReal, getObjectProperty, ObjectType } = require("lib.扩展函数.YDWE函数.index");
const { safeTimerStart, safeDestroyTimer } = require("系统.00．核心系统.07．联机安全工具");
const { PERCENT_COST_THRESHOLD } = require("系统.03．技能系统.02．技能消耗.00．消耗常量");
const pendingManaRefundByTimerHid = {};
function onManaRefundTimerExpire() {
    const timer = jass.GetExpiredTimer();
    if (!timer)
        return;
    const timerHid = jass.GetHandleId(timer);
 ...
 * 默认仅处理 nightelf 技能
 */
export function isRefundableAbility(abilityId) {
 ...
 * 获取技能固定消耗
 */
export function getAbilityManaCost(unit, abilityId, level) {
    return YDWEGetUnitAbilityDataInteger(unit, abilityId, level, 104);
}
/**
 * 获取技能百分比消耗
 */
export function getAbilityPercentCost(unit, abilityId, level) {
    return YDWEGetUnitAbilityDataReal(unit, abilityId, level, 102);
}
/**
 * 计算技能总消耗
 */
export function calcTotalManaCost(unit, abilityId, level) {
 ...
 * 获取魔法消耗属性
 */
export function getManaCostReduction(unit) {
    const player = jass.GetOwningPlayer(unit);
    if (player == null)
        return 0;
    return YDUserDataGet("player", player, "魔法消耗", "real");
}
export function hasEffectiveManaCostReduction(unit) {
 ...
 * 执行魔法返还
 */
export function applyManaRefund(unit, manaCost) {
    const reduction = getManaCostReduction(unit);
    const refundRatio = reduction < 0 ? -reduction : reduction;
    if (refundRatio < 0.01)
        return;
 ...
 * @returns 是否执行了返还
 */
export function handleManaRefund(unit, abilityId) {
    if (!isRefundableAbility(abilityId))
        return false;
    if (!hasEffectiveManaCostReduction(unit))
        return false;
