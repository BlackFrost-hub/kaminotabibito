...
function 获取弹幕系统触发器() {
    return jglobals.gg_trg_____________DMXT ?? globalThis.gg_trg_____________DMXT ?? null;
}
function 获取弹幕系统单位组() {
    return YDUserDataGet("string", "弹幕系统", "单位组", "group");
}
function 写入布尔属性(单位, 属性名, 值) {
    if (值 === undefined)
        return;
    YDUserDataSet("unit", 单位, 属性名, "boolean", 值);
}
function 写入实数属性(单位, 属性名, 值) {
    if (值 === undefined)
        return;
    YDUserDataSet("unit", 单位, 属性名, "real", 值);
}
function 写入单位属性(单位, 属性名, 值) {
    if (值 == null || 值 === 0)
        return;
    YDUserDataSet("unit", 单位, 属性名, "unit", 值);
}
function 归一化生命周期Buff(buff) {
    if (typeof buff === "number")
        return buff;
    if (typeof buff === "string" && buff.length === 4)
        return stringToFourCC(buff);
    return stringToFourCC("BHwe");
}
function 按伤害类型回填布尔标记(参数) {
    if (参数.伤害类型 == null)
        return;
    if (参数.伤害类型 === "攻击特效") {
        参数.攻击特效 = true;
...
export function 注册单位到弹幕系统(弹幕单位, 参数) {
    if (弹幕单位 == null || 弹幕单位 === 0)
        return null;
    if (参数 == null || 参数.主人 == null || 参数.主人 === 0)
        return null;
    设置弹幕外观(弹幕单位, 参数);
    设置弹幕系统属性(弹幕单位, 参数);
    const 单位组 = 获取弹幕系统单位组();
...
export function 快捷注册直线弹幕(弹幕单位, 主人, 飞行速度, 伤害半径, 伤害系数, 额外参数) {
...
export function 注册冲击波弹幕(弹幕单位, 主人, 伤害配置, 半径, 额外参数) {
