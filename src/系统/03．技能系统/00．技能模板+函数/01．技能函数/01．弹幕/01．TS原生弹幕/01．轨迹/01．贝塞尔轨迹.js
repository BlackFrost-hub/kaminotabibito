...
function 取加速度进度(弹幕ID, delta, 路径长度, 初始速度, 加速度, 加速度开始距离) {
    let 状态 = 加速度状态表[弹幕ID];
    if (状态 == null) {
...
export function 创建二阶贝塞尔轨迹(起点X, 起点Y, 控制X, 控制Y, 终点X, 终点Y) {
    return function 二阶贝塞尔采样(实例, _delta) {
        const t = 取弹幕轨迹进度(实例);
...
export function 创建二阶贝塞尔XYZ轨迹(起点X, 起点Y, 起点Z, 控制X, 控制Y, 控制Z, 终点X, 终点Y, 终点Z) {
    return function 二阶贝塞尔XYZ采样(实例, _delta) {
        const t = 取弹幕轨迹进度(实例);
        const x = 二阶贝塞尔值(t, 起点X, 控制X, 终点X);
        const y = 二阶贝塞尔值(t, 起点Y, 控制Y, 终点Y);
...
export function 创建二阶贝塞尔抛物线轨迹(起点X, 起点Y, 起点Z, 控制X, 控制Y, 终点X, 终点Y, 终点Z, 最大抬高) {
    const 控制Z = 起点Z > 终点Z ? 起点Z + 最大抬高 : 终点Z + 最大抬高;
    return 创建二阶贝塞尔XYZ轨迹(起点X, 起点Y, 起点Z, 控制X, 控制Y, 控制Z, 终点X, 终点Y, 终点Z);
}
export function 创建二阶贝塞尔加速度XYZ轨迹(起点X, 起点Y, 起点Z, 控制X, 控制Y, 控制Z, 终点X, 终点Y, 终点Z, 初始速度, 加速度 = 0, 加速度开始距离 = 0) {
    const 路径长度 = 取二阶贝塞尔近似长度(起点X, 起点Y, 控制X, 控制Y, 终点X, 终点Y);
    return function 二阶贝塞尔加速度XYZ采样(实例, delta) {
...
export function 创建二阶贝塞尔加速度抛物线轨迹(起点X, 起点Y, 起点Z, 控制X, 控制Y, 终点X, 终点Y, 终点Z, 最大抬高, 初始速度, 加速度 = 0, 加速度开始距离 = 0) {
    const 控制Z = 起点Z > 终点Z ? 起点Z + 最大抬高 : 终点Z + 最大抬高;
    return 创建二阶贝塞尔加速度XYZ轨迹(起点X, 起点Y, 起点Z, 控制X, 控制Y, 控制Z, 终点X, 终点Y, 终点Z, 初始速度, 加速度, 加速度开始距离);
}
export function 创建锁定单位二阶贝塞尔XYZ轨迹(起点X, 起点Y, 起点Z, 控制X, 控制Y, 控制Z, 目标单位, 目标Z偏移 = 0) {
    return function 锁定单位二阶贝塞尔XYZ采样(实例, _delta) {
        const t = 取弹幕轨迹进度(实例);
...
export function 创建锁定单位二阶贝塞尔加速度XYZ轨迹(起点X, 起点Y, 起点Z, 控制X, 控制Y, 控制Z, 目标单位, 目标Z偏移, 初始速度, 加速度 = 0, 加速度开始距离 = 0) {
    return function 锁定单位二阶贝塞尔加速度XYZ采样(实例, delta) {
        const endX = 目标单位 != null && 目标单位 !== 0 ? GetUnitX(目标单位) : 实例.当前X;
        const endY = 目标单位 != null && 目标单位 !== 0 ? GetUnitY(目标单位) : 实例.当前Y;
        const endZ = 目标单位 != null && 目标单位 !== 0 ? GetUnitFlyHeight(目标单位) + 目标Z偏移 : 起点Z;
...
export function 创建三阶贝塞尔轨迹(起点X, 起点Y, 控制1X, 控制1Y, 控制2X, 控制2Y, 终点X, 终点Y) {
    return function 三阶贝塞尔采样(实例, _delta) {
        const t = 取弹幕轨迹进度(实例);
...
export function 创建三阶贝塞尔XYZ轨迹(起点X, 起点Y, 起点Z, 控制1X, 控制1Y, 控制1Z, 控制2X, 控制2Y, 控制2Z, 终点X, 终点Y, 终点Z) {
    return function 三阶贝塞尔XYZ采样(实例, _delta) {
        const t = 取弹幕轨迹进度(实例);
