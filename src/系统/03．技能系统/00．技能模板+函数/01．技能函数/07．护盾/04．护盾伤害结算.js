...
import { 获取单位护盾实例列表, 删除护盾实例, 取句柄ID } from "./02．护盾实例";
import { 获取可匹配护盾列表 } from "./03．护盾优先级";
import { 显示护盾破碎漂浮文字 } from "./08．护盾回调模板";
const { RMinBJ } = require("lib.扩展函数.BJ函数.12．数学函数");
...
function 取护盾闪色类型(护盾) {
    ...
    if (护盾.类型 === 护盾类型.火)
        return 4;
    if (护盾.类型 === 护盾类型.水 || 护盾.类型 === 护盾类型.冰)
        return 5;
    if (护盾.类型 === 护盾类型.雷)
        return 6;
    if (护盾.类型 === 护盾类型.金 || 护盾.类型 === 护盾类型.毒)
        return 7;
    if (护盾.类型 === 护盾类型.木 || 护盾.类型 === 护盾类型.风)
        return 8;
    if (护盾.类型 === 护盾类型.光)
        return 9;
    if (护盾.类型 === 护盾类型.暗)
...
function 取护盾吸收类型名称(护盾, 伤害) {
    ...
    if (护盾.类型 === 护盾类型.强化)
        return "强化";
    if (护盾.类型 === 护盾类型.金 || 护盾.类型 === 护盾类型.毒)
        return 护盾.类型 === 护盾类型.毒 ? "毒" : "金";
    if (护盾.类型 === 护盾类型.木 || 护盾.类型 === 护盾类型.风)
        return 护盾.类型 === 护盾类型.风 ? "风" : "木";
    if (护盾.类型 === 护盾类型.水 || 护盾.类型 === 护盾类型.冰)
        return 护盾.类型 === 护盾类型.冰 ? "冰" : "水";
    if (护盾.类型 === 护盾类型.火)
        return "火";
    if (护盾.类型 === 护盾类型.雷)
        return "雷";
...
function 构建伤害信息(目标, 伤害值, 是物理伤害, 是魔法伤害, 攻击者, 属性) {
    return {
        目标,
        攻击者,
        伤害值,
 ...
 * @returns 吸收结果
 */
export function 吸收伤害(目标, 伤害值, 是物理伤害, 是魔法伤害, 攻击者, 属性) {
    const 结果 = {
        剩余伤害: 伤害值,
        总吸收量: 0,
        闪色类型: 0,
 ...
 */
export function 注册护盾吸收钩子() {
    if (shieldModifierRegistered)
        return;
    shieldModifierRegistered = true;
    registerDamageModifier((context) => {
        const 结果 = 吸收伤害(context.target, context.currentDamage, context.isPhysicalDamage, context.isMagicDamage, context.attacker, {
        ...
        if (结果.总吸收量 > 0) {
            const g = globalThis;
            const 护盾条闪色 = g._shieldBarFlashColor;
