...
const { onTick10ms, offTick10ms, addDelayedCallback } = require("系统.00．核心系统.05．中心计时器");
...
const DzGetColor = japi.DzGetColor;
...
function 删除实例ID(实例ID) {
    const index = 倒计时实例ID表.indexOf(实例ID);
    if (index >= 0)
        倒计时实例ID表.splice(index, 1);
}
function 尝试关闭中心计时器() {
    if (!已注册中心计时器)
        return;
    if (倒计时实例ID表.length > 0)
        return;
    已注册中心计时器 = false;
    offTick10ms(驱动单位倒计时);
}
...
function 设置倒计时特效颜色(effect, 参数) {
    if (effect == null || effect === 0)
        return;
    const color = DzGetColor(限制到颜色字节(参数.透明度), 限制到颜色字节(参数.红), 限制到颜色字节(参数.绿), 限制到颜色字节(参数.蓝));
    DzSetEffectVertexColor(effect, color);
}
function 结束单位倒计时实例(实例ID, 是否到期) {
...
function 分发单位倒计时到期效果(实例) {
    if (实例.到期效果ID === 强化2效果ID) {
        执行强化2到期效果(实例);
    }
}
function 推进单个单位倒计时(实例) {
    if (单位已死亡(实例.单位)) {
        结束单位倒计时实例(实例.ID, false);
        return;
    }
    if (IsUnitPaused(实例.单位))
        return;
    实例.已经过时间 += 倒计时周期秒;
    if (实例.已经过时间 >= 实例.持续时间) {
...
function on单位倒计时延迟击杀原单位() {
    const unit = 延迟击杀单位队列.shift();
    if (!单位有效(unit))
        return;
    KillUnit(unit);
}
export function 启动单位倒计时核心(参数) {
    if (!单位有效(参数.单位))
        return 0;
    if (!(参数.持续时间 > 0))
        return 0;
    ...
    const effect = EC_CreateEffect(倒计时特效模型, 参数.X, 参数.Y, 默认特效Z, 默认特效朝向, 默认特效缩放, 默认特效速度, -1);
    if (effect != null && effect !== 0) {
        设置倒计时特效颜色(effect, 参数);
