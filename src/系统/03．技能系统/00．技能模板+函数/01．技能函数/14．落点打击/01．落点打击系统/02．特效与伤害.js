 ...
 */
import { AddSpecialEffect, DestroyEffect, UnitDamageTarget, 默认落雷特效, 默认攻击类型, 默认伤害类型, 默认武器类型, 单位是否受影响, } from "./00．共享";
const { 创建渐变圆形提示圈 } = require("系统.03．技能系统.00．技能模板+函数.02．通用函数.09．提示特效");
const { getUnitsInRange } = require("lib.扩展函数.自定义扩展函数.01．选取中心范围");
const { 单位是否还能命中, 记录单位命中 } = require("系统.03．技能系统.00．技能模板+函数.02．通用函数.10．命中规则");
export function 创建落点提示特效(参数, 落点) {
    if (参数.提示特效启用 === false) {
        return;
    }
    ...
    创建渐变圆形提示圈(落点.X, 落点.Y, 提示半径, 落点.触发延迟, 参数.提示特效动画速度);
}
function 创建落点命中特效(参数, X, Y) {
...
export function 结算单次落点伤害(实例, 落点序号) {
    ...
    if (伤害值 > 0 && 实例.参数.伤害半径 > 0) {
        ...
        for (const 单位 of 单位列表) {
            ...
            UnitDamageTarget(实例.参数.所有者 ?? 单位, 单位, 伤害值, false, false, 实例.参数.攻击类型 ?? 默认攻击类型, 实例.参数.伤害类型 ?? 默认伤害类型, 实例.参数.武器类型 ?? 默认武器类型);
            记录单位命中(实例.命中规则状态, 单位);
