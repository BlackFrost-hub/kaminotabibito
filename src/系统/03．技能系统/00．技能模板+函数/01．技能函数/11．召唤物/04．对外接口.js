...
const GetLocationX = jass.GetLocationX;
const GetLocationY = jass.GetLocationY;
...
const { fourCCToString } = require("lib.扩展函数.封装函数.01．通用工具.01．FourCC转换");
...
function 是否有效单位四字码(rawcode) {
    if (!(rawcode > 0))
        return false;
    const slkTable = (globalThis.slk?.unit ?? null);
    if (slkTable == null)
        return false;
    const id = fourCCToString(rawcode);
    return slkTable[id] != null;
}
function 尝试纠正单位四字码(rawcode) {
    if (!(rawcode > 0))
        return rawcode;
    if (是否有效单位四字码(rawcode))
        return rawcode;
    let best = 0;
    ...
    if (best !== 0 && best !== rawcode) {
        debugLogForce(模块名, "纠正损坏 unitType", "raw=", rawcode, "rawStr=", fourCCToString(rawcode), "fixed=", best, "fixedStr=", fourCCToString(best));
        return best;
    }
    return rawcode;
}
function 归一化单位类型(单位类型) {
    if (typeof 单位类型 === "number" && 单位类型 !== 0)
        return 尝试纠正单位四字码(单位类型);
    if (typeof 单位类型 === "string" && 单位类型.length === 4)
        return stringToFourCC(单位类型);
    return undefined;
}
function 解析位置X(参数) {
    if (参数.X != null)
...
function 解析飞行高度(参数) {
    ...
    if (参数.z != null)
        return 参数.z;
    if (参数.moveHeight != null)
        return 参数.moveHeight;
    if (参数.MoveHeight != null)
        return 参数.MoveHeight;
    return undefined;
}
function 解析模型文件(参数) {
    if (参数.模型文件 != null && 参数.模型文件 !== "")
        return 参数.模型文件;
    if (参数.模型路径 != null && 参数.模型路径 !== "")
        return 参数.模型路径;
    if (参数.ModelFileID != null && 参数.ModelFileID !== "")
        return 参数.ModelFileID;
    return undefined;
}
function 规范化召唤物参数输入(参数) {
    return {
...
export function 快捷创建召唤物(主人单位, 单位类型, X, Y, 持续时间, 额外参数) {
    return 创建召唤物({
        主人单位,
        单位类型,
        X,
...
export function SUO_CreateUnit_Loc(所属玩家, uid, loc, z, fac, alpha, red, green, blue, time, b) {
    return 创建召唤物({
        所属玩家,
        uid,
        loc,
...
export function 创建召唤物并套用JASS模板(参数) {
    return 创建召唤物(参数);
}
