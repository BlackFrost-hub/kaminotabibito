...
const { 获取物品次数, 设置物品次数, 增加物品次数, } = require("系统.02．物品系统.15．装备技能.05．物品使用.00．公共.02．物品使用工具");
export function 转移物品次数(来源物品, 目标物品, 数量) {
    if (来源物品 == null || 来源物品 === 0 || 目标物品 == null || 目标物品 === 0 || 数量 <= 0)
        return 0;
...
export function 转移单位物品次数(来源单位, 目标单位, 物品类型ID, 数量) {
    if (来源单位 == null || 来源单位 === 0 || 目标单位 == null || 目标单位 === 0 || 物品类型ID === 0 || 数量 <= 0)
        return 0;
    const 来源物品 = GetItemOfTypeFromUnitBJ(来源单位, 物品类型ID);
    const 目标物品 = GetItemOfTypeFromUnitBJ(目标单位, 物品类型ID);
    if (来源物品 == null || 来源物品 === 0 || 目标物品 == null || 目标物品 === 0)
        return 0;
    return 转移物品次数(来源物品, 目标物品, 数量);
}
export { 获取物品次数, 设置物品次数, 增加物品次数, };
