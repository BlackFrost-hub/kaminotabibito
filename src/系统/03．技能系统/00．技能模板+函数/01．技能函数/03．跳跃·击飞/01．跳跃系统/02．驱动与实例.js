 ...
 */
import { CENTER_TIMER_TICKS, DEFAULT_JUMP_EFFECT_MODEL, X_GAFC, 申请单位暂停占用, 释放单位暂停占用, 单位是否存在其他暂停占用, 零秒后重置单位动画, 活动跳跃列表, 跳跃映射, 单位当前跳跃, 分配新跳跃ID, 取句柄ID, 单位存活, 计算每tick位移, 确保单位可设置飞行高度, 单位已被暂停, GetUnitX, GetUnitY, GetUnitFlyHeight, SetUnitFlyHeight, } from "./00．共享";
...
export function 结束跳跃ID(跳跃ID, 原因) {
    const 实例 = 跳跃映射[跳跃ID];
    if (!实例)
        return false;
    结束跳跃实例(实例, 原因);
...
export function 停止单位跳跃(单位, 原因 = "中断") {
    const 跳跃ID = 单位当前跳跃[取句柄ID(单位)];
    if (!跳跃ID)
        return false;
    return 结束跳跃ID(跳跃ID, 原因);
}
export function on跳跃系统Tick() {
    tick计数 += 1;
...
export function 创建跳跃实例(单位, 角度, 参数) {
    if (!单位存活(单位))
        return 0;
    if (参数.距离 == null || 参数.距离 <= 0)
        return 0;
