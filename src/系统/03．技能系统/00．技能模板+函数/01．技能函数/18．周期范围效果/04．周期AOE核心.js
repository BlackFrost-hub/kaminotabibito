...
const { 创建区域效果 } = require("系统.03．技能系统.00．技能模板+函数.01．技能函数.04．区域效果.区域效果");
...
function 销毁周期范围效果上下文(id) {
    delete 周期范围效果实例表[id];
}
function on周期范围效果周期(区域内单位, 回调上下文ID) {
    const id = 回调上下文ID ?? 0;
...
function on周期范围效果销毁(回调上下文ID) {
    销毁周期范围效果上下文(回调上下文ID ?? 0);
}
export function 启动周期范围效果(参数) {
