/** @noSelfInFile */
import { CENTER_TIMER_TICKS, DEFAULT_ATTACK_TYPE, DEFAULT_DAMAGE_TYPE, DEFAULT_MOVE_EFFECT_MODEL, DEFAULT_WEAPON_TYPE, jass, 活动位移列表, 位移映射, 单位当前位移, 分配新位移ID, 取句柄ID, 单位存活, 清理命中记录, 计算每Tick位移, 单位已被暂停, 申请单位暂停占用, 释放单位暂停占用, 单位是否存在其他暂停占用, 零秒后重置单位动画, } from "./00．共享";
...
export function 结束位移实例(实例, 原因, 命中目标) {
    if (位移映射[实例.id] !== 实例)
        return;
    const 单位 = 实例.单位;
    const 位移ID = 实例.id;
...
export function 结束位移ID(位移ID, 原因, 命中目标) {
    const 实例 = 位移映射[位移ID];
    if (!实例)
        return false;
    结束位移实例(实例, 原因, 命中目标);
...
export function 停止单位位移(单位, 原因 = "中断") {
    const 位移ID = 单位当前位移[取句柄ID(单位)];
    if (!位移ID)
        return false;
    return 结束位移ID(位移ID, 原因);
}
export function on冲锋击退系统Tick() {
    tick计数 += 1;
...
export function 创建位移实例(单位, 角度, 参数) {
    if (!单位存活(单位))
        return 0;
