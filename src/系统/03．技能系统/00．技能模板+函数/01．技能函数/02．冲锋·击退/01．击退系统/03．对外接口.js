/** @noSelfInFile */
import { X_GAFC, jass, SetUnitAnimation, SetUnitTimeScale, 零秒后播放单位动作, 活动位移列表, 位移映射, 单位当前位移, 取句柄ID, 快照单位组, } from "./00．共享";
import { 创建位移实例, 结束位移ID, 停止单位位移 } from "./02．驱动与实例";
function 计算冲锋行走动画倍率(持续时间) {
    if (持续时间 == null || 持续时间 <= 0) {
        return 1.5;
    }
    if (持续时间 >= 1.0) {
...
function 解析冲锋角度(单位, 参数) {
    if (参数.角度 != null)
        return 参数.角度;
    if (参数.目标X != null && 参数.目标Y != null) {
        return X_GAFC(jass.GetUnitX(单位), jass.GetUnitY(单位), 参数.目标X, 参数.目标Y);
    }
    return null;
}
function 解析击退角度(单位, 参数) {
    if (参数.角度 != null)
        return 参数.角度;
    if (参数.来源单位 != null && 参数.来源单位 !== 0) {
        return X_GAFC(jass.GetUnitX(参数.来源单位), jass.GetUnitY(参数.来源单位), jass.GetUnitX(单位), jass.GetUnitY(单位));
    }
    if (参数.来源X != null && 参数.来源Y != null) {
        return X_GAFC(参数.来源X, 参数.来源Y, jass.GetUnitX(单位), jass.GetUnitY(单位));
    }
    return null;
}
export function 开始冲锋(单位, 参数) {
    ...
    const 合并参数 = {
        ...
        结束回调: function (移动单位, 原因, 位移ID, 命中目标) {
...
export function 开始单位组冲锋(单位组, 参数) {
...
export function 开始单位组击退(单位组, 参数) {
    const 单位列表 = 快照单位组(单位组);
    const 结果 = [];
    for (const 单位 of 单位列表) {
...
export function 停止位移(位移ID, 原因 = "中断") {
    return 结束位移ID(位移ID, 原因);
}
export { 停止单位位移 };
export function 单位是否正在位移(单位) {
    const 位移ID = 单位当前位移[取句柄ID(单位)];
    return !!(位移ID && 位移映射[位移ID]);
}
export function 获取单位当前位移ID(单位) {
    return 单位当前位移[取句柄ID(单位)] ?? 0;
}
export function 获取活跃位移数量() {
