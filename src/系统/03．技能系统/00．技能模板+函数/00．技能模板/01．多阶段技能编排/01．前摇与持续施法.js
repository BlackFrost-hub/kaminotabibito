...
const { 开始充能, 停止充能, 停止单位充能, 单位是否正在充能, 获取单位当前充能ID, } = require("系统.03．技能系统.00．技能模板+函数.01．技能函数.06．施法·蓄力·充能.充能系统");
const { 注册施法被打断回调, 取消注册施法被打断回调, } = require("系统.03．技能系统.00．技能模板+函数.01．技能函数.06．施法·蓄力·充能.施法状态");
...
function on技能前摇自我打断(单位, 前摇ID, 方式) {
    const 上下文 = 前摇上下文表[前摇ID];
    if (上下文 == null)
        return;
    上下文.强制结束原因 = "自我打断";
...
export function 停止技能前摇(前摇ID, 原因 = "中断") {
    const 上下文 = 前摇上下文表[前摇ID];
    if (上下文 != null) {
        上下文.强制结束原因 = 原因;
    }
...
export function 停止单位技能前摇(单位, 原因 = "中断") {
...
export function 单位是否正在技能前摇(单位) {
    const 前摇ID = 获取单位当前技能前摇ID(单位);
    return 前摇ID > 0 && 单位是否正在充能(单位);
}
export function 单位是否正在技能施法(单位) {
    return 单位是否正在技能前摇(单位);
}
export function 单位是否正在技能蓄力(单位) {
    return 单位是否正在技能前摇(单位);
}
export function 注册技能施法被打断回调(回调) {
    注册施法被打断回调(回调);
}
export function 取消注册技能施法被打断回调(回调) {
    取消注册施法被打断回调(回调);
}
export function 获取单位当前技能前摇ID(单位) {
...
export function 根据单位句柄ID获取技能前摇ID(单位句柄ID) {
