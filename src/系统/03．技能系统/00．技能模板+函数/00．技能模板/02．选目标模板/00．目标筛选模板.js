...
const { isValidUnit, isUnitEnemy, isUnitAlly } = require("lib.扩展函数.自定义扩展函数.02．条件判断函数");
const { 获取扇形区域单位 } = require("系统.03．技能系统.00．技能模板+函数.01．技能函数.09．形状区域.扇形区域");
const { 获取矩形区域单位 } = require("系统.03．技能系统.00．技能模板+函数.01．技能函数.09．形状区域.矩形区域");
function 计算平方距离(x1, y1, x2, y2) {
    const dx = x2 - x1;
    const dy = y2 - y1;
    return dx * dx + dy * dy;
}
function 单位满足技能筛选条件(单位, 参数) {
    if (!isValidUnit(单位))
        return false;
    if (参数.排除单位 != null && 参数.排除单位 !== 0 && 单位 === 参数.排除单位)
        return false;
    const 影响目标 = 参数.影响目标 ?? "全部";
    if (影响目标 === "敌方") {
        if (参数.来源单位 == null || 参数.来源单位 === 0)
            return false;
        if (!isUnitEnemy(单位, 参数.来源单位))
            return false;
    }
    else if (影响目标 === "友方") {
        if (参数.来源单位 == null || 参数.来源单位 === 0)
            return false;
        if (!isUnitAlly(单位, 参数.来源单位))
            return false;
    }
    const 自定义条件 = 参数.自定义条件;
    if (自定义条件 != null && !自定义条件(单位))
        return false;
    return true;
}
function 获取筛选后单位列表(参数) {
    if (参数.半径 <= 0)
        return [];
...
export function 获取范围内单位组(参数) {
    return 获取筛选后单位列表(参数);
}
export function 选择范围内最近目标(参数) {
...
export function 选择范围内血量最低目标(参数) {
    const 单位列表 = 获取筛选后单位列表(参数);
    let 最佳目标 = null;
    let 最低血量 = 0;
    for (const 单位 of 单位列表) {
...
export function 选择范围内血量最高目标(参数) {
    const 单位列表 = 获取筛选后单位列表(参数);
    let 最佳目标 = null;
    let 最高血量 = 0;
    for (const 单位 of 单位列表) {
...
export function 选择主目标和副目标(参数) {
    const 结果 = {
        主目标: 参数.主目标,
        副目标列表: [],
    };
    if (参数.主目标 == null || 参数.主目标 === 0)
        return 结果;
    if (参数.副目标数量 <= 0)
        return 结果;
    const 副目标筛选参数 = {
...
export function 选择扇形区域内最近目标(参数) {
    const 单位列表 = 获取扇形区域单位({
        X: 参数.X,
        Y: 参数.Y,
        半径: 参数.半径,
...
export function 选择矩形区域内最近目标(参数) {
    const 单位列表 = 获取矩形区域单位({
        X: 参数.X,
        Y: 参数.Y,
        长度: 参数.长度,
