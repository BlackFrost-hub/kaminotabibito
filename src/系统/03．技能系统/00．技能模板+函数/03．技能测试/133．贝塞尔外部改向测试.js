...
import { 创建二阶贝塞尔加速度抛物线轨迹, 创建原生弹幕, 设置原生弹幕指定角度飞行, } from "../01．技能函数/01．弹幕/01．TS原生弹幕/index";
...
function on聊天发射() {
    ...
    const 实例 = 创建原生弹幕({
        ...
        轨迹采样器: 创建二阶贝塞尔加速度抛物线轨迹(startX, startY, 60, controlX, controlY, endX, endY, 60, 240, 320),
        on结束,
    });
    最近弹幕ID = 实例.弹幕ID;
    最近测试目标面向 = face;
