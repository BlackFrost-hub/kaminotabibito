...
function 取朝向终点X(x, 朝向角, 距离) {
    return x + jass.Cos(朝向角 * jass.bj_DEGTORAD) * 距离;
}
function 取朝向终点Y(y, 朝向角, 距离) {
    return y + jass.Sin(朝向角 * jass.bj_DEGTORAD) * 距离;
}
function 冲锋路径区域_结束日志(单位, 原因, 位移ID) {
    debugLogForce(模块名, "冲锋结束，原因=", 原因, "位移ID=", 位移ID, "终点=(", GetUnitX(单位), ",", GetUnitY(单位), ")");
}
function 冲锋路径区域_命中日志(_移动单位, 目标单位, 位移ID, 原因) {
    debugLogForce(模块名, "路径命中：位移ID=", 位移ID, "原因=", 原因, "目标=(", GetUnitX(目标单位), ",", GetUnitY(目标单位), ")");
}
function 执行冲锋路径斩杀(大法师) {
    ...
    debugLogForce(模块名, "已启动测试：位移ID=", 位移ID, "起点=(", 起点X, ",", 起点Y, ") 斩杀终点=(", 目标X, ",", 目标Y, ") 路径长度=", 冲锋距离, " 路径半径=", 路径半径, " 伤害=200");
}
function 前摇开始_播放施法动作(单位, 前摇ID) {
    开始硬直(单位, 前摇时间);
    debugLogForce(模块名, "前摇开始：前摇ID=", 前摇ID, "先硬直，再由前摇模块零秒后播放动作=attack 硬直=", 前摇时间);
}
function 前摇结束_恢复待机动作(单位, 原因, 前摇ID) {
