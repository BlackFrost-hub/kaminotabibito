...
function 执行1014原生追踪测试() {
    ...
    debugLogForce(模块名, "准备发射", "source=", GetUnitName(施法者), "#", GetHandleId(施法者), "target=", GetUnitName(目标单位), "#", GetHandleId(目标单位), "sourcePos=(", GetUnitX(施法者), ",", GetUnitY(施法者), ")", "targetPos=(", GetUnitX(目标单位), ",", GetUnitY(目标单位), ")");
    const 实例 = 创建原生弹幕({
        所有者: 施法者,
        X: GetUnitX(施法者),
        Y: GetUnitY(施法者),
