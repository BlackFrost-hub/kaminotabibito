...
import { 开始纯跳链 } from "../01．技能函数/10．跳链/index";
const 模块名 = "跳链测试";
const 测试命令 = "1010";
const 搜索半径 = 900;
const 最大跳数 = 5;
...
function 跳链测试_每跳回调(单位, 数值, 当前跳数, 跳链ID) {
    debugLogForce(模块名, "每跳命中", "ID=", 跳链ID, " 跳数=", 当前跳数, " 数值=", 数值, " 单位=", GetUnitName(单位), "#", GetHandleId(单位));
}
function 跳链测试_结束回调(原因, 已完成跳数, 跳链ID) {
...
function on聊天1010测试() {
    ...
    debugLogForce(模块名, "已启动跳链测试", " 命令=", 测试命令, " 跳链ID=", 实例.跳链ID, " 最大跳数=", 最大跳数, " 每跳距离=", 每跳最大距离, " 初始伤害=", 初始伤害);
}
