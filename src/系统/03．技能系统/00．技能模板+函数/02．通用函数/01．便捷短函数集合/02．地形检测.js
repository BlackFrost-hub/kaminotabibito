...
const { X_IsTerrainWalkable, X_GetAbleX, X_GetAbleY, } = require("lib.扩展函数.Star扩展函数.Star扩展库.06．X库函数");
...
export function 检测坐标是否可通行(x, y) {
    ...
    if (!X_IsTerrainWalkable(x, y)) {
        ...
        const dist = SquareRoot((可通行X - x) * (可通行X - x) + (可通行Y - y) * (可通行Y - y));
