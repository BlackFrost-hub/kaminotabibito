...
export { GS_Suspend, GS_IsUnitSuspending, GS_LoadSuspend, GS_UnitSuspend, } from "../../../../lib/扩展函数/Star扩展函数/Star扩展库/03．硬直暂停系统";
export { SFB_Init, SFB_setBuff, SFB_setSlow, SFB_施加通用Buff, } from "../../../../lib/扩展函数/Star扩展函数/Star扩展库/04．快速Buff系统";
export { getBuffRuntime, getBuffIdsOnUnit, isUnitInBuffPool, 移除单位指定Buff, } from "../../../05．Buff系统/00．Buff系统";
export { 移除单位指定类型Buff, 移除单位增益Buff, 移除单位负面Buff, 按驱散等级移除单位Buff, 一级驱散单位Buff, 二级驱散单位Buff, } from "../../../05．Buff系统/05．Buff清除函数";
import { GS_Suspend, GS_IsUnitSuspending, GS_LoadSuspend, GS_UnitSuspend, } from "../../../../lib/扩展函数/Star扩展函数/Star扩展库/03．硬直暂停系统";
import { SFB_Init, SFB_setBuff, SFB_setSlow, SFB_施加通用Buff, } from "../../../../lib/扩展函数/Star扩展函数/Star扩展库/04．快速Buff系统";
import { getBuffRuntime, getBuffIdsOnUnit, isUnitInBuffPool, 移除单位指定Buff, } from "../../../05．Buff系统/00．Buff系统";
import { 移除单位指定类型Buff, 移除单位增益Buff, 移除单位负面Buff, 按驱散等级移除单位Buff, 一级驱散单位Buff, 二级驱散单位Buff, } from "../../../05．Buff系统/05．Buff清除函数";
import { 获取单位重伤, 施加重伤, 移除单位重伤, } from "../../../04．伤害系统/03．重伤系统/01．核心功能";
...
function 单位拥有Buff效果(单位, BuffID) {
    if (单位 == null || 单位 === 0 || BuffID == null || BuffID === 0)
        return false;
    return GetUnitAbilityLevel(单位, BuffID) > 0;
}
function 单位拥有任意Buff效果合集(单位, Buff列表) {
    if (单位 == null || 单位 === 0)
        return false;
    let index = 0;
    while (index < Buff列表.length) {
        if (单位拥有Buff效果(单位, Buff列表[index]))
            return true;
        index++;
    }
    return false;
 ...
 */
export function 单位是否处于施法硬直效果(单位) {
    if (单位 == null || 单位 === 0)
        return false;
    return 单位拥有Buff效果(单位, 施法硬直Buff_魔法效果);
}
export function 单位是否处于硬控制效果合集(单位) {
    return 单位拥有任意Buff效果合集(单位, 硬控制Buff合集);
}
export function 单位是否处于软控制效果合集(单位) {
    return 单位拥有任意Buff效果合集(单位, 软控制Buff合集);
}
export function 单位是否处于削弱Buff合集(单位) {
    return 单位拥有任意Buff效果合集(单位, 削弱Buff合集);
}
export function 单位是否处于持续伤害Buff合集(单位) {
    return 单位拥有任意Buff效果合集(单位, 持续伤害Buff合集);
}
export function 单位是否处于负面Buff合集(单位) {
    return 单位拥有任意Buff效果合集(单位, 负面Buff合集);
}
export function 单位是否处于全部负面Buff合集(单位) {
