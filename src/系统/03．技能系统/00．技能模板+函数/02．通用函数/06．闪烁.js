/** @noSelfInFile */
/**
 * 技能通用函数 - 闪烁/瞬移便捷封装
 *
 * 说明：
 * 1. 这是技能侧快捷模板，不修改底层位移系统。
 * 2. 支持开始特效、结束特效、闪烁耗时、闪烁期间隐藏单位、结束后重新选中单位。
 * 3. 重新选中是本地玩家表现：默认让单位拥有者重新选中该单位。
 */
const jass = require("jass.common");
const { onTick10ms, offTick10ms } = require("系统.00．核心系统.05．中心计时器");
const { YDWETimerDestroyEffect } = require("lib.扩展函数.YDWE函数.00．YDWE函数");
const AddSpecialEffect = jass["AddSpecialEffect"];
const GetUnitX = jass["GetUnitX"];
const GetUnitY = jass["GetUnitY"];
const GetUnitFacing = jass["GetUnitFacing"];
const GetOwningPlayer = jass["GetOwningPlayer"];
const GetLocalPlayer = jass["GetLocalPlayer"];
const ClearSelection = jass["ClearSelection"];
const SelectUnit = jass["SelectUnit"];
const ShowUnit = jass["ShowUnit"];
const PauseUnit = jass["PauseUnit"];
const IsUnitPaused = jass["IsUnitPaused"];
const SetUnitFacing = jass["SetUnitFacing"];
const SetUnitPosition = jass["SetUnitPosition"];
const GetUnitState = jass["GetUnitState"];
const TICK_INTERVAL = 0.01;
const UNIT_ALIVE_LIFE = 0.405;
const 活动闪烁列表 = [];
const 闪烁映射 = {};
let 下一个闪烁ID = 1;
let 已注册闪烁Tick = false;
function 单位存活(unit) {
    return unit != null && unit !== 0 && GetUnitState(unit, jass.UNIT_STATE_LIFE) > UNIT_ALIVE_LIFE;
}
function 播放闪烁特效(模型, x, y, 生命周期) {
    if (模型 == null || 模型 === "")
        return;
    const effect = AddSpecialEffect(模型, x, y);
    if (effect == null || effect === 0)
        return;
    YDWETimerDestroyEffect(生命周期, effect);
}
function 本地为单位拥有者重新选中单位(单位) {
    const 拥有者 = GetOwningPlayer(单位);
    if (GetLocalPlayer() === 拥有者) {
        ClearSelection();
        SelectUnit(单位, true);
    }
}
function 结束闪烁实例(实例, 是否完成) {
    delete 闪烁映射[实例.ID];
    const idx = 活动闪烁列表.indexOf(实例);
    if (idx >= 0) {
        活动闪烁列表.splice(idx, 1);
    }
    if (是否完成 && 单位存活(实例.单位)) {
        SetUnitPosition(实例.单位, 实例.目标X, 实例.目标Y);
        if (实例.朝向 != null) {
            SetUnitFacing(实例.单位, 实例.朝向);
        }
        ShowUnit(实例.单位, true);
        if (实例.闪烁期间暂停单位 && !实例.单位原本已暂停) {
            PauseUnit(实例.单位, false);
        }
        播放闪烁特效(实例.结束特效, 实例.目标X, 实例.目标Y, 实例.特效生命周期);
        if (实例.结束后选中单位) {
            本地为单位拥有者重新选中单位(实例.单位);
        }
    }
    else if (实例.闪烁期间暂停单位 && 单位存活(实例.单位) && !实例.单位原本已暂停) {
        PauseUnit(实例.单位, false);
        ShowUnit(实例.单位, true);
    }
    if (活动闪烁列表.length === 0 && 已注册闪烁Tick) {
        已注册闪烁Tick = false;
        offTick10ms(on闪烁Tick);
    }
}
function on闪烁Tick() {
    let i = 0;
    while (i < 活动闪烁列表.length) {
        const 实例 = 活动闪烁列表[i];
        if (!单位存活(实例.单位)) {
            结束闪烁实例(实例, false);
            continue;
        }
        实例.剩余时间 -= TICK_INTERVAL;
        if (实例.剩余时间 <= 0) {
            结束闪烁实例(实例, true);
            continue;
        }
        i += 1;
    }
}
function 注册闪烁Tick() {
    if (已注册闪烁Tick)
        return;
    已注册闪烁Tick = true;
    onTick10ms(on闪烁Tick);
}
export function 开始闪烁(单位, 参数) {
    if (!单位存活(单位))
        return 0;
    const 持续时间 = 参数.持续时间 > 0 ? 参数.持续时间 : 0;
    const 特效生命周期 = 参数.特效生命周期 != null && 参数.特效生命周期 > 0
        ? 参数.特效生命周期
        : 1.0;
    const 闪烁期间暂停单位 = 参数.闪烁期间暂停单位 !== false;
    const 结束后选中单位 = 参数.结束后选中单位 !== false;
    const 当前X = GetUnitX(单位);
    const 当前Y = GetUnitY(单位);
    播放闪烁特效(参数.开始特效, 当前X, 当前Y, 特效生命周期);
    const 原本已暂停 = IsUnitPaused(单位);
    if (闪烁期间暂停单位 && !原本已暂停) {
        PauseUnit(单位, true);
    }
    ShowUnit(单位, false);
    if (持续时间 <= 0) {
        SetUnitPosition(单位, 参数.目标X, 参数.目标Y);
        if (参数.朝向 != null) {
            SetUnitFacing(单位, 参数.朝向);
        }
        else {
            SetUnitFacing(单位, GetUnitFacing(单位));
        }
        ShowUnit(单位, true);
        if (闪烁期间暂停单位 && !原本已暂停) {
            PauseUnit(单位, false);
        }
        播放闪烁特效(参数.结束特效, 参数.目标X, 参数.目标Y, 特效生命周期);
        if (结束后选中单位) {
            本地为单位拥有者重新选中单位(单位);
        }
        return 0;
    }
    const id = 下一个闪烁ID;
    下一个闪烁ID += 1;
    const 实例 = {
        ID: id,
        单位,
        目标X: 参数.目标X,
        目标Y: 参数.目标Y,
        剩余时间: 持续时间,
        朝向: 参数.朝向,
        结束特效: 参数.结束特效,
        特效生命周期,
        闪烁期间暂停单位,
        结束后选中单位,
        单位原本已暂停: 原本已暂停,
    };
    活动闪烁列表.push(实例);
    闪烁映射[id] = 实例;
    注册闪烁Tick();
    return id;
}
export function 停止闪烁(id) {
    const 实例 = 闪烁映射[id];
    if (!实例)
        return;
    结束闪烁实例(实例, false);
}
