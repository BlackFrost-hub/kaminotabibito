...
const { SelectUnitForPlayerSingle } = require("lib.扩展函数.BJ函数.index");
...
/** 触发器 */
let bbTeleportTrigger = null;
/**
 * 按B传送BB事件处理
 */
function onBKeyTeleport() {
    // 检查按键是否为B
 ...
 * 初始化按B传送BB功能
 */
export function initBBTeleport() {
    if (bbTeleportTrigger != null)
        return;
