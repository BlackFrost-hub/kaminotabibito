...
const { applyEquipStatsTS } = require("lib.扩展函数.Star扩展函数.index");
const { AddGoldWithFeedback, fourCCToString } = require("系统.00．核心系统.01．封装函数");
...
function findStatKey(raw) {
    if (KEY_TO_NAME[raw] !== undefined)
        return raw;
...
function addHeroXP(unit, amount) {
    ...
    if (remainder > 0 && typeof jass.AddHeroXP === "function") {
        jass.AddHeroXP(unit, remainder, true);
    }
}
function getHeroLevel(unit) {
    return typeof jass.GetHeroLevel === "function" ? jass.GetHeroLevel(unit) : 1;
}
/**
...
function getPctStatValue(unit, key) {
    if (key === "int")
        return typeof jass.GetHeroInt === "function" ? jass.GetHeroInt(unit, true) : 0;
    if (key === "str")
        return typeof jass.GetHeroStr === "function" ? jass.GetHeroStr(unit, true) : 0;
    if (key === "agi")
        return typeof jass.GetHeroAgi === "function" ? jass.GetHeroAgi(unit, true) : 0;
    if (key === "hp")
        return typeof jass.GetUnitState === "function" ? jass.GetUnitState(unit, jass.ConvertUnitState(1)) : 0;
    if (key === "mp")
        return typeof jass.GetUnitState === "function" ? jass.GetUnitState(unit, jass.ConvertUnitState(3)) : 0;
    if (key === "dmg")
        return typeof jass.GetUnitState === "function" ? jass.GetUnitState(unit, jass.ConvertUnitState(0x15)) : 0;
    if (key === "armor")
        return typeof jass.GetUnitState === "function" ? jass.GetUnitState(unit, jass.ConvertUnitState(0x20)) : 0;
    return 0;
}
...
function applyGoldPct(unit, pct) {
    ...
    const cur = typeof jass.GetPlayerState === "function"
        ? jass.GetPlayerState(player, stateGold) : 0;
...
function executeSegment(unit, seg) {
    ...
    for (const eff of seg.effects) {
        if (eff.type === "gold") {
            ...
            if (add > 0 && typeof jass.SetHeroLevel === "function") {
    ...
    if (goldPct !== 0) {
        if (seg.timeSec <= 0) {
            ...
            const dt = typeof jass.CreateTimer === "function" ? jass.CreateTimer() : undefined;
            if (dt && typeof jass.TimerStart === "function") {
                const t = dt;
                jass.TimerStart(t, 1.0, true, () => {
    ...
    if (statEffects.length > 0) {
        ...
        if (seg.timeSec > 0) {
            ...
            if (dt && typeof jass.TimerStart === "function") {
                const t = dt;
                jass.TimerStart(t, seg.timeSec, false, () => {
...
function onUseItem() {
    const unit = typeof jass.GetManipulatingUnit === "function"
        ? jass.GetManipulatingUnit() : undefined;
    const item = typeof jass.GetManipulatedItem === "function"
        ? jass.GetManipulatedItem() : undefined;
    if (!unit || !item)
        return;
    ...
    if (glob[key])
        return;
    glob[key] = true;
    const ct = typeof jass.CreateTimer === "function" ? jass.CreateTimer() : undefined;
    if (ct && typeof jass.TimerStart === "function") {
        const t = ct;
        jass.TimerStart(t, 0.5, false, () => {
            glob[key] = undefined;
            if (typeof jass.DestroyTimer === "function")
                jass.DestroyTimer(t);
        });
...
function init() {
    ...
    for (let i = 0; i <= 6; i++)
        jass.TriggerRegisterPlayerUnitEvent(trig, jass.Player(i), useItemEv, undefined);
    const p13 = jass.Player?.(13);
    if (p13 != null)
        jass.TriggerRegisterPlayerUnitEvent(trig, p13, useItemEv, undefined);
