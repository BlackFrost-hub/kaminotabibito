...
import { 单位持有攻击效果装备, 单位有效存活, 单位是精英目标, 攻击者类型满足, 单位武器类型满足, 是否攻击效果全局跳过, 距离满足限制, 命中概率通过, 取攻击力, 取力量, 取当前生命, 取最大生命, 取最大魔法, 攻击效果造成伤害, 攻击效果治疗生命魔法, 攻击效果减少生命魔法, 获取敌方范围单位, 播放目标特效, 播放单位坐标特效, 施加攻击效果减速, 施加攻击效果眩晕, 施加攻击效果击飞, 临时修改攻速, 临时修改护甲, } from "./00．公共/01．攻击效果工具";
const { registerAppliedFinalDamageListener, 延后一帧执行伤害派生效果 } = require("系统.04．伤害系统.00．伤害计算.04．主计算流程");
...
function 冷却通过(配置, unit) {
    if (配置.冷却毫秒 == null || 配置.冷却毫秒 <= 0)
        return true;
    ...
    if (last != null && now - last < 配置.冷却毫秒)
        return false;
    攻击效果冷却[key] = now;
    return true;
}
function 基础条件通过(配置, ctx) {
    if (!单位有效存活(ctx.source) || !单位有效存活(ctx.target))
        return false;
    if (配置.仅普通攻击 === true && !(ctx.snapshot != null && ctx.snapshot.isNormalAttack === true))
        return false;
    if (配置.仅物理 === true && !(ctx.snapshot != null && ctx.snapshot.isPhysicalDamage === true))
        return false;
    if (!攻击者类型满足(ctx.source, 配置.攻击者类型))
        return false;
    if (!单位武器类型满足(ctx.source, 配置.需要武器类型))
        return false;
    if (!距离满足限制(ctx.source, ctx.target, 配置.最小距离, 配置.最大距离))
        return false;
    if (!命中概率通过(配置.概率))
        return false;
    if (!冷却通过(配置, ctx.source))
        return false;
    return true;
}
function 计算伤害(配置, ctx) {
    let amount = 配置.固定伤害 ?? 0;
    if (配置.攻击系数 != null)
        amount += 取攻击力(ctx.source) * 配置.攻击系数;
    if (配置.力量系数 != null)
        amount += 取力量(ctx.source) * 配置.力量系数;
    if (配置.生命系数 != null)
        amount += 取最大生命(ctx.target) * 配置.生命系数;
    if (配置.伤害倍率 != null)
        amount += ctx.applied * 配置.伤害倍率;
    return amount;
}
function 执行反击伤害(配置, ctx) {
...
function 执行低血斩杀(配置, ctx) {
    ...
    if (!(line > 0))
        return;
    if (取当前生命(ctx.target) / maxLife > line)
        return;
    攻击效果造成伤害(ctx.source, ctx.target, maxLife, 配置.伤害类型);
}
function 执行范围击飞(配置, ctx) {
    const radius = 配置.范围 ?? 0;
    const list = 获取敌方范围单位(ctx.source, ctx.target, radius, true);
    const amount = 计算伤害(配置, ctx);
    for (let i = 0; i < list.length; i++) {
...
function 执行攻击效果配置(配置, ctx) {
    if (配置.触发侧 === "攻击者" && !单位持有攻击效果装备(ctx.source, 配置.装备名))
        return;
    if (配置.触发侧 === "受击者" && !单位持有攻击效果装备(ctx.target, 配置.装备名))
        return;
    if (!基础条件通过(配置, ctx))
        return;
    const effectCtx = 配置.触发侧 === "受击者" && 配置.效果类型 !== "反击伤害"
        ...
        : ctx;
    if (配置.特效 != null && 配置.特效 !== "")
        播放目标特效(effectCtx.target, 配置.特效);
    if (配置.点特效 != null && 配置.点特效 !== "")
        播放单位坐标特效(effectCtx.target, 配置.点特效, 配置.点特效缩放);
    if (配置.效果类型 === "反击伤害")
        执行反击伤害(配置, ctx);
    else if (配置.效果类型 === "额外伤害")
        执行额外伤害(配置, effectCtx);
    else if (配置.效果类型 === "范围伤害")
        执行范围伤害(配置, effectCtx);
    else if (配置.效果类型 === "持续伤害")
        执行持续伤害(配置, effectCtx);
    else if (配置.效果类型 === "低血斩杀")
        执行低血斩杀(配置, effectCtx);
    else if (配置.效果类型 === "范围击飞")
        执行范围击飞(配置, effectCtx);
    else if (配置.效果类型 === "临时攻速")
        执行临时攻速(配置, effectCtx);
    else if (配置.效果类型 === "护甲削减")
        执行护甲削减(配置, effectCtx);
    else if (配置.效果类型 === "资源偷取")
        执行额外伤害(配置, effectCtx);
}
function on攻击效果最终伤害(target, attacker, applied, snapshot) {
    if (!(applied >= 1))
        return;
    if (snapshot != null && snapshot.isTrueDamage === true)
        return;
    if (snapshot != null && snapshot.isNormalAttack !== true && snapshot.isSkillAttack !== true)
        return;
    if (是否攻击效果全局跳过(attacker, snapshot))
        return;
    const ctx = { source: attacker, target, applied, snapshot };
    const list = 获取攻击效果配置列表();
    for (let i = 0; i < list.length; i++) {
        const cfg = list[i];
        if (cfg == null || cfg.触发侧 === "伤害修正")
            continue;
        执行攻击效果配置(cfg, ctx);
    }
}
function on攻击效果伤害修正(context) {
    let result = context.currentDamage;
    if (!(result >= 1))
        return result;
    if (context.isTrueDamage === true)
        return result;
    if (是否攻击效果全局跳过(context.attacker))
        return result;
    const list = 获取攻击效果配置列表();
    for (let i = 0; i < list.length; i++) {
        const cfg = list[i];
        ...
        if (cfg.效果类型 !== "转换火焰伤害")
            continue;
        if (context.isNormalAttack !== true || context.isPhysicalDamage !== true)
            continue;
        if (!攻击者类型满足(context.attacker, cfg.攻击者类型))
            continue;
        if (!单位持有攻击效果装备(context.attacker, cfg.装备名))
            continue;
...
function on攻击效果持续伤害Tick() {
    const now = getServerTime();
    let write = 0;
    for (let i = 0; i < 持续伤害列表.length; i++) {
        const record = 持续伤害列表[i];
        if (record == null || !单位有效存活(record.source) || !单位有效存活(record.target) || record.remainTicks <= 0)
            continue;
        if (now >= record.nextTime) {
            if (record.effect !== "")
                播放目标特效(record.target, record.effect);
            攻击效果造成伤害(record.source, record.target, record.amount, record.damageType);
    ...
    while (持续伤害列表.length > write)
        持续伤害列表.pop();
}
function on攻击效果临时属性结束() {
    ...
    if (record.type === "攻速")
        ...
        临时修改护甲(record.unit, record.value);
}
export function init攻击效果事件() {
    if (已初始化)
        return;
    已初始化 = true;
