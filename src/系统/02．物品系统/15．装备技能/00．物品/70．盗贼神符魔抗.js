...
export function 处理盗贼神符魔抗使用(ctx) {
    if (!是否为使用物品(ctx.物品, 物品使用装备ID.盗贼神符魔抗))
        return;
    const unit = ctx.施法单位;
    debugLogForce(调试模块名, "使用命中", "unitId=" + GetHandleId(unit), "delta=" + 物品使用数值配置.盗贼神符魔抗.魔抗提升, "durationMs=" + 物品使用数值配置.盗贼神符魔抗.持续毫秒);
    调整玩家属性(unit, "魔抗", 物品使用数值配置.盗贼神符魔抗.魔抗提升);
    回退队列.push(unit);
