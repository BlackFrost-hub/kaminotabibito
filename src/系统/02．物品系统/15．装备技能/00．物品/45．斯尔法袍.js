...
export function 处理斯尔法袍伤害修正(context) {
    const target = context.target;
    if (!单位持有伤害事件装备(target, 伤害事件装备ID.斯尔法袍))
        return context.currentDamage;
    if (context.currentDamage < 取当前生命(target))
        return context.currentDamage;
    if (取当前魔法(target) <= 取最大魔法(target) * 0.2)
        return context.currentDamage;
