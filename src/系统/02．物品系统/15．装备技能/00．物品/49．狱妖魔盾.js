...
const { 快照单位组 } = require("系统.03．技能系统.00．技能模板+函数.02．通用函数.07．单位组工具");
const { 开始护盾, 护盾类型, 查询单位标签护盾值, 充能单位标签护盾, 移除单位标签护盾 } = require("系统.03．技能系统.00．技能模板+函数.01．技能函数.07．护盾");
import { 物品使用装备ID, 物品使用数值配置 } from "../05．物品使用/00．公共/01．物品使用配置表";
import { 是否为使用物品, 单位持有物品, 单位是英雄, 取句柄ID, 单位存活, 取当前生命, 取单位X, 取单位Y, 取最大生命, 设置生命, 获取范围敌人, 造成强化伤害, 施加眩晕, 击退远离来源, } from "../05．物品使用/00．公共/02．物品使用工具";
...
function on拾取狱妖魔盾(单位, 物品) {
    if (!是否为使用物品(物品, 狱妖魔盾物品ID))
        return;
    if (!单位是英雄(单位))
        return;
    加入持有者(单位);
}
function on丢弃狱妖魔盾(单位, 物品) {
    if (!是否为使用物品(物品, 狱妖魔盾物品ID))
        return;
    if (!单位是英雄(单位))
        return;
...
function 尝试充能狱妖魔盾(单位) {
    const id = 取句柄ID(单位);
    if (id === 0)
        return;
    const 冷却到期 = 冷却到期表[id] ?? 0;
    if (冷却到期 > getServerTime())
        return;
    ...
    if (amount > shieldRoom)
        amount = shieldRoom;
    const canPayLife = currentLife - 1;
    if (amount > canPayLife)
        amount = canPayLife;
    if (!(amount > 0))
        return;
...
function 补登记现有持有者() {
    ...
    for (let i = 0; i < 单位列表.length; i++) {
        ...
        if (!单位是英雄(单位))
            continue;
        if (!单位持有物品(单位, 狱妖魔盾物品ID))
            continue;
        加入持有者(单位);
    }
}
function on狱妖魔盾充能Tick() {
    补登记现有持有者();
    for (let i = 持有者列表.length - 1; i >= 0; i--) {
        尝试充能狱妖魔盾(持有者列表[i]);
    }
}
function 开始狱妖魔盾冷却(单位) {
    const id = 取句柄ID(单位);
    if (id === 0)
        return;
    冷却到期表[id] = getServerTime() + 狱妖魔盾配置.冷却毫秒;
}
export function 初始化狱妖魔盾持有充能() {
    if (已初始化)
        return;
    已初始化 = true;
...
export function 处理狱妖魔盾使用(上下文) {
    if (!是否为使用物品(上下文.物品, 狱妖魔盾物品ID))
        return;
    const 单位 = 上下文.施法单位;
    if (!单位是英雄(单位))
        return;
