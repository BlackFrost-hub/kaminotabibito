...
const { 临时调整攻击, 单位存活 } = require("../05．物品使用/00．公共/02．物品使用工具");
...
function 加入守护之盾持有者(unit) {
    const id = 取单位ID(unit);
    if (id === 0 || 守护之盾持有者表[id] != null)
        return;
    守护之盾持有者表[id] = unit;
    守护之盾持有者列表.push(unit);
}
function 移除守护之盾持有者(unit) {
    const id = 取单位ID(unit);
    if (id === 0)
        return;
    delete 守护之盾持有者表[id];
...
function 取或创建守护之盾攻击状态(unit) {
    const id = 取单位ID(unit);
    const state = 守护之盾攻击状态表[id];
    if (state != null)
        return state;
    const nextState = { 当前加成: 0 };
    守护之盾攻击状态表[id] = nextState;
    return nextState;
}
function 清理守护之盾攻击加成(unit) {
    const id = 取单位ID(unit);
    if (id === 0)
        return;
    const state = 守护之盾攻击状态表[id];
...
function on守护之盾攻击同步(unit, currentCount) {
...
function on守护之盾丢弃(unit) {
    清理守护之盾攻击加成(unit);
    移除守护之盾持有者(unit);
}
function on守护之盾获得(unit, currentCount) {
    if (currentCount > 0) {
        加入守护之盾持有者(unit);
    }
}
function on守护之盾失去(unit, currentCount) {
    if (currentCount <= 0) {
        on守护之盾丢弃(unit);
    }
}
function 取转移承受者(target) {
    ...
    for (let i = 0; i < 守护之盾持有者列表.length; i++) {
        ...
        if (holder == null || holder === 0 || holder === target)
            continue;
        if (!单位存活(holder))
            continue;
        if (!IsUnitAlly(holder, owner))
            continue;
        if (获取单位当前持有指定物品数量(holder, 获得物品装备ID.守护之盾) <= 0)
            continue;
