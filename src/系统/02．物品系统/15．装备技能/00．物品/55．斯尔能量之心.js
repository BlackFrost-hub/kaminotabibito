...
export function 处理斯尔能量之心击杀(_dyingUnit, killingUnit) {
    if (!单位持有物品(killingUnit, 物品使用装备ID.斯尔能量之心))
        return;
    const cfg = 物品使用数值配置.斯尔能量之心;
    增加物品次数(killingUnit, 物品使用装备ID.斯尔能量之心, cfg.击杀层数, cfg.触发层数);
}
export function 处理斯尔能量之心使用(ctx) {
    if (!是否为使用物品(ctx.物品, 物品使用装备ID.斯尔能量之心))
        return;
    const unit = ctx.施法单位;
    const cfg = 物品使用数值配置.斯尔能量之心;
    if (获取物品次数(unit, 物品使用装备ID.斯尔能量之心) < cfg.触发层数)
        return;
