...
const { 监听指定物品获取丢弃, 获取单位当前持有指定物品数量 } = require("系统.02．物品系统.15．装备技能.06．获取丢弃.index");
const { 获取单位玩家英雄配置 } = require("系统.01．单位系统.00．单位初始化创建.01．玩家英雄.01．玩家英雄配置工具");
...
const { 创建Dz绑定单位特效, 是否已有Dz绑定单位特效, 销毁Dz绑定单位特效 } = require("lib.扩展函数.封装函数.01．通用工具.03．特效");
...
function 同步冥炎之裙持有表现(unit, currentCount) {
...
function 调整冥炎之裙女性全属性(unit, deltaCount) {
    if (!单位是女性英雄(unit) || deltaCount === 0)
        return;
    SGSS_SetState(unit, 6, 冥炎之裙配置.女性全属性加成 * deltaCount);
}
function on获得冥炎之裙(unit, _item, currentCount, previousCount) {
    if (!单位是英雄(unit))
        return;
...
function on失去冥炎之裙(unit, _item, currentCount, previousCount) {
    if (!单位是英雄(unit))
        return;
