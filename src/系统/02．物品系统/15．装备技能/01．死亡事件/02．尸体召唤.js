...
const 当前生命状态 = jass.UNIT_STATE_LIFE;
const 死亡单位类型 = jass.UNIT_TYPE_DEAD;
const IsUnitType = jass.IsUnitType;
const IsUnitEnemy = jass.IsUnitEnemy;
const GetUnitState = jass.GetUnitState;
function 是否符合持盾召唤条件(单位, 上下文, 物品四字码) {
    if (单位 == null || 单位 === 0)
        return false;
    if (IsUnitType(单位, 死亡单位类型))
        return false;
    if (GetUnitState(单位, 当前生命状态) <= 0.405)
        return false;
    if (IsUnitType(单位, 机械单位类型))
        return false;
    if (IsUnitType(单位, 远古单位类型))
        return false;
    if (!IsUnitEnemy(单位, 上下文.死亡单位所有者))
        return false;
    return itemJudgeFns.UnitHasItemOfTypeBJ(单位, 物品四字码);
}
function 计算召唤生命值(持有者, 基础值, 系数) {
    return 基础值 + jass.GetUnitState(持有者, 最大生命状态) * 系数;
}
function 计算召唤攻击力(持有者, 基础值, 攻击状态, 系数) {
    return 基础值 + jass.GetUnitState(持有者, jass.ConvertUnitState(攻击状态)) * 系数;
}
function 创建尸体召唤物(持有者, 上下文) {
    ...
    jass.SetUnitState(召唤物, 当前生命状态, jass.GetUnitState(召唤物, 最大生命状态));
    createTimedEffect(配置.特效路径, 上下文.死亡坐标X, 上下文.死亡坐标Y, 0, 配置.特效持续时间);
}
export function 处理尸体召唤(上下文) {
    const 配置 = 获取死亡事件配置().尸体召唤;
