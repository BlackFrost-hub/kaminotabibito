...
const items = require("系统.02．物品系统.01．装备数据").default;
...
const { fourCCToString, isSpecialUnit } = require("系统.00．核心系统.01．封装函数");
...
if (!NAME_TO_KEY["移速"])
    NAME_TO_KEY["移速"] = "moveSpeed"; // JASS TempMoveSpeed 用，不参与 addStat
...
function initEvents() {
    ...
    jass.TriggerAddAction(trig, () => {
        ...
        let primary = {};
        if (primaryBonus) {
            const typeId = typeof jass.GetUnitTypeId === "function" ? jass.GetUnitTypeId(unit) : 0;
        ...
        const playerName = (typeof jass.GetPlayerName === "function" ? jass.GetPlayerName(owner) : "") ?? "";
        const actionText = isAdd ? "获得" : "丢弃";
        const levelText = itemData.level || "";
        let levelColor;
        if (levelText === "E-" || levelText === "E")
